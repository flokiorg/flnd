...
func TestBudgetAggregatorFilterInputsAuxBudget(t *testing.T) {
	...
	const shortfall = chainutil.Amount(100)
	auxErr := errors.New("aux failure")

	testCases := []struct {
		name       string
		ownBudget  chainutil.Amount
		auxResult  fn.Result[chainutil.Amount]
		expectKept bool
	}{
		{
		...
		},
		{
			...
			ownBudget:  minFee,
			auxResult:  fn.Err[chainutil.Amount](auxErr),
			expectKept: true,
		},
		{
...
func TestBudgetAggregatorSortInputs(t *testing.T) {
	...
	var (
		// Create two budgets.
		budgetLow   = chainutil.Amount(1000)
		budgetHight = budgetLow + chainutil.Amount(1000)
	)
