...
func createP2WKHInput(amt chainutil.Amount) input.Input {
...
func TestBudgetInputSetAddInput(t *testing.T) {
	...
	// Create a testing input with a budget of 100 loki.
	...
	// The set should have a budget of 200 loki.
	require.Equal(t, chainutil.Amount(200), set.Budget())
}
...
func TestNeedWalletInput(t *testing.T) {
	...
	// We now create two pending inputs each has a budget of 100 loki.
	const budget = 100
...
func TestAddWalletInputsNotEnoughInputs(t *testing.T) {
	...
	// Assume the desired budget is 10k loki.
	const budget = 10_000
	...
	// Create a pending input that requires 10k loki.
	pi := &SweeperInput{
		Input:  mockInput,
		params: Params{Budget: budget},
	}
