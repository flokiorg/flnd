...
func (b *BudgetInputSet) attachExtraBudget(s fn.Option[AuxSweeper]) error {
	extraBudget, err := fn.MapOptionZ(
		s, func(aux AuxSweeper) fn.Result[chainutil.Amount] {
...
func (b *BudgetInputSet) NeedWalletInput() bool {
	var (
		...
		// other inputs.
		budgetBorrowable chainutil.Amount
	)
	...
	for _, inp := range b.inputs {
		...
		output := chainutil.Amount(inp.SignDesc().Output.Value)
...
func (b *BudgetInputSet) Budget() chainutil.Amount {
	budget := chainutil.Amount(0)
	for _, input := range b.inputs {
...
func (b *BudgetInputSet) inputAmts() (chainutil.Amount, chainutil.Amount) {
	var (
		totalAmt     chainutil.Amount
		spendableAmt chainutil.Amount
	)
