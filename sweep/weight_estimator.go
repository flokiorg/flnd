...
// parent transactions.
func (w *weightEstimator) fee() chainutil.Amount {
...
func (w *weightEstimator) feeWithParent() chainutil.Amount {
	...
	if fee > maxFee {
		...
		childFeeRate := chainfee.SatPerKWeight(
			fee * 1000 / chainutil.Amount(childWeight),
		)
