...
type mSatPerKWeight lnwire.MilliLoki

// String returns a human-readable string of the fee rate.
func (m mSatPerKWeight) String() string {
	s := lnwire.MilliLoki(m)
...
func NewLinearFeeFunction(maxFeeRate chainfee.SatPerKWeight,
	...
	*LinearFeeFunction, error) {
	...
	delta := chainutil.Amount(end - start).MulF64(1000 / float64(l.width))
...
func (l *LinearFeeFunction) feeRateAtPosition(p uint32) chainfee.SatPerKWeight {
	...
	// fee rate in sat/kw.
	feeRateDelta := chainutil.Amount(l.deltaFeeRate).MulF64(float64(p) / 1000)
