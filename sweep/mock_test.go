...
// broadcasts the passed transaction to the Flokicoin network.
...
func (m *MockInputSet) Budget() chainutil.Amount {
	args := m.Called()
...
func (m *MockAuxSweeper) ExtraBudgetForInputs(
	_ []input.Input) fn.Result[chainutil.Amount] {

	args := m.Called()
	amt := args.Get(0)

	return amt.(fn.Result[chainutil.Amount])
}
...
func (*MockAuxSweeper) NotifyBroadcast(_ *BumpRequest, _ *wire.MsgTx,
	_ chainutil.Amount, _ map[wire.OutPoint]int) error {
