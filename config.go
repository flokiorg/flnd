...
const (
	...
	defaultLogFilename        = "flnd.log"
	defaultRPCPort            = 10005
	defaultRESTPort           = 5050
	...
	defaultTorDNSHost              = "ln.myfloki.com"
	...
	// FlokicoinChainName is a string that represents the Flokicoin blockchain.
	FlokicoinChainName = "flokicoin"
...
var (
	...
	DefaultLndDir = chainutil.AppDataDir("flnd", false)
	...
	defaultBtcdDir         = chainutil.AppDataDir(btcdBackendName, false)
	defaultBtcdRPCCertFile = filepath.Join(defaultBtcdDir, "rpc.cert")

	defaultFlokicoindDir = chainutil.AppDataDir(FlokicoinChainName, false)
	...
	defaultFlokicoindEstimateMode = "CONSERVATIVE"
	bitcoindEstimateModes         = [2]string{"ECONOMICAL", defaultFlokicoindEstimateMode}
)
...
type Config struct {
	...
	RawExternalIPs    []string `long:"externalip" description:"Add an ip:port (local addresses we listen on) to advertise to the network (default port 5521 is used if port is not specified). Note: Removing this option does not clear previously advertised addresses; remove them with 'lncli peers updatenodeannouncement --address_remove=host:port'."`
	ExternalHosts     []string `long:"externalhosts" description:"Add a hostname:port that should be periodically resolved to announce IPs for. If a port is not specified, the default (5521) will be used."`
	...
	MinChanSize                   int64         `long:"minchansize" description:"The smallest channel size (in loki) that we should accept. Incoming channels smaller than this will be rejected"`
	MaxChanSize                   int64         `long:"maxchansize" description:"The largest channel size (in loki) that we should accept. Incoming channels larger than this will be rejected"`
	...
	DustThreshold uint64 `long:"dust-threshold" description:"DEPRECATED: Sets the max fee exposure in loki for a channel after which HTLC's will be failed." hidden:"true"`

	MaxFeeExposure uint64 `long:"channel-max-fee-exposure" description:" Limits the maximum fee exposure in loki of a channel. This value is enforced for all channels and is independent of the channel initiator."`
	...
	ActiveNetParams chainreg.FlokicoinNetParams
...
func DefaultConfig() Config {
	return Config{
		...
		Flokicoin: &lncfg.Chain{
			MinHTLCIn:     chainreg.DefaultFlokicoinMinHTLCInMSat,
			MinHTLCOut:    chainreg.DefaultFlokicoinMinHTLCOutMSat,
			BaseFee:       chainreg.DefaultFlokicoinBaseFeeMSat,
		...
		FlokicoindMode: &lncfg.Flokicoind{
			Dir:                defaultFlokicoindDir,
			RPCHost:            defaultRPCHost,
			EstimateMode:       defaultFlokicoindEstimateMode,
		...
		ActiveNetParams:           chainreg.FlokicoinTestNetParams,
...
func ValidateConfig(cfg Config, interceptor signal.Interceptor, fileParser,
	...
	cfg.FlokicoindMode.Dir = CleanAndExpandPath(cfg.FlokicoindMode.Dir)
	cfg.FlokicoindMode.ConfigPath = CleanAndExpandPath(
		cfg.FlokicoindMode.ConfigPath,
	)
	cfg.FlokicoindMode.RPCCookie = CleanAndExpandPath(cfg.FlokicoindMode.RPCCookie)
	...
	if err := validateMaxOutgoingCltvExpiry(
		cfg.MaxOutgoingCltvExpiry, cfg.Flokicoin.TimeLockDelta,
	); err != nil {
	...
	// while we're at it.
	numNets := 0
	if cfg.Flokicoin.MainNet {
		numNets++
		cfg.ActiveNetParams = chainreg.FlokicoinMainNetParams
	}
	if cfg.Flokicoin.TestNet3 {
		numNets++
		cfg.ActiveNetParams = chainreg.FlokicoinTestNetParams
	}
	if cfg.Flokicoin.TestNet4 {
		numNets++
		cfg.ActiveNetParams = chainreg.FlokicoinTestNet4Params
	}
	if cfg.Flokicoin.RegTest {
		numNets++
		cfg.ActiveNetParams = chainreg.FlokicoinRegTestNetParams
	}
	if cfg.Flokicoin.SimNet {
		numNets++
		cfg.ActiveNetParams = chainreg.FlokicoinSimNetParams
		...
		// the raw params.
		targetCoinType := chainreg.FlokicoinSigNetParams.CoinType
		cfg.ActiveNetParams.Params.HDCoinType = targetCoinType
	}
	if cfg.Flokicoin.SigNet {
		numNets++
		cfg.ActiveNetParams = chainreg.FlokicoinSigNetParams
		...
		if cfg.Flokicoin.SigNetChallenge != "" {
			challenge, err := hex.DecodeString(
				cfg.Flokicoin.SigNetChallenge,
			)
			if err != nil {
		...
		if len(cfg.Flokicoin.SigNetSeedNode) > 0 {
			sigNetSeeds = make([]chaincfg.DNSSeed, len(
				cfg.Flokicoin.SigNetSeedNode,
			))
			for idx, seed := range cfg.Flokicoin.SigNetSeedNode {
	...
	switch cfg.Flokicoin.Node {
	case btcdBackendName:
		err := parseRPCParams(
			cfg.Flokicoin, cfg.BtcdMode, cfg.ActiveNetParams,
		)
		if err != nil {
	...
	case bitcoindBackendName:
		...
		err := parseRPCParams(
			cfg.Flokicoin, cfg.FlokicoindMode, cfg.ActiveNetParams,
		)
		if err != nil {
			return nil, mkErr("unable to load RPC "+
	...
	cfg.Flokicoin.ChainDir = filepath.Join(
		cfg.DataDir, defaultChainSubDirname, FlokicoinChainName,
	)
	...
	towerDir := filepath.Join(
		cfg.Watchtower.TowerDir, FlokicoinChainName,
		lncfg.NormalizeNetwork(cfg.ActiveNetParams.Name),
	)
	...
	if fileParser != nil && flagParser != nil { // #FLZ_CHANGE
	...
	case cfg.MaxFeeExposure == 0:
		cfg.MaxFeeExposure = uint64(
			htlcswitch.DefaultMaxFeeExposure.ToLokis(),
		)
	}
...
func parseRPCParams(cConfig *lncfg.Chain, nodeConfig interface{},
	netParams chainreg.FlokicoinNetParams) error {
	...
	case *lncfg.Flokicoind:
		...
		confFileBase = FlokicoinChainName
	...
	case bitcoindBackendName:
		nConf := nodeConfig.(*lncfg.Flokicoind)
		rpcUser, rpcPass, zmqBlockHost, zmqTxHost, err :=
			extractFlokicoindRPCParams(netParams.Params.Name,
...
// extractFlokicoindRPCParams attempts to extract the RPC credentials for an
...
func extractFlokicoindRPCParams(networkName, bitcoindDataDir, bitcoindConfigPath,
