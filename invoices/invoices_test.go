...
func randInvoice(value lnwire.MilliLoki) (*invpkg.Invoice, error) {
	var (
...
func testInvoiceCancelSingleHtlc(t *testing.T,
	...
	testInvoice := &invpkg.Invoice{
		Htlcs: map[models.CircuitKey]*invpkg.InvoiceHTLC{},
		Terms: invpkg.ContractTerm{
			Value:           lnwire.NewMSatFromLokis(10000),
			Features:        emptyFeatures,
...
func testInvoiceCancelSingleHtlcAMP(t *testing.T,
	...
	// At this point, we should detect that 3k loki total has been
	// paid.
...
func getUpdateInvoice(htlcID uint64,
	amt lnwire.MilliLoki) invpkg.InvoiceUpdateCallback {
...
func makeAMPInvoiceHTLC(amt lnwire.MilliLoki, setID [32]byte,
...
// settles the invoice with the given amount.
func updateAcceptAMPHtlc(id uint64, amt lnwire.MilliLoki,
...
func testUnexpectedInvoicePreimage(t *testing.T,
	...
	invoice, err := randInvoice(lnwire.MilliLoki(100))
	require.NoError(t, err)
...
func testDeleteInvoices(t *testing.T,
	...
	for i := 0; i < numInvoices; i++ {
		invoice, err := randInvoice(lnwire.MilliLoki(i + 1))
		require.NoError(t, err)
