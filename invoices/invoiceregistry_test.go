...
func testSettleInvoice(t *testing.T,
	...
	amtPaid := lnwire.MilliLoki(100500)
	resolution, err = ctx.registry.NotifyExitHopHtlc(
...
func testCancelHoldInvoice(t *testing.T,
	...
	amtPaid := lnwire.MilliLoki(100000)
	hodlChan := make(chan interface{}, 1)
...
func testMppPaymentWithOverpayment(t *testing.T,
	...
	f := func(overpaymentRand uint64) bool {
		...
		overpayment := lnwire.MilliLoki((overpaymentRand % 999) + 1)
...
func testHeightExpiryWithRegistryImpl(t *testing.T, numParts int, settle bool,
	...
	htlcAmt := testInvoice.Terms.Value / lnwire.MilliLoki(numParts)
...
func testSpontaneousAmpPaymentImpl(
	...
	const (
		totalAmt = lnwire.MilliLoki(360)
		expiry   = uint32(testCurrentHeight + 20)
	)

	var (
		shardAmt = totalAmt / lnwire.MilliLoki(numShards)
		payAddr  [32]byte
		setID    [32]byte
	)
...
func testFailPartialAMPPayment(t *testing.T,
	...
	var (
		shardAmt = testInvoiceAmount / lnwire.MilliLoki(numShards)
		setID    [32]byte
		payAddr  [32]byte
	)
	_, err := rand.Read(payAddr[:])
