...
LND_VERSION_REGEX="flnd version (.+) commit"
...
function check_tag_correct() {
  ...
  go build ${PKG}/cmd/flnd

  # Extract version command output.
  lnd_version_output=$(./flnd --version)
...
required Go version ($goversion)."
  ...
  for i in $sys; do
    ...
    env CGO_ENABLED=0 GOOS=$os GOARCH=$arch GOARM=$arm go build -v -trimpath -ldflags="${ldflags}" -tags="${buildtags}" ${PKG}/cmd/flnd
    env CGO_ENABLED=0 GOOS=$os GOARCH=$arch GOARM=$arm go build -v -trimpath -ldflags="${ldflags}" -tags="${buildtags}" ${PKG}/cmd/flncli
    popd
