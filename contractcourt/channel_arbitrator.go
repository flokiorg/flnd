...
const (
	...
	AnchorOutputValue = chainutil.Amount(330)
)
...
type ContractReport struct {
	...
	// contract.
	LimboBalance chainutil.Amount

	// RecoveredBalance is the total value that has been successfully swept
	// back to the user's wallet.
	RecoveredBalance chainutil.Amount
}
...
func (c *ChannelArbitrator) findCommitmentDeadlineAndValue(heightHint uint32,
	htlcs htlcSet) (fn.Option[int32], chainutil.Amount, error) {

	deadlineMinHeight := uint32(math.MaxUint32)
	totalValue := chainutil.Amount(0)
