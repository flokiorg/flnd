...
func (h *htlcSuccessResolver) checkpointClaim(spendTx *chainhash.Hash) error {
	...
	amt := chainutil.Amount(h.htlcResolution.SweepSignDesc.Output.Value)
...
func (h *htlcSuccessResolver) initReport() {
	...
	finalAmt := h.htlc.Amt.ToLokis()
	if h.htlcResolution.SignedSuccessTx != nil {
		finalAmt = chainutil.Amount(
...
func (h *htlcSuccessResolver) sweepRemoteCommitOutput() error {
	...
	budget := calculateBudget(
		chainutil.Amount(inp.SignDesc().Output.Value),
...
func (h *htlcSuccessResolver) sweepSuccessTx() error {
	...
	// Calculate the budget for this sweep.
	value := chainutil.Amount(secondLevelInput.SignDesc().Output.Value)
	budget := calculateBudget(
