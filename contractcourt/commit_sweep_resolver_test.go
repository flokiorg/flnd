...
func TestCommitSweepResolverNoDelay(t *testing.T) {
	...
	amt := chainutil.Amount(res.SelfOutputSignDesc.Output.Value)
...
func testCommitSweepResolverDelay(t *testing.T, sweepErr error) {
	...
	expectedReport := ContractReport{
		...
		Amount:       chainutil.Amount(amt),
		LimboBalance: chainutil.Amount(amt),
	}
	...
	expectedRecoveredBalance := chainutil.Amount(amt)
	if sweepErr != nil {
