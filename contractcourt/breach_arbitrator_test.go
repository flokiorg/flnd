...
import (
	...
	"github.com/flokiorg/flnd/lntest/channels"
...
func initBreachedOutputs() error {
	for i := range breachedOutputs {
		...
		pubkey, err := crypto.ParsePubKey(breachKeys[i])
		if err != nil {
...
func TestBreachCreateJusticeTx(t *testing.T) {
	...
	aliceKeyPriv, _ := crypto.PrivKeyFromBytes(
...
func createTestArbiter(t *testing.T, contractBreaches chan *ContractBreachEvent,
	...
	aliceKeyPriv, _ := crypto.PrivKeyFromBytes(channels.AlicesPrivKey)
	signer := input.NewMockSigner([]*crypto.PrivateKey{aliceKeyPriv}, nil)
...
// createInitChannels creates two initialized test channels funded with 10 FLC,
// with 5 FLC allocated to each side. Within the channel, Alice is the
// initiator.
func createInitChannels(t *testing.T) (
	*lnwallet.LightningChannel, *lnwallet.LightningChannel, error) {

	aliceKeyPriv, aliceKeyPub := crypto.PrivKeyFromBytes(
		channels.AlicesPrivKey,
	)
	bobKeyPriv, bobKeyPub := crypto.PrivKeyFromBytes(
		channels.BobsPrivKey,
	)

	channelCapacity, err := chainutil.NewAmount(10)
	if err != nil {
		return nil, nil, err
	}
	...
	aliceCfg := channeldb.ChannelConfig{
		ChannelStateBounds: channeldb.ChannelStateBounds{
			MaxPendingAmount: lnwire.MilliLoki(rand.Int63()),
			ChanReserve:      0,
			MinHTLC:          0,
	...
	aliceCommit := channeldb.ChannelCommitment{
		CommitHeight:  0,
		LocalBalance:  lnwire.NewMSatFromLokis(channelBal - commitFee),
		RemoteBalance: lnwire.NewMSatFromLokis(channelBal),
	...
	bobCommit := channeldb.ChannelCommitment{
		CommitHeight:  0,
		LocalBalance:  lnwire.NewMSatFromLokis(channelBal),
		RemoteBalance: lnwire.NewMSatFromLokis(channelBal - commitFee),
		FeePerKw:      chainutil.Amount(feePerKw),
		CommitFee:     commitFee,
	...
	aliceSigner := input.NewMockSigner(
		[]*crypto.PrivateKey{aliceKeyPriv}, nil,
	)
	bobSigner := input.NewMockSigner([]*crypto.PrivateKey{bobKeyPriv}, nil)
...
// TODO(conner) remove code duplication
func createHTLC(data int, amount lnwire.MilliLoki) (*lnwire.UpdateAddHTLC, [32]byte) {
...
func TestUpdateBreachInfoCountsFinalTaprootRevokedFunds(t *testing.T) {
	...
	const revokedAmt = chainutil.Amount(1234)
