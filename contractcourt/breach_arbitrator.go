...
func convertToSecondLevelRevoke(bo *breachedOutput, breachInfo *retributionInfo,
	...
	bo.amt = chainutil.Amount(newAmt)
...
func updateBreachInfo(breachInfo *retributionInfo, spends []spend) (
	chainutil.Amount, chainutil.Amount) {

	inputs := breachInfo.breachedOutputs
	doneOutputs := make(map[int]struct{})

	var totalFunds, revokedFunds chainutil.Amount
...
// Amount returns the number of loki contained in the breached output.
func (bo *breachedOutput) Amount() chainutil.Amount {
	return bo.amt
}
...
func (b *BreachArbitrator) createSweepTx(
	...
	// outputs from the regular, FLC only outputs. So we only need one such
...
func (b *BreachArbitrator) sweepSpendableOutputsTxn(txWeight lntypes.WeightUnit,
	...
	var totalAmt chainutil.Amount
	for _, inp := range inputs {
		totalAmt += chainutil.Amount(inp.SignDesc().Output.Value)
	}
...
func (bo *breachedOutput) Decode(r io.Reader) error {
	...
	bo.amt = chainutil.Amount(binary.BigEndian.Uint64(scratch[:8]))
