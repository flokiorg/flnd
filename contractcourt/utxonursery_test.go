...
func init() {
	...
	for i := range signDescriptors {
		pk, err := crypto.ParsePubKey(keys[i])
		if err != nil {
...
func assertNurseryReport(t *testing.T, nursery *UtxoNursery,
	...
	expectedLimboBalance chainutil.Amount) {
...
func TestKidOutputDecode(t *testing.T) {
	...
	kid := kidOutput{
		breachedOutput: breachedOutput{
			amt:         chainutil.Amount(signDesc.Output.Value),
			outpoint:    op,
