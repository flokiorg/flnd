...
type chainWatcherConfig struct {
	...
	// known to us.
	isOurAddr func(chainutil.Address) bool
...
func (c *chainWatcher) processDetectedSpend(
	...
	// ASYNC PATH: Multiple confirmations (production).
	// We'll now register for confirmation notification for the spending
	// transaction. Once confirmed, we'll advance the state machine to the
	// graduation stage.
...
// being dust.
func (c *chainWatcher) toSelfAmount(tx *wire.MsgTx) chainutil.Amount {
	...
	// Here we define a function capable of identifying whether an output
	// belongs to the flnd wallet. We use this as a heuristic in the case
	...
	// Return the sum.
	return chainutil.Amount(fn.Sum(vals))
}
	...
	for _, out := range closeTx.TxOut {
		if len(localDeliveryScript) > 0 &&
			...
			localCloseOutput = fn.Some(types.CloseOutput{
				Amt:       chainutil.Amount(out.Value),
...
func (c *chainWatcher) dispatchLocalForceClose(
	...
	if resolutions.CommitResolution != nil {
		localBalance := chanSnapshot.LocalBalance.ToLokis()
...
func (c *chainWatcher) dispatchRemoteForceClose(
	commitSpend *chainntnfs.SpendDetail,
	remoteCommit channeldb.ChannelCommitment,
	commitSet CommitSet, commitPoint *crypto.PublicKey) error {
...
func (c *chainWatcher) dispatchContractBreach(spendEvent *chainntnfs.SpendDetail,
	...
	settledBalance := chainSet.remoteCommit.LocalBalance.ToLokis()
	closeSummary := channeldb.ChannelCloseSummary{
		ChanPoint:               c.cfg.chanState.FundingOutpoint,
		ChainHash:               c.cfg.chanState.ChainHash,
...
func (c *chainWatcher) waitForCommitmentPoint() *crypto.PublicKey {
