...
func TestCalculateBudget(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			value:    chainutil.Amount(1000),
			ratio:    0,
			max:      0,
			expected: chainutil.Amount(500),
		},
		{
		...
		},
		{
			...
			value:    chainutil.Amount(1000),
			ratio:    0.1,
			max:      chainutil.Amount(1),
			expected: chainutil.Amount(1),
		},
	}
