...
func (a *OnionPeerActor) Receive(ctx context.Context,
	req *Request) extfn.Result[*Response] {
	...
	case <-ctx.Done():
		...
		return extfn.Err[*Response](ErrActorShuttingDown)
	default:
	}
	...
	log.DebugS(logCtx, "OnionPeerActor received OnionMessage",
		flog.HexN("onion_blob", req.msg.OnionBlob, 10),
	...
	if sendErr := a.updateDispatcher.SendUpdate(update); sendErr != nil {
		...
		return extfn.Err[*Response](sendErr)
	}

	return extfn.Ok(&Response{Success: true})
}
