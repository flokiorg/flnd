...
func pubKeyToArray(pk *crypto.PublicKey) [33]byte {
	var out [33]byte
...
func buildForwardNextNodeHops(
	...
	nextNodeKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	nextNodePub := nextNodeKey.PubKey()

	nextNode := fn.NewLeft[*crypto.PublicKey, lnwire.ShortChannelID](
		nextNodePub,
	)
	...
	privKeys := []*crypto.PrivateKey{h.nodeKey, nextNodeKey}
...
func buildForwardSCIDHops(t *testing.T, h *actorHarness) hopBuildResult {
	...
	nextNode := fn.NewRight[*crypto.PublicKey](scid)
	rdA := record.NewNonFinalBlindedRouteDataOnionMessage(
		nextNode, nil, nil,
	)
	rdB := &record.BlindedRouteData{}
...
func buildDeliverHops(t *testing.T, h *actorHarness) hopBuildResult {
	...
	privKeys := []*crypto.PrivateKey{h.nodeKey}
...
func buildConcatenatedPathHops(
	...
	introNodeKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	introNodePub := introNodeKey.PubKey()

	finalNodeKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	finalNodePub := finalNodeKey.PubKey()

	// Build the receiver's blinded path: introNode -> finalNode.
	nextNodeReceiver := fn.NewLeft[*crypto.PublicKey, lnwire.ShortChannelID](
		finalNodePub,
	)
	...
	nextNodeSender := fn.NewLeft[*crypto.PublicKey, lnwire.ShortChannelID](
		introNodePub,
	)
	...
	privKeys := []*crypto.PrivateKey{h.nodeKey, introNodeKey, finalNodeKey}
...
func TestOnionPeerActorSamePeerCycle(t *testing.T) {
	...
	type nextNodeFn func(h *actorHarness,
		pub *crypto.PublicKey) fn.Either[*crypto.PublicKey,
	...
	tests := []struct {
	...
	}{
		{
			...
			nextNode: func(_ *actorHarness,
				pub *crypto.PublicKey) fn.Either[
				*crypto.PublicKey, lnwire.ShortChannelID] {

				return fn.NewLeft[*crypto.PublicKey,
		...
		},
		{
			...
			nextNode: func(h *actorHarness,
				...
				*crypto.PublicKey, lnwire.ShortChannelID] {
				...
				return fn.NewRight[*crypto.PublicKey](scid)
			},
		},
	}
