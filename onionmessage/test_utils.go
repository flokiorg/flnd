...
type mockNodeIDResolver struct {
	peers map[lnwire.ShortChannelID]*crypto.PublicKey
}
...
func newMockNodeIDResolver() *mockNodeIDResolver {
	return &mockNodeIDResolver{
		peers: make(map[lnwire.ShortChannelID]*crypto.PublicKey),
	}
}
...
func BuildOnionMessage(t *testing.T, blindedPath *sphinx.BlindedPathInfo,
	...
	onionSessionKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
...
func PeelOnionLayers(t *testing.T, privKeys []*crypto.PrivateKey,
...
func deriveNextPathKeyForTest(router *sphinx.Router,
	currentPathKey *crypto.PublicKey,
