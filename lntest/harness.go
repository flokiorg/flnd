...
import (
	...
	"github.com/flokiorg/flnd/kvdb/etcd"
...
// createAndSendOutput send amt loki from the internal mining node to the
...
func (h *HarnessTest) createAndSendOutput(target *node.HarnessNode,
	amt chainutil.Amount, addrType lnrpc.AddressType) {
...
// started. In addition, 5 UTXO of 1 FLC each are sent to the node.
func (h *HarnessTest) NewNodeWithCoins(name string,
	extraArgs []string) *node.HarnessNode {

	node := h.NewNode(name, extraArgs)

	// Load up the wallets of the node with 5 outputs of 1 FLC each.
	const (
		numOutputs  = 5
		fundAmount  = 1 * chainutil.LokiPerFlokicoin
...
type OpenChannelParams struct {
	...
	// SatPerVByte is the amount of loki to spend in chain fees per
	// virtual byte of the transaction.
	SatPerVByte chainutil.Amount
...
// fundCoins attempts to send amt loki from the internal mining node to the
// targeted lightning node. The confirmed boolean indicates whether the
// transaction that pays to the target should confirm. For neutrino backend,
// the `confirmed` param is ignored.
func (h *HarnessTest) fundCoins(amt chainutil.Amount, target *node.HarnessNode,
	...
	if !h.IsNeutrinoBackend() {
		expectedBalance := chainutil.Amount(
	...
	expectedBalance := chainutil.Amount(initialBalance.ConfirmedBalance) + amt
...
// FundCoinsNP2WKH attempts to send amt loki from the internal mining node
// to the targeted lightning node using a NP2WKH address.
func (h *HarnessTest) FundCoinsNP2WKH(amt chainutil.Amount,
...
// FundCoinsP2TR attempts to send amt loki from the internal mining node to
// the targeted lightning node using a P2TR address.
func (h *HarnessTest) FundCoinsP2TR(amt chainutil.Amount,
	target *node.HarnessNode) *wire.MsgTx {
...
// has an amount of 1 FLC. 1 blocks are mined to confirm the tx.
func (h *HarnessTest) FundNumCoins(hn *node.HarnessNode, num int) {
	// Get the initial balance first.
	resp := hn.RPC.WalletBalance()
	initialBalance := chainutil.Amount(resp.ConfirmedBalance)

	const fundAmount = 1 * chainutil.LokiPerFlokicoin
	...
	// Now block until the wallet have fully synced up.
	totalAmount := chainutil.Amount(fundAmount * num)
...
func (h *HarnessTest) OpenChannelPsbt(srcNode, destNode *node.HarnessNode,
	...
	fundingAddr, err := chainutil.DecodeAddress(
	...
	case lnrpc.CommitmentType_SIMPLE_TAPROOT:
		require.IsType(h, &chainutil.AddressTaproot{}, fundingAddr)

	default:
		require.IsType(
			h, &chainutil.AddressWitnessScriptHash{}, fundingAddr,
		)
	}
...
func (h *HarnessTest) CreatePayReqs(hn *node.HarnessNode,
	paymentAmt chainutil.Amount, numInvoices int,
...
func (h *HarnessTest) CalculateTxFee(tx *wire.MsgTx) chainutil.Amount {
	...
	for _, in := range tx.TxIn {
		...
		balance += chainutil.Amount(value)
	}

	for _, out := range tx.TxOut {
		balance -= chainutil.Amount(out.Value)
	}
...
func (h *HarnessTest) SendCoins(a, b *node.HarnessNode,
	amt chainutil.Amount) *wire.MsgTx {
...
// channels are opened. Each node will be funded with a single UTXO of 1 FLC
// except the last one.
//
...
func (h *HarnessTest) createNodes(nodeCfgs [][]string) []*node.HarnessNode {
	...
	for i := 0; i < len(nodes)-1; i++ {
		node := nodes[i]
		h.FundCoinsUnconfirmed(chainutil.LokiPerFlokicoin, node)
	}
...
func (h *HarnessTest) DeriveFundingShim(alice, bob *node.HarnessNode,
	chanSize chainutil.Amount, thawHeight uint32, publish bool,
	...
	*lnrpc.ChannelPoint) {
	...
	if commitType == lnrpc.CommitmentType_SIMPLE_TAPROOT ||
		...
		var carolKey, daveKey *crypto.PublicKey
		carolKey, err = crypto.ParsePubKey(carolFundingKey.RawKeyBytes)
		require.NoError(h, err)
		daveKey, err = crypto.ParsePubKey(daveFundingKey.RawKeyBytes)
		require.NoError(h, err)
