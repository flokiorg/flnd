...
func CalcStaticFee(c lnrpc.CommitmentType, numHTLCs int) chainutil.Amount {
	//nolint:ll
	const (
...
func CalculateMaxHtlc(chanCap chainutil.Amount) uint64 {
	const ratio = 100
	reserve := lnwire.NewMLokFromLokis(chanCap / ratio)
	max := lnwire.NewMLokFromLokis(chanCap) - reserve
...
func CalcStaticFeeBuffer(c lnrpc.CommitmentType, numHTLCs int) chainutil.Amount {
	//nolint:ll
	const (
		htlcWeight         = input.HTLCWeight
		defaultSatPerVByte = lnwallet.DefaultAnchorsCommitMaxFeeRateSatPerVByte
