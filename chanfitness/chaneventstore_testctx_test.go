...
func (c *chanEventStoreTestCtx) newChannel() (route.Vertex, *crypto.PublicKey,
	wire.OutPoint) {

	// Create a pubkey for our channel peer.
	pubKey := crypto.NewPublicKey(
		new(crypto.FieldVal).SetInt(uint16(c.testVarIdx)),
		new(crypto.FieldVal).SetInt(uint16(c.testVarIdx)),
	)
...
func (c *chanEventStoreTestCtx) createChannel() (route.Vertex, *crypto.PublicKey,
	wire.OutPoint) {
...
// been opened.
func (c *chanEventStoreTestCtx) sendChannelOpenedUpdate(pubkey *crypto.PublicKey,
