...
func init() {
	...
	var err error
	sphinxPrivKey, err = crypto.NewPrivateKey()
	if err != nil {
		panic(err)
	}

	// And another, whose public key will serve as the test ephemeral key.
	testEphemeralPriv, err := crypto.NewPrivateKey()
	if err != nil {
		panic(err)
	}
...
var halfCircuitTests = []struct {
	hash      [32]byte
	inValue   chainutil.Amount
	outValue  chainutil.Amount
...
func TestHalfCircuitSerialization(t *testing.T) {
	...
	for i, test := range halfCircuitTests {
		circuit := &htlcswitch.PaymentCircuit{
			...
			IncomingAmount: lnwire.NewMLokFromLokis(test.inValue),
			OutgoingAmount: lnwire.NewMLokFromLokis(test.outValue),
