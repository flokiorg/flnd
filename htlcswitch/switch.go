...
var (
	...
	// set to 500m msats.
	DefaultMaxFeeExposure = lnwire.MilliLoki(500_000_000)
)
...
func (s *Switch) htlcForwarder() {
	...
	var (
		...
		totalSatSent    chainutil.Amount
		totalSatRecv    chainutil.Amount
	)
	...
	for {
		...
		case <-s.cfg.LogEventTicker.Ticks():
			...
			var (
				...
				newSatSent    chainutil.Amount
				newSatRecv    chainutil.Amount
			)
			...
			for _, link := range s.linkIndex {
				...
				newSatSent += sent.ToLokis()
				newSatRecv += recv.ToLokis()
			}
			s.indexMtx.RUnlock()

			var (
				diffNumUpdates uint64
				diffSatSent    chainutil.Amount
				diffSatRecv    chainutil.Amount
			)
			...
			log.Debugf("Sent %d loki and received %d loki "+
...
func (s *Switch) dustExceedsFeeThreshold(link ChannelLink,
	amount lnwire.MilliLoki, incoming bool) bool {
	...
	isLocalDust := isDust(
		feeRate, incoming, lntypes.Local, amount.ToLokis(),
	)
	isRemoteDust := isDust(
		feeRate, incoming, lntypes.Remote, amount.ToLokis(),
	)
