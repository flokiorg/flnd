...
type InvoiceDatabase interface {
	...
	NotifyExitHopHtlc(payHash lntypes.Hash, paidAmount lnwire.MilliLoki,
...
type dustHandler interface {
	...
	getDustSum(whoseCommit lntypes.ChannelParty,
		fee fn.Option[chainfee.SatPerKWeight]) lnwire.MilliLoki
	...
	// getCommitFee returns the commitment fee in loki from either the
	// local or remote commitment. This does not include dust.
	getCommitFee(remote bool) chainutil.Amount
}
...
type ChannelUpdateHandler interface {
	...
	// Bandwidth returns the amount of milli-loki which current link
	...
	// parameter.
	MayAddOutgoingHtlc(lnwire.MilliLoki) error
...
type ChannelLink interface {
	...
	// issue.
	CheckHtlcForward(payHash [32]byte, incomingAmt lnwire.MilliLoki,
	...
	CheckHtlcTransit(payHash [32]byte, amt lnwire.MilliLoki,
		timeout uint32, heightNow uint32,
		customRecords lnwire.CustomRecords) *LinkError

	// Stats return the statistics of channel link. Number of updates,
	// total sent/received milli-loki.
	Stats() (uint64, lnwire.MilliLoki, lnwire.MilliLoki)
	...
	// FLC bandwidth for custom channels. This will always return fn.None()
	// for a regular (non-custom) channel.
	AuxBandwidth(amount lnwire.MilliLoki, cid lnwire.ShortChannelID,
...
type InterceptedPacket struct {
	...
	OutgoingAmount lnwire.MilliLoki
...
type AuxHtlcModifier interface {
	...
	ProduceHtlcExtraData(totalAmount lnwire.MilliLoki,
...
type AuxTrafficShaper interface {
	...
	PaymentBandwidth(fundingBlob, htlcBlob,
		...
		linkBandwidth, htlcAmt lnwire.MilliLoki,
