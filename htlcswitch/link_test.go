...
func TestChannelLinkRevThenSig(t *testing.T) {
	...
	const chanAmt = chainutil.LokiPerFlokicoin * 5
	const chanReserve = chainutil.LokiPerFlokicoin * 1
...
func TestChannelLinkSingleHopPayment(t *testing.T) {
	...
	alice, bob, err := createMirroredChannel(
		t, chainutil.LokiPerFlokicoin*3, chainutil.LokiPerFlokicoin*5,
	)
	...
	amount := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
...
func testChannelLinkInboundFee(t *testing.T, //nolint:thelper
	...
	amount := lnwire.MilliLoki(1_000_000)
	htlcAmt := lnwire.MilliLoki(1_000_000 +
	...
	bobCarolDelta := lnwire.MilliLoki(
...
func TestChannelLinkCancelFullCommitment(t *testing.T) {
	...
	amt := lnwire.NewMSatFromLokis(20000)
...
func TestExitNodeHTLCTimelockExceedsPayload(t *testing.T) {
	...
	channels, _, err := createClusterChannels(
		t, chainutil.LokiPerFlokicoin*5, chainutil.LokiPerFlokicoin*5,
	)
	require.NoError(t, err, "unable to create channel")
	...
	const amount = chainutil.LokiPerFlokicoin
...
func TestExitNodeHTLCUnderpaysPayloadAmount(t *testing.T) {
	...
	channels, _, err := createClusterChannels(
		t, chainutil.LokiPerFlokicoin*5,
		chainutil.LokiPerFlokicoin*5,
	)
	require.NoError(t, err, "unable to create channel")
	...
	f := func(underpaymentRand uint64) bool {
		const amount = chainutil.LokiPerFlokicoin / 100
		underpayment := lnwire.MilliLoki(
...
func TestLinkForwardTimelockPolicyMismatch(t *testing.T) {
	...
	// We'll be sending 1 FLC over a 2-hop (3 vertex) route.
	amount := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
...
func TestLinkForwardFeePolicyMismatch(t *testing.T) {
	...
	// We'll be sending 1 FLC over a 2-hop (3 vertex) route. Given the
	// current default fee of 1 SAT, if we just send a single FLC over in
	// an HTLC, it should be rejected.
	amountNoFee := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
...
func TestLinkForwardMinHTLCPolicyMismatch(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromLokis(1)
...
func TestLinkForwardMaxHTLCPolicyMismatch(t *testing.T) {
	...
	// a new max HTLC of 10 loki.
	maxHtlc := lnwire.NewMSatFromLokis(10)
...
func TestUpdateForwardingPolicy(t *testing.T) {
	...
	amountNoFee := lnwire.NewMSatFromLokis(10)
	...
	newPolicy.BaseFee = lnwire.NewMSatFromLokis(1000)
...
// bob<->alice channel has insufficient FLC capacity/bandwidth. In this test we
...
func TestChannelLinkMultiHopInsufficientPayment(t *testing.T) {
	...
	// We'll attempt to send 4 FLC although the alice-to-bob channel only
	// has 3 FLC total capacity. As a result, this payment should be
	// rejected.
	amount := lnwire.NewMSatFromLokis(4 * chainutil.LokiPerFlokicoin)
	htlcAmt, totalTimelock, hops := generateHops(amount, testStartingHeight,
		n.firstBobChannelLink, n.carolChannelLink)
...
func TestChannelLinkMultiHopDecodeError(t *testing.T) {
	...
	n.carolChannelLink.cfg.ExtractErrorEncrypter = func(
		*crypto.PublicKey) (hop.ErrorEncrypter, lnwire.FailCode) {
...
func (m *mockPeer) IdentityKey() *crypto.PublicKey {
	return nil
}
...
func newSingleLinkTestHarness(t *testing.T, chanAmt,
	chanReserve chainutil.Amount) (singleLinkTestHarness, error) {
	...
	var (
		...
		globalPolicy = models.ForwardingPolicy{
			MinHTLCOut:    lnwire.NewMSatFromLokis(5),
			MaxHTLC:       lnwire.NewMSatFromLokis(chanAmt),
			BaseFee:       lnwire.NewMSatFromLokis(1),
			TimeLockDelta: 6,
		}
	...
	aliceCfg := ChannelLinkConfig{
		...
		ExtractErrorEncrypter: func(*crypto.PublicKey) (
...
func TestChannelLinkBandwidthConsistency(t *testing.T) {
	...
	var (
		...
		chanAmtMSat  = lnwire.NewMSatFromLokis(chanAmt)
	...
	// Next, we'll create an HTLC worth 1 FLC, and send it into the link as
...
func TestChannelLinkTrimCircuitsPending(t *testing.T) {
	...
	const (
		chanAmt   = chainutil.LokiPerFlokicoin * 5
		numHtlcs  = 4
	...
	// We'll start by creating a new link with our chanAmt (5 FLC). We will
	...
	// Next, we'll create an HTLC worth 1 FLC that will be used as a dummy
...
func TestChannelLinkTrimCircuitsRemoteCommit(t *testing.T) {
	...
	// We'll start by creating a new link with our chanAmt (5 FLC).
	harness, err :=
		newSingleLinkTestHarness(t, chanAmt, 0)
	require.NoError(t, err, "unable to create link")
...
func TestChannelLinkBandwidthChanReserve(t *testing.T) {
	...
	var (
		...
		aliceMsgs       chan lnwire.Message
		chanAmtMSat     = lnwire.NewMSatFromLokis(chanAmt)
		chanReserveMSat = lnwire.NewMSatFromLokis(chanReserve)
		cType           = channeldb.SingleFunderTweaklessBit
		commitWeight    = lnwallet.CommitWeight(cType)
	)
	...
	// Next, we'll create an HTLC worth 3 FLC, and send it into the link as
	// a switch initiated payment.  The resulting bandwidth should
	// now be decremented to reflect the new HTLC.
	htlcAmt := lnwire.NewMSatFromLokis(3 * chainutil.LokiPerFlokicoin)
	...
	const bobChanAmt = chainutil.LokiPerFlokicoin * 1
	const bobChanReserve = chainutil.LokiPerFlokicoin * 1.5
...
func TestChannelLinkUpdateCommitFee(t *testing.T) {
	...
	// for this test.
	const aliceInitialBalance = chainutil.LokiPerFlokicoin * 3
	channels, _, err := createClusterChannels(
		t, aliceInitialBalance, chainutil.LokiPerFlokicoin*5,
	)
	require.NoError(t, err, "unable to create channel")
	...
	highFeeExposure := lnwire.NewMSatFromLokis(
		2 * chainutil.LokiPerFlokicoin,
	)
...
func TestChannelLinkAcceptOverpay(t *testing.T) {
	...
	// We'll request a route to send 10k loki via Alice -> Bob ->
	// Carol.
	amount := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	htlcAmt, totalTimelock, hops := generateHops(
		amount, testStartingHeight,
...
func generateHtlcAndInvoice(t *testing.T,
	...
	htlcAmt := lnwire.NewMSatFromLokis(10000)
...
func TestExpectedFee(t *testing.T) {
	testCases := []struct {
		baseFee  lnwire.MilliLoki
		feeRate  lnwire.MilliLoki
		htlcAmt  lnwire.MilliLoki
		expected lnwire.MilliLoki
	}{
		{
			lnwire.MilliLoki(0),
		...
		},
		{
			lnwire.MilliLoki(0),
			lnwire.MilliLoki(1),
			lnwire.MilliLoki(999999),
			lnwire.MilliLoki(0),
		},
		{
			lnwire.MilliLoki(0),
			lnwire.MilliLoki(1),
			lnwire.MilliLoki(1000000),
			lnwire.MilliLoki(1),
		},
		{
			lnwire.MilliLoki(0),
			lnwire.MilliLoki(1),
			lnwire.MilliLoki(1000001),
			lnwire.MilliLoki(1),
		},
		{
			lnwire.MilliLoki(1),
...
type hodlInvoiceTestCtx struct {
	n                   *twoHopNetwork
	startBandwidthAlice lnwire.MilliLoki
	startBandwidthBob   lnwire.MilliLoki
	hash                lntypes.Hash
...
func TestLinkFlushApiDirectionIsolation(t *testing.T) {
	harness, _ :=
		newSingleLinkTestHarness(
			t, 5*chainutil.LokiPerFlokicoin,
			1*chainutil.LokiPerFlokicoin,
		)
	aliceLink := harness.aliceLink
...
func TestLinkQuiescenceExitHopProcessingDeferred(t *testing.T) {
	...
	alice, bob, err := createMirroredChannel(
		t, chainutil.LokiPerFlokicoin, chainutil.LokiPerFlokicoin,
	)
	require.NoError(t, err)
