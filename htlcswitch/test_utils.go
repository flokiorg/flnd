...
func createTestChannel(t *testing.T, alicePrivKey, bobPrivKey []byte,
	aliceAmount, bobAmount, aliceReserve, bobReserve chainutil.Amount,
	chanID lnwire.ShortChannelID) (*testLightningChannel,
	*testLightningChannel, error) {

	aliceKeyPriv, aliceKeyPub := crypto.PrivKeyFromBytes(alicePrivKey)
	bobKeyPriv, bobKeyPub := crypto.PrivKeyFromBytes(bobPrivKey)
	...
	aliceBounds := channeldb.ChannelStateBounds{
		MaxPendingAmount: lnwire.NewMLokFromLokis(
			channelCapacity),
	...
	aliceCommitParams := channeldb.CommitmentParams{
		DustLimit: chainutil.Amount(200),
	...
	bobCommitParams := channeldb.CommitmentParams{
		DustLimit: chainutil.Amount(800),
	...
	aliceCommit := channeldb.ChannelCommitment{
		CommitHeight:  0,
		LocalBalance:  lnwire.NewMLokFromLokis(aliceAmount - commitFee),
		RemoteBalance: lnwire.NewMLokFromLokis(bobAmount),
		CommitFee:     commitFee,
	...
	bobCommit := channeldb.ChannelCommitment{
		CommitHeight:  0,
		LocalBalance:  lnwire.NewMLokFromLokis(bobAmount),
		RemoteBalance: lnwire.NewMLokFromLokis(aliceAmount - commitFee),
		CommitFee:     commitFee,
		FeePerKw:      chainutil.Amount(feePerKw),
	...
	bobSigner := input.NewMockSigner(
		[]*crypto.PrivateKey{bobKeyPriv}, nil,
	)
...
func generatePaymentWithPreimage(invoiceAmt, htlcAmt lnwire.MilliLoki,
...
// invoice which should be added by destination peer.
func generatePayment(invoiceAmt, htlcAmt lnwire.MilliLoki, timelock uint32,
...
// the specified path.
func generateHops(payAmt lnwire.MilliLoki, startingHeight uint32,
	path ...*channelLink) (lnwire.MilliLoki, uint32, []*hop.Payload) {
...
func makePayment(sendingPeer, receivingPeer lnpeer.Peer,
	...
	invoiceAmt, htlcAmt lnwire.MilliLoki,
...
func createClusterChannels(t *testing.T, aliceToBob, bobToCarol chainutil.Amount) (
...
func createMirroredChannel(t *testing.T, aliceToBob,
	bobToAlice chainutil.Amount) (*testLightningChannel,
	*testLightningChannel, error) {
