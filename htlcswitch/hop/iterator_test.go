...
func TestSphinxHopIteratorForwardingInstructions(t *testing.T) {
	...
	expectedFwdInfo := ForwardingInfo{
		...
		AmountToForward: lnwire.MilliLoki(hopData.ForwardAmount),
...
func TestForwardingAmountCalc(t *testing.T) {
	...
	tests := []struct {
		...
		proportional   uint32
		forwardAmount  lnwire.MilliLoki
		expectErr      bool
	}{
		{
			name:           "overflow",
	...
	for _, testCase := range tests {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			require.Equal(t, testCase.forwardAmount.ToLokis(),
				actual.ToLokis())
		})
	}
}
...
func (m *mockProcessor) DecryptBlindedHopData(_ *crypto.PublicKey,
...
func (m *mockProcessor) NextEphemeral(*crypto.PublicKey) (*crypto.PublicKey,
	error) {
...
func TestParseAndValidateRecipientData(t *testing.T) {
	...
	blindedData := record.NewNonFinalBlindedRouteData(
		...
		&record.PaymentConstraints{
			MaxCltvExpiry:   1000,
			HtlcMinimumMsat: lnwire.MilliLoki(1),
		},
		nil,
	)
	...
	tests := []struct {
		...
		data              []byte
		incomingCLTV      uint32
		updateAddBlinding *crypto.PublicKey
		payloadBlinding   *crypto.PublicKey
