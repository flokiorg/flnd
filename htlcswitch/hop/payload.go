...
func NewLegacyPayload(f *sphinx.HopData) *Payload {
	...
	return &Payload{
		FwdInfo: ForwardingInfo{
			...
			AmountToForward: lnwire.MilliLoki(f.ForwardAmount),
...
func ParseTLVPayload(r io.Reader) (*Payload, map[tlv.Type][]byte, error) {
	...
	return &Payload{
		FwdInfo: ForwardingInfo{
			...
			AmountToForward: lnwire.MilliLoki(amt),
			OutgoingCTLV:    cltv,
		},
		MPP:           mpp,
		AMP:           amp,
		...
		totalAmtMsat:  lnwire.MilliLoki(totalAmtMsat),
	}, parsedTypes, nil
}
...
func (h *Payload) BlindingPoint() *crypto.PublicKey {
...
// payee.
func (h *Payload) TotalAmtMsat() lnwire.MilliLoki {
...
func ValidateBlindedRouteData(blindedData *record.BlindedRouteData,
	incomingAmount lnwire.MilliLoki, incomingTimelock uint32) error {
