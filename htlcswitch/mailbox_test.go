...
func TestMailBoxCouriers(t *testing.T) {
	...
	for i := 0; i < numPackets; i++ {
		pkt := &htlcPacket{
			...
			amount:         lnwire.MilliLoki(prand.Int63()),
...
func testMailBoxDust(t *testing.T, chantype channeldb.ChannelType) {
	...
	localDustLimit := chainutil.Amount(400)
	remoteDustLimit := chainutil.Amount(500)
	...
	// htlc channel type.
	firstAmt := lnwire.MilliLoki(600_000)

	if chantype.ZeroHtlcTxFee() {
		firstAmt = lnwire.MilliLoki(450_000)
	}
	...
	localSum, remoteSum = ctx.mailbox.DustPackets()
	require.Equal(t, lnwire.MilliLoki(0), localSum)
	require.Equal(t, lnwire.MilliLoki(0), remoteSum)
}
