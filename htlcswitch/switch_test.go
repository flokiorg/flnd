...
func TestMultiHopPaymentForwardingEvents(t *testing.T) {
	...
	finalAmt := lnwire.NewMLokFromLokis(100000)
	htlcAmt, totalTimelock, hops := generateHops(
...
func TestSwitchDustForwarding(t *testing.T) {
	...
	amt := lnwire.NewMLokFromLokis(700)
	...
	maxFeeExposure := lnwire.NewMLokFromLokis(74500)
	...
	nondustAmt := lnwire.NewMLokFromLokis(10_000)
...
func TestSwitchMailboxDust(t *testing.T) {
	...
	require.NoError(t, err)
	rhash := sha256.Sum256(preimage[:])
	amt := lnwire.NewMLokFromLokis(350)
	addMsg := &lnwire.UpdateAddHTLC{
		PaymentHash: rhash,
		Amount:      amt,
		ChanID:      chanID1,
