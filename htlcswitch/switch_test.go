...
func TestMultiHopPaymentForwardingEvents(t *testing.T) {
	...
	// We'll make now 10 payments, of 100k loki each from Alice to
	// Carol via Bob.
	const numPayments = 10
	finalAmt := lnwire.NewMSatFromLokis(100000)
	htlcAmt, totalTimelock, hops := generateHops(
...
func TestSwitchDustForwarding(t *testing.T) {
	...
	amt := lnwire.NewMSatFromLokis(700)
	...
	maxFeeExposure := lnwire.NewMSatFromLokis(74500)
	...
	nondustAmt := lnwire.NewMSatFromLokis(10_000)
...
func sendDustHtlcs(t *testing.T, n *threeHopNetwork, alice bool,
	amt lnwire.MilliLoki, sid lnwire.ShortChannelID, numHTLCs int) {
...
func TestSwitchMailboxDust(t *testing.T) {
	...
	// to 400 loki and the feerate to 0. We'll fill the mailbox up with
	// dust packets and assert that calls to SendHTLC will fail.
	preimage, err := genPreimage()
	require.NoError(t, err)
	rhash := sha256.Sum256(preimage[:])
	amt := lnwire.NewMSatFromLokis(350)
	addMsg := &lnwire.UpdateAddHTLC{
		PaymentHash: rhash,
		Amount:      amt,
		ChanID:      chanID1,
...
func testSwitchHandlePacketForward(t *testing.T, zeroConf, private,
	...
	harness, err :=
		newSingleLinkTestHarness(t, chainutil.LokiPerFlokicoin, 0)
	require.NoError(t, err)
