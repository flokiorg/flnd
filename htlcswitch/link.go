...
// this given instance. The value returned is expressed in MilliLoki and can
...
func (l *channelLink) exceedsFeeExposureLimit(
	...
	totalLocalDust := localDustSum + lnwire.NewMLokFromLokis(localFee)
	...
	totalRemoteDust := remoteDustSum + lnwire.NewMLokFromLokis(
		remoteFee,
	)
...
func (l *channelLink) isOverexposedWithHtlc(htlc *lnwire.UpdateAddHTLC,
	...
	additional := lnwire.NewMLokFromLokis(
	...
	if isLocalDust {
		...
		localDustSum += lnwire.NewMLokFromLokis(amount)
	} else {
	...
	if isRemoteDust {
		// If this is dust, it doesn't contribute to weight but does
		// contribute to the overall dust sum.
		remoteDustSum += lnwire.NewMLokFromLokis(amount)
	} else {
		// Account for the fee increase that comes with an increase in
		// weight.
