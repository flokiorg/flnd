...
func ExpectedFee(f models.ForwardingPolicy,
	htlcAmt lnwire.MilliLoki) lnwire.MilliLoki {
...
type ChannelLinkConfig struct {
	...
	// MaxFeeExposure is the threshold in milli-loki after which we'll
...
func getResolutionFailure(resolution *invoices.HtlcFailResolution,
	amount lnwire.MilliLoki) *LinkError {
...
//
// NOTE: Part of the ChannelLink interface.
func (l *channelLink) Bandwidth() lnwire.MilliLoki {
...
func (l *channelLink) MayAddOutgoingHtlc(amt lnwire.MilliLoki) error {
...
// getCommitFee returns either the local or remote CommitFee in loki. This
...
//
// NOTE: Part of the dustHandler interface.
func (l *channelLink) getCommitFee(remote bool) chainutil.Amount {
	if remote {
...
func (l *channelLink) exceedsFeeExposureLimit(
	...
	totalLocalDust := localDustSum + lnwire.NewMSatFromLokis(localFee)
	...
	totalRemoteDust := remoteDustSum + lnwire.NewMSatFromLokis(
		remoteFee,
	)
...
func (l *channelLink) isOverexposedWithHtlc(htlc *lnwire.UpdateAddHTLC,
	...
	additional := lnwire.NewMSatFromLokis(
	...
	if isLocalDust {
		...
		localDustSum += lnwire.NewMSatFromLokis(amount)
	} else {
	...
	if isRemoteDust {
		// If this is dust, it doesn't contribute to weight but does
		// contribute to the overall dust sum.
		remoteDustSum += lnwire.NewMSatFromLokis(amount)
	} else {
		// Account for the fee increase that comes with an increase in
		// weight.
...
func dustHelper(chantype channeldb.ChannelType, localDustLimit,
	...
	isDust := func(feerate chainfee.SatPerKWeight, incoming bool,
		...
		var dustLimit chainutil.Amount
...
func (l *channelLink) CheckHtlcForward(payHash [32]byte, incomingHtlcAmt,
	amtToForward lnwire.MilliLoki, incomingTimeout,
...
func (l *channelLink) CheckHtlcTransit(payHash [32]byte,
	amt lnwire.MilliLoki, timeout uint32, heightNow uint32,
	customRecords lnwire.CustomRecords) *LinkError {
...
func (l *channelLink) canSendHtlc(policy models.ForwardingPolicy,
	payHash [32]byte, amt lnwire.MilliLoki, timeout uint32,
	heightNow uint32, originalScid lnwire.ShortChannelID,
	customRecords lnwire.CustomRecords) *LinkError {
	...
	if auxBandwidth.IsHandled && auxBandwidth.Bandwidth.IsSome() {
		auxBandwidth.Bandwidth.WhenSome(
			func(bandwidth lnwire.MilliLoki) {
...
// in milli-satoshi. This might be different from the regular FLC bandwidth for
// custom channels. This will always return fn.None() for a regular (non-custom)
// channel.
func (l *channelLink) AuxBandwidth(amount lnwire.MilliLoki,
...
//
// NOTE: Part of the ChannelLink interface.
func (l *channelLink) Stats() (uint64, lnwire.MilliLoki, lnwire.MilliLoki) {
...
func (l *channelLink) validateHtlcAmount(policy models.ForwardingPolicy,
	payHash [32]byte, amt lnwire.MilliLoki,
