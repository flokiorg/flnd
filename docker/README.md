 ...
 * [Using bitcoind version](#using-bitcoind-version)
   * [Start Flokicoin Node with bitcoind using Docker Compose](#start-bitcoin-node-with-bitcoind-using-docker-compose)
...
that's similar to Flokicoin Core's `regtest` mode. In `simnet` mode blocks can be
...
+ ----- +                   + --- +         create channels and interact with each other using the   
    |                          |            Flokicoin network as source of truth. 
    |                          |            
    + - - - -  - + - - - - - - +            
                 |
        + --------------- +
        | Flokicoin network |  <---  In the current scenario for simplicity we create only one  
        + --------------- +        "btcd" node which represents the Flokicoin network, in a 
                                    real situation Alice and Bob will likely be 
                                    connected to different Flokicoin nodes.
```
...
+ ----- +                   + ------ +                    + --- +        
 ...
 create channels to interact with each other using the Flokicoin network 
 as source of truth.
 
 (3) In current scenario "Alice" and "Faucet" lightning network nodes 
 connect to different Flokicoin nodes. If you decide to connect "Bob"
