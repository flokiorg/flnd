...
func TestConfigToFlatMap(t *testing.T) {
	cfg := DefaultConfig()
	cfg.FlokicoindMode.RPCPass = testPassword
	...
	require.Contains(t, deprecated, "flokicoin.active")
	require.Contains(t, deprecated, "tor.v2")

	// Pick a couple of random values to check.
	require.Equal(t, DefaultLndDir, result["lnddir"])
	require.Equal(
		t, fmt.Sprintf("%v", chainreg.DefaultFlokicoinTimeLockDelta),
		result["flokicoin.timelockdelta"],
	)
	require.Equal(
