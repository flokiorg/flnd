...
func createNode(priv *crypto.PrivateKey) *models.Node {
...
func TestEdgeInsertionDeletion(t *testing.T) {
	...
	copy(edgeInfo.FlokicoinKey1Bytes[:], node1Pub.SerializeCompressed())
	copy(edgeInfo.FlokicoinKey2Bytes[:], node2Pub.SerializeCompressed())
...
func assertEdgeInfoEqual(t *testing.T, e1 *models.ChannelEdgeInfo,
	...
	if !bytes.Equal(e1.FlokicoinKey1Bytes[:], e2.FlokicoinKey1Bytes[:]) {
		t.Fatalf("bitcoinkey1 doesn't match")
	}
	if !bytes.Equal(e1.FlokicoinKey2Bytes[:], e2.FlokicoinKey2Bytes[:]) {
	...
	require.True(t, bytes.Equal(
		e1.AuthProof.FlokicoinSig1Bytes,
		e2.AuthProof.FlokicoinSig1Bytes,
	))
	require.True(t, bytes.Equal(
		e1.AuthProof.FlokicoinSig2Bytes, e2.AuthProof.FlokicoinSig2Bytes,
	))
...
func createChannelEdge(node1, node2 *models.Node,
	...
	*models.ChannelEdgePolicy, *models.ChannelEdgePolicy) {
	...
	copy(edgeInfo.FlokicoinKey1Bytes[:], firstNode[:])
	copy(edgeInfo.FlokicoinKey2Bytes[:], secondNode[:])
...
func newEdgePolicy(chanID uint64, updateTime int64) *models.ChannelEdgePolicy {
	return &models.ChannelEdgePolicy{
		...
		MinHTLC:                   lnwire.MilliLoki(prand.Int63()),
		MaxHTLC:                   lnwire.MilliLoki(prand.Int63()),
		FeeBaseMSat:               lnwire.MilliLoki(prand.Int63()),
		FeeProportionalMillionths: lnwire.MilliLoki(prand.Int63()),
	}
}
...
func fillTestGraph(t testing.TB, graph *ChannelGraph, numNodes,
	...
	for n := 0; n < numNodes-1; n++ {
		...
		for i := 0; i < numChannels; i++ {
			...
			copy(edgeInfo.FlokicoinKey1Bytes[:], node1.PubKeyBytes[:])
			copy(edgeInfo.FlokicoinKey2Bytes[:], node2.PubKeyBytes[:])
			err := graph.AddChannelEdge(ctx, &edgeInfo)
			require.NoError(t, err)
...
func TestGraphPruning(t *testing.T) {
	...
	for i := 0; i < numNodes-1; i++ {
		...
		copy(edgeInfo.FlokicoinKey1Bytes[:], graphNodes[i].PubKeyBytes[:])
		copy(
			edgeInfo.FlokicoinKey2Bytes[:],
...
func TestComputeFee(t *testing.T) {
	var (
		...
		outgoingAmt = lnwire.MilliLoki(1000000)
		expectedFee = lnwire.MilliLoki(40000)
	)
