...
var (
	...
	alicePrivKey, alicePubKey = crypto.PrivKeyFromBytes(
		alicePrivKeyBytes[:],
	)
	...
	bobPrivKey, bobPubKey = crypto.PrivKeyFromBytes(bobPrivKeyBytes[:])
	...
	fundingNetParams = chainreg.FlokicoinTestNetParams
...
func (n *testNode) IdentityKey() *crypto.PublicKey {
...
func createTestFundingManager(t *testing.T, privKey *crypto.PrivateKey,
	...
	fundingCfg := Config{
		...
		RequiredRemoteDelay: func(amt chainutil.Amount) uint16 {
			return 4
		},
		RequiredRemoteChanReserve: func(chanAmt,
			dustLimit chainutil.Amount) chainutil.Amount {
		...
		RequiredRemoteMaxValue: func(chanAmt chainutil.Amount) lnwire.MilliLoki {
			reserve := lnwire.NewMSatFromLokis(chanAmt / 100)
			return lnwire.NewMSatFromLokis(chanAmt) - reserve
		},
...
func openChannel(t *testing.T, alice, bob *testNode, localFundingAmt,
	pushAmt chainutil.Amount, numConfs uint32,
...
func fundChannel(t *testing.T, alice, bob *testNode, localFundingAmt,
	pushAmt chainutil.Amount, subtractFees bool, fundUpToMaxAmt,
	minFundAmt chainutil.Amount, numConfs uint32,
	updateChan chan *lnrpc.OpenStatusUpdate, announceChan bool,
	...
	initReq := &InitFundingMsg{
		...
		PushAmt:         lnwire.NewMSatFromLokis(pushAmt),
...
func assertNumPendingReservations(t *testing.T, node *testNode,
	peerPubKey *crypto.PublicKey, expectedNum int) {
...
func assertChannelAnnouncements(t *testing.T, alice, bob *testNode,
	capacity chainutil.Amount, customMinHtlc []lnwire.MilliLoki,
	customMaxHtlc []lnwire.MilliLoki, baseFees []lnwire.MilliLoki,
	feeRates []lnwire.MilliLoki) {
...
func testNormalWorkflow(t *testing.T, chanType *lnwire.ChannelType) {
	...
	// Run through the process of opening the channel, up until the funding
	// transaction is broadcasted.
	localAmt := chainutil.Amount(500000)
...
func testLocalCSVLimit(t *testing.T, aliceMaxCSV, bobRequiredCSV uint16) {
	...
	bob.fundingMgr.cfg.RequiredRemoteDelay = func(_ chainutil.Amount) uint16 {
...
func TestFundingManagerCustomChannelParameters(t *testing.T) {
	...
	localAmt := chainutil.Amount(5000000)
	pushAmt := chainutil.Amount(0)
	capacity := localAmt + pushAmt
	...
	reserve := lnwire.NewMSatFromLokis(fundingAmt / 100)
	maxValueAcceptChannel := lnwire.NewMSatFromLokis(fundingAmt) - reserve
	...
	assertMinHtlc := func(resCtx *reservationWithCtx,
		expOurMinHtlc, expTheirMinHtlc lnwire.MilliLoki) error {
	...
	assertMaxHtlc := func(resCtx *reservationWithCtx,
		expOurMaxValue, expTheirMaxValue lnwire.MilliLoki) error {
	...
	assertFees := func(forwardingPolicy *models.ForwardingPolicy,
		baseFee, feeRate lnwire.MilliLoki) error {
	...
	minHtlcArr := []lnwire.MilliLoki{5, minHtlcIn}
	...
	baseFees := []lnwire.MilliLoki{
		lnwire.MilliLoki(baseFee), defaultBaseFee,
	}
	feeRates := []lnwire.MilliLoki{
		lnwire.MilliLoki(feeRate), defaultFeeRate,
	}
...
func TestFundingManagerInvalidChanReserve(t *testing.T) {
	...
	var (
		fundingAmt  = chainutil.Amount(500000)
		pushAmt     = lnwire.NewMSatFromLokis(10)
		genesisHash = *fundingNetParams.GenesisHash
	)

	tests := []struct {
		name          string
		chanReserve   chainutil.Amount
		expectErr     bool
...
func TestFundingManagerFundAll(t *testing.T) {
	...
	allCoins := []*lnwallet.Utxo{
		{
			...
			Value: chainutil.Amount(
				0.05 * chainutil.LokiPerFlokicoin,
			),
		...
		},
		{
			AddressType: lnwallet.WitnessPubKey,
			Value: chainutil.Amount(
				0.06 * chainutil.LokiPerFlokicoin,
			),
			PkScript: mock.CoinPkScript,
			OutPoint: wire.OutPoint{
				Hash:  chainhash.Hash{},
	...
	tests := []struct {
	...
	}{
		{
			...
			spendAmt: chainutil.Amount(
				0.11 * chainutil.LokiPerFlokicoin,
			),
			change: false,
		},
		{
			...
			spendAmt: chainutil.Amount(
				0.10 * chainutil.LokiPerFlokicoin,
			),
			change: true,
		},
	}
...
func TestFundingManagerFundMax(t *testing.T) {
	...
	constructTestUtxos := func(values ...chainutil.Amount) []*lnwallet.Utxo {
	...
	tests := []struct {
		name           string
		coins          []*lnwallet.Utxo
		fundUpToMaxAmt chainutil.Amount
		minFundAmt     chainutil.Amount
...
func TestMaxChannelSizeConfig(t *testing.T) {
	...
	alice, bob = setupFundingManagers(t, func(cfg *Config) {
		...
		cfg.MaxChanSize = chainutil.Amount(100000000)
	})
	...
	// Attempt to create a channel above the limit
	// imposed by --maxchansize, which should be rejected.
	initReq.LocalFundingAmt = chainutil.LokiPerFlokicoin + 1
...
func TestWumboChannelConfig(t *testing.T) {
	...
	// should be rejected.
	initReq.LocalFundingAmt = chainutil.LokiPerFlokicoin
...
func testUpfrontFailure(t *testing.T, pkscript []byte, expectErr bool) {
	...
	fundingAmt := chainutil.Amount(500000)
	pushAmt := lnwire.NewMSatFromLokis(chainutil.Amount(0))
...
func TestFundingManagerCoinbase(t *testing.T) {
	...
	const chanSize = chainutil.Amount(1_000_000)
