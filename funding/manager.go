...
type reservationWithCtx struct {
	...
	remoteMinHtlc     lnwire.MilliLoki
	remoteMaxValue    lnwire.MilliLoki
	remoteMaxHtlcs    uint16
	remoteChanReserve chainutil.Amount
...
type InitFundingMsg struct {
	...
	// MaxValueInFlight is the maximum amount of coins in MilliLoki
	// that can be pending within the channel. It only applies to the
	// remote party.
	MaxValueInFlight lnwire.MilliLoki
...
// live pubkey object.
func newSerializedKey(pubKey *crypto.PublicKey) serializedPubKey {
...
type Config struct {
	...
	DefaultMinHtlcIn lnwire.MilliLoki
	...
	NumRequiredConfs func(chainutil.Amount, lnwire.MilliLoki) uint16
	...
	// contract breach.
	RequiredRemoteDelay func(chainutil.Amount) uint16
	...
	// capacity, returns the amount of MilliLokis that our remote peer
	// can have in total outstanding HTLCs with us.
	RequiredRemoteMaxValue func(chainutil.Amount) lnwire.MilliLoki

	// RequiredRemoteMaxHTLCs is a function closure that, given the channel
	// capacity, returns the number of maximum HTLCs the remote peer can
	// offer us.
	RequiredRemoteMaxHTLCs func(chainutil.Amount) uint16
	...
	WatchNewChannel func(*channeldb.OpenChannel, *crypto.PublicKey) error
	...
	NotifyOpenChannelEvent func(wire.OutPoint, *crypto.PublicKey)
	...
	NotifyPendingOpenChannelEvent func(wire.OutPoint,
		*channeldb.OpenChannel, *crypto.PublicKey)
	...
	// zero-conf.
	NotifyFundingTimeout func(wire.OutPoint, *crypto.PublicKey)
...
func (f *Manager) fundeeProcessFundingCreated(peer lnpeer.Peer,
	...
	deleteFromDatabase := func() {
		localBalance := completeChan.LocalCommitment.LocalBalance.ToLokis()
...
func (f *Manager) fundingTimeout(c *channeldb.OpenChannel,
	...
	// channel initiator.
	localBalance := c.LocalCommitment.LocalBalance.ToLokis()
	closeInfo := &channeldb.ChannelCloseSummary{
...
func (f *Manager) receivedChannelReady(node *crypto.PublicKey,
...
func (f *Manager) extractAnnounceParams(c *channeldb.OpenChannel) (
	lnwire.MilliLoki, lnwire.MilliLoki) {
	...
	capacityMSat := lnwire.NewMSatFromLokis(c.Capacity)
...
func (f *Manager) newChanAnnouncement(localPubKey,
	remotePubKey *crypto.PublicKey, localFundingKey *keychain.KeyDescriptor,
	remoteFundingKey *crypto.PublicKey, shortChanID lnwire.ShortChannelID,
	chanID lnwire.ChannelID, fwdMinHTLC, fwdMaxHTLC lnwire.MilliLoki,
	...
	if bytes.Compare(selfBytes, remoteBytes) == -1 {
		...
		copy(
			chanAnn.FlokicoinKey1[:],
			localFundingKey.PubKey.SerializeCompressed(),
		)
		copy(
			chanAnn.FlokicoinKey2[:],
...
func (f *Manager) announceChannel(localIDKey, remoteIDKey *crypto.PublicKey,
...
func (f *Manager) handleInitFundingMsg(msg *InitFundingMsg) {
	...
	if baseFee != nil {
		forwardingPolicy.BaseFee = lnwire.MilliLoki(*baseFee)
	}

	if feeRate != nil {
		forwardingPolicy.FeeRate = lnwire.MilliLoki(*feeRate)
	}
...
// reservation.
func (f *Manager) cancelReservationCtx(peerKey *crypto.PublicKey,
	pendingChanID PendingChanID,
...
func (f *Manager) deleteReservationCtx(peerKey *crypto.PublicKey,
	pendingChanID PendingChanID) {
...
func (f *Manager) getReservationCtx(peerKey *crypto.PublicKey,
...
func copyPubKey(pub *crypto.PublicKey) *crypto.PublicKey {
	var tmp crypto.JacobianPoint
	pub.AsJacobian(&tmp)
	tmp.ToAffine()
	return crypto.NewPublicKey(&tmp.X, &tmp.Y)
}
...
func (f *Manager) waitForPeerOnline(peerPubkey *crypto.PublicKey) (lnpeer.Peer,
	error) {
