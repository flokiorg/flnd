...
type SecretKeyRing interface {
	...
	DerivePrivKey(keyDesc KeyDescriptor) (*crypto.PrivateKey, error)
}
...
type ECDHRing interface {
	...
	ECDH(keyDesc KeyDescriptor, pubKey *crypto.PublicKey) ([32]byte, error)
}
...
type SingleKeyECDH interface {
	...
	// compressed format.
	ECDH(pubKey *crypto.PublicKey) ([32]byte, error)
}
