...
import (
	...
	"github.com/flokiorg/walletd/snacl"
	"github.com/flokiorg/walletd/waddrmgr"
	"github.com/flokiorg/walletd/wallet"
	"github.com/flokiorg/walletd/walletdb"
	_ "github.com/flokiorg/walletd/walletdb/bdb" // Required in order to create the default database.
...
func TestKeyRingDerivation(t *testing.T) {
	...
	keyRingImplementations := []keyRingConstructor{
		func() (string, KeyRing, error) {
			wallet, err := createTestBtcWallet(t, CoinTypeFlokicoin)
			require.NoError(t, err)

			keyRing := NewBtcWalletKeyRing(wallet, CoinTypeFlokicoin)
