...
func TestCalculateFees(t *testing.T) {
	...
	type testCase struct {
		...
		expectedFeeNoChange   chainutil.Amount
		expectedFeeWithChange chainutil.Amount
		expectedErr           error
	}
...
func TestCoinSelect(t *testing.T) {
	...
	const (
		...
		dustLimit = chainutil.Amount(1000)
		fullCoin  = chainutil.LokiPerFlokicoin
	)

	type testCase struct {
		name        string
		outputValue chainutil.Amount
		coins       []wallet.Coin
		changeType  ChangeAddressType

		expectedInput  []chainutil.Amount
		expectedChange chainutil.Amount
		expectErr      bool
	}

	testCases := []testCase{
		{
			// We have 1.0 FLC available, and wants to send 0.5.
		...
		{
			// We have 1 FLC available, and we want to send 1 FLC.
		...
		{
			// We have a 1 FLC input, and want to create an output
...
func TestCalculateChangeAmount(t *testing.T) {
	...
	testCases := []struct {
		name          string
		totalInputAmt chainutil.Amount
		requiredAmt   chainutil.Amount
		feeNoChange   chainutil.Amount
		feeWithChange chainutil.Amount
		dustLimit     chainutil.Amount
		changeType    ChangeAddressType
		maxFeeRatio   float64

		expectErr       string
		expectChangeAmt chainutil.Amount
		expectNeedMore  chainutil.Amount
	}{{
		...
		expectErr: "fee 0.00000045 FLC exceeds max fee (0.00000011 " +
			"FLC) on total output value 0.00000055 FLC",
	}, {
		name:          "fee percent ok",
		totalInputAmt: 100,
		requiredAmt:   50,
...
func TestCoinSelectSubtractFees(t *testing.T) {
	...
	const dustLimit = chainutil.Amount(1000)
	const dust = chainutil.Amount(100)
	...
	type testCase struct {
		name       string
		highFee    bool
		spendValue chainutil.Amount
		coins      []wallet.Coin

		expectedInput      []chainutil.Amount
		expectedFundingAmt chainutil.Amount
		expectedChange     chainutil.Amount
		expectErr          string
	}
	...
	testCases := []testCase{
		{
			// We have 1.0 FLC available, spend them all. This
			...
			coins: []wallet.Coin{
				{
					TxOut: wire.TxOut{
						PkScript: p2wkhScript,
						Value:    1 * chainutil.LokiPerFlokicoin,
					},
				},
			},
			spendValue: 1 * chainutil.LokiPerFlokicoin,

			// The one and only input will be selected.
			expectedInput: []chainutil.Amount{
				1 * chainutil.LokiPerFlokicoin,
			},
			expectedFundingAmt: 1*chainutil.LokiPerFlokicoin - fundingFee(feeRate, 1, false),
			expectedChange:     0,
		},
		{
			// We have 1.0 FLC available and spend half of it. This
			...
			spendValue: 0.5 * chainutil.LokiPerFlokicoin,

			// The one and only input will be selected.
			expectedInput: []chainutil.Amount{
				1 * chainutil.LokiPerFlokicoin,
			},
			expectedFundingAmt: 0.5*chainutil.LokiPerFlokicoin - fundingFee(feeRate, 1, true),
			expectedChange:     0.5 * chainutil.LokiPerFlokicoin,
		},
		{
			...
			expectErr: "output amount(<amt> FLC) after subtracting " +
				"fees(<amt> FLC) below dust limit(<amt> FLC)",
		},
		{
			...
			spendValue: 1*chainutil.LokiPerFlokicoin - dust,
		...
		},
		{
			...
			expectErr: "fee <amt> FLC exceeds max fee (<amt> " +
				"FLC) on total output value <amt> FLC with " +
...
func TestCoinSelectUpToAmount(t *testing.T) {
	...
	const (
		feeRate   = chainfee.SatPerKWeight(100)
		dustLimit = chainutil.Amount(1000)
		dust      = chainutil.Amount(100)
		coin      = chainutil.LokiPerFlokicoin
		minValue  = 20_000
	)

	type testCase struct {
		name     string
		minValue chainutil.Amount
		maxValue chainutil.Amount
		reserved chainutil.Amount
		coins    []wallet.Coin
	...
	testCases := []testCase{{
		// We have 1.0 FLC available, spend them all.
		...
		// The one and only input will be selected.
		expectedInput:      []chainutil.Amount{1 * coin},
		expectedFundingAmt: 1*coin - fundingFee(feeRate, 1, false),
		expectedChange:     0,
	}, {
		// We have 1.0 FLC available and want to spend up to 2 FLC.
		// This should lead to a funding TX with one output, the rest
		// goes to fees.
		name: "spend more",
		coins: []wallet.Coin{{
		...
		// We have 1.0 FLC available and want to spend up to 0.5 FLC.
		...
		// We have 1.0 FLC available and want to spend just 1 Loki
		// below that amount.
		// This should lead to a funding TX with one output where the
		// fee is subtracted from the total 1 FLC input value.
		...
		expectErr: "output amount(0.00000100 FLC) after subtracting " +
			"fees(0.00000048 FLC) below dust limit(0.00001000 FLC)",
	}, {
		...
		expectErr: "fee 0.00000192 FLC exceeds max fee (0.00000153 " +
			"FLC) on total output value 0.00000768 FLC with max " +
			"fee ratio of 0.20",
	}, {
