...
var (
	...
	chanCapacity   chainutil.Amount = 644000
...
func TestPsbtIntent(t *testing.T) {
	...
	_, localPubkey := crypto.PrivKeyFromBytes(localPrivkey)
	_, remotePubkey := crypto.PrivKeyFromBytes(remotePrivkey)
	psbtIntent.BindKeys(
...
func TestVerifyAllInputsSegWit(t *testing.T) {
	testCases := []struct {
		...
		name: "Flokicoind p2sh multisig spend",
