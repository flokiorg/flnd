...
type ErrInsufficientFunds struct {
	amountAvailable chainutil.Amount
	amountSelected  chainutil.Amount
}
...
// change+fees.
func selectInputs(amt chainutil.Amount, coins []wallet.Coin,
	strategy wallet.CoinSelectionStrategy,
	feeRate chainfee.SatPerKWeight) (chainutil.Amount, []wallet.Coin, error) {
	...
	satSelected := chainutil.Amount(0)
	for i, coin := range arrangedCoins {
		satSelected += chainutil.Amount(coin.Value)
		if satSelected >= amt {
...
func calculateFees(utxos []wallet.Coin, feeRate chainfee.SatPerKWeight,
	...
	changeType ChangeAddressType) (chainutil.Amount, chainutil.Amount, error) {
...
// allows.
func sanityCheckFee(totalOut, fee chainutil.Amount, maxFeeRatio float64) error {
	...
	maxFee := chainutil.Amount(float64(totalOut) * maxFeeRatio)
...
// change output to fund amt loki, adhering to the specified fee rate. The
// specified fee rate should be expressed in sat/kw for coin selection to
// function properly.
func CoinSelect(feeRate chainfee.SatPerKWeight, amt, dustLimit chainutil.Amount,
...
func CalculateChangeAmount(totalInputAmt, requiredAmt, requiredFeeNoChange,
	requiredFeeWithChange, dustLimit chainutil.Amount,
	changeType ChangeAddressType, maxFeeRatio float64) (chainutil.Amount,
	chainutil.Amount, error) {
	...
	var changeAmt chainutil.Amount
...
func CoinSelectSubtractFees(feeRate chainfee.SatPerKWeight, amt,
	dustLimit chainutil.Amount, coins []wallet.Coin,
	strategy wallet.CoinSelectionStrategy,
	existingWeight input.TxWeightEstimator, changeType ChangeAddressType,
	maxFeeRatio float64) ([]wallet.Coin, chainutil.Amount, chainutil.Amount,
	error) {
...
func CoinSelectUpToAmount(feeRate chainfee.SatPerKWeight, minAmount, maxAmount,
	reserved, dustLimit chainutil.Amount, coins []wallet.Coin,
	strategy wallet.CoinSelectionStrategy,
	existingWeight input.TxWeightEstimator,
	changeType ChangeAddressType, maxFeeRatio float64) ([]wallet.Coin,
	...
	var totalBalance chainutil.Amount
	for _, coin := range coins {
		totalBalance += chainutil.Amount(coin.Value)
	}
	...
	case err == nil:
		...
		var sumSelected chainutil.Amount
		for _, coin := range selected {
			sumSelected += chainutil.Amount(coin.Value)
		}
	...
	sum := func(coins []wallet.Coin) chainutil.Amount {
		var sum chainutil.Amount
		for _, coin := range coins {
			sum += chainutil.Amount(coin.Value)
		}
