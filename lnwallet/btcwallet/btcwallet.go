...
import (
	...
	btcjsonChainhash "github.com/btcsuite/btcd/chaincfg/chainhash"
	...
	"github.com/flokiorg/walletd/wallet/txrules"
...
func (b *BtcWallet) ConfirmedBalance(confs int32,
	accountFilter string) (chainutil.Amount, error) {
...
func (b *BtcWallet) NewAddress(t lnwallet.AddressType, change bool,
	accountName string) (chainutil.Address, error) {
...
//
// This is a part of the WalletController interface.
func (b *BtcWallet) IsOurAddress(a chainutil.Address) bool {
...
func (b *BtcWallet) AddressInfo(a chainutil.Address) (waddrmgr.ManagedAddress,
	error) {
...
// RequiredReserve returns the minimum amount of loki that should be
// kept in the wallet in order to fee bump anchor channels if necessary.
// The value scales with the number of public anchor channels but is
// capped at a maximum.
func (b *BtcWallet) RequiredReserve(
	numAnchorChans uint32) chainutil.Amount {

	anchorChanReservedValue := lnwallet.AnchorChanReservedValue
	reserved := chainutil.Amount(numAnchorChans) * anchorChanReservedValue
...
func (b *BtcWallet) ListAddresses(name string,
	...
	addressBalance := make(map[string]chainutil.Amount)
	...
	for _, output := range outputs {
		amount, err := chainutil.NewAmount(output.Amount)
		if err != nil {
			return nil, err
		}
...
func (b *BtcWallet) ImportAccount(name string, accountPubKey *hdkeychain.ExtendedKey,
	...
	dryRun bool) (*waddrmgr.AccountProperties, []chainutil.Address,
	[]chainutil.Address, error) {
	...
	externalAddrs := make([]chainutil.Address, len(extAddrs))
	for i := 0; i < len(extAddrs); i++ {
		externalAddrs[i] = extAddrs[i].Address()
	}

	internalAddrs := make([]chainutil.Address, len(intAddrs))
...
//
// This is a part of the WalletController interface.
func (b *BtcWallet) ImportPublicKey(pubKey *crypto.PublicKey,
...
// SendOutputs funds, signs, and broadcasts a Flokicoin transaction paying out to
...
func (b *BtcWallet) SendOutputs(inputs fn.Set[wire.OutPoint],
	...
	// SendOutputs.
	feeSatPerKB := chainutil.Amount(feeRate.FeePerKVByte())
...
// CreateSimpleTx creates a Flokicoin transaction paying to the specified
...
func (b *BtcWallet) ListUnspentWitness(minConfs, maxConfs int32,
	...
	for _, output := range unspentOutputs {
		...
		if addressType == lnwallet.WitnessPubKey ||
			...
			// FLC to loki.
			amt, err := chainutil.NewAmount(output.Amount)
			if err != nil {
				return nil, err
			}
...
// finally broadcasts the passed transaction to the Flokicoin network. If
...
func (b *BtcWallet) PublishTransaction(tx *wire.MsgTx, label string) error {
	...
	// testing mempool acceptance. The default max feerate is 0.10 FLC/kvb,
	// or 10,000 sat/vb.
...
func (b *BtcWallet) SubmitPackage(txns []*wire.MsgTx,
		...
		for _, tx := range txns {
			...
			results[tx.WitnessHash().String()] =
				btcjson.SubmitPackageTxResult{
					TxID: btcjsonChainhash.Hash(txHash),
				}
		}
	...
	// flokicoind does not implement the submitpackage RPC that bitcoind's
	// v3/TRUC package relay depends on, so there's no chain backend call
	// to forward to for the non-neutrino path.
	return nil, fmt.Errorf("package relay is not supported by the " +
		"%v backend", b.chain.BackEnd())
}
...
func satPerVByteToBTCPerKvB(rate chainfee.SatPerVByte) float64 {
	return float64(rate) * vBytesPerKvB / chainutil.LokiPerFlokicoin
}
...
func minedTransactionsToDetails(
	...
	for _, tx := range block.Transactions {
		...
		for i, txOut := range wireTx.TxOut {
			var addresses []chainutil.Address
