...
var (
	...
	testPubKey, _          = crypto.ParsePubKey(testPubKeyBytes)
...
func TestScriptImport(t *testing.T) {
	...
	builder.AddData(chainutil.Hash160([]byte("foobar")))
...
func TestMaybeTweakPrivKey(t *testing.T) {
	...
	require.NoError(t, err)
	doubleTweak, _ := crypto.PrivKeyFromBytes(doubleTweakBytes)

	testCases := []struct {
		name        string
		singleTweak []byte
		doubleTweak *crypto.PrivateKey
		validate    func(*testing.T, *crypto.PrivateKey,
			*crypto.PublicKey)
	}{
		{
			...
			singleTweak: nil,
			doubleTweak: nil,
			validate: func(t *testing.T, result *crypto.PrivateKey,
				_ *crypto.PublicKey) {
