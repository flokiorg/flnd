...
func (b *BtcWallet) GetUtxo(op *wire.OutPoint, pkScript []byte,
	...
	case *chain.RPCClient:
		...
		// We'll ensure we properly convert the amount given in FLC to
		// loki.
		amt, err := chainutil.NewAmount(txout.Value)
		if err != nil {
			return nil, err
		}
	...
	case *chain.LokidClient:
		...
		// Sadly, gettxout returns the output value in FLC instead of
		// loki.
		amt, err := chainutil.NewAmount(txout.Value)
		if err != nil {
			return nil, err
