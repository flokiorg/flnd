...
var (
	...
	testCommitSecret, testCommitPoint = crypto.PrivKeyFromBytes(
	...
	remoteRevocationBasePubKey, _ = crypto.ParsePubKey(
...
func (i testInputType) output(t *testing.T,
	privKey *crypto.PrivateKey) (*wire.TxOut, []byte) {
	...
	switch i {
	case plainP2WKH:
		h := chainutil.Hash160(privKey.PubKey().SerializeCompressed())
		addr, err = chainutil.NewAddressWitnessPubKeyHash(h, netParams)
	...
	case nestedP2WKH:
		...
		addr, err = chainutil.NewAddressScriptHash(
	...
	case singleKeyP2WSH:
		...
		addr, err = chainutil.NewAddressWitnessScriptHash(h[:], netParams)
		require.NoError(t, err)
...
func (i testInputType) decorateInput(t *testing.T, privKey *crypto.PrivateKey,
	in *psbt.PInput) {
