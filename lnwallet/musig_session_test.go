...
func newMuSessionHarness(t *testing.T) *muSessionHarness {
	...
	alicePriv, alicePub := crypto.PrivKeyFromBytes(testWalletPrivKey)
	aliceSigner := input.NewMockSigner([]*crypto.PrivateKey{alicePriv}, nil)
	...
	bobPriv, bobPub := crypto.PrivKeyFromBytes(bobsPrivKey)
	bobSigner := input.NewMockSigner([]*crypto.PrivateKey{bobPriv}, nil)
