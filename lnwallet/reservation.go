...
type ChannelContribution struct {
	...
	FirstCommitmentPoint *crypto.PublicKey
...
type ChannelReservation struct {
	...
	// pushMSat the amount of milli-loki that should be pushed to the
...
func NewChannelReservation(capacity, localFundingAmt chainutil.Amount,
	...
	if localFundingAmt == 0 {
		...
		if int64(theirBalance) < 0 {
			return nil, ErrFunderBalanceDust(
				int64(commitFee), int64(theirBalance.ToLokis()),
				int64(2*defaultDust),
			)
		}
	} else {
	...
	if initiator && ourBalance.ToLokis() <= 2*defaultDust {
		return nil, ErrFunderBalanceDust(
			int64(commitFee),
			int64(ourBalance.ToLokis()),
			int64(2*defaultDust),
		)
	}
	...
	// initiator.
	if !initiator && theirBalance.ToLokis() <= 2*defaultDust {
		return nil, ErrFunderBalanceDust(
			int64(commitFee),
			int64(theirBalance.ToLokis()),
			int64(2*defaultDust),
		)
	}
	...
	return &ChannelReservation{
		ourContribution: &ChannelContribution{
			FundingAmount: ourBalance.ToLokis(),
		...
		partialState: &channeldb.OpenChannel{
			...
			LocalCommitment: channeldb.ChannelCommitment{
				...
				FeePerKw:      chainutil.Amount(req.CommitFeePerKw),
				CommitFee:     commitFee,
			},
...
// of loki that can be transferred in a single commitment. This function
...
func (r *ChannelReservation) Capacity() chainutil.Amount {
	r.RLock()
	defer r.RUnlock()
...
func VerifyConstraints(bounds *channeldb.ChannelStateBounds,
	...
	channelCapacity chainutil.Amount) error {
