...
type CloseSigner interface {
	...
	CompleteCooperativeClose(localSig, remoteSig input.Signature,
		...
		closeOpt ...lnwallet.ChanCloseOpt) (*wire.MsgTx, chainutil.Amount,
		error)
}
...
type Environment struct {
	...
	// loki we'll pay given a local and/or remote output.
...
func (c *CloseChannelTerms) DeriveCloseTxOuts() (*wire.TxOut, *wire.TxOut) {
	deriveTxOut := func(balance chainutil.Amount,
	...
	localTxOut := deriveTxOut(
		c.LocalBalance.ToLokis(),
		c.LocalDeliveryScript,
	)
	remoteTxOut := deriveTxOut(
		c.RemoteBalance.ToLokis(),
...
func (c *CloseChannelTerms) RemoteAmtIsDust() bool {
	return c.RemoteBalance.ToLokis() < lnwallet.DustLimitForSize(
...
func (c *CloseChannelTerms) LocalAmtIsDust() bool {
	return c.LocalBalance.ToLokis() < lnwallet.DustLimitForSize(
...
func (c *CloseChannelTerms) LocalCanPayFees(absoluteFee chainutil.Amount) bool {
	return c.LocalBalance.ToLokis() >= absoluteFee
}

// RemoteCanPayFees returns true if the remote party can pay the absolute fee
// from their remote settled balance.
func (c *CloseChannelTerms) RemoteCanPayFees(absoluteFee chainutil.Amount) bool {
	return c.RemoteBalance.ToLokis() >= absoluteFee
}
