...
func TestRbfChannelFlushingTransitions(t *testing.T) {
	...
	localBalance := lnwire.NewMLokFromLokis(10_000)
	remoteBalance := lnwire.NewMLokFromLokis(50_000)
	...
	for _, isFreshFlush := range []bool{true, false} {
		...
		flushEvent.ShutdownBalances.LocalBalance = lnwire.NewMLokFromLokis( //nolint:ll
			absoluteFee * 3,
		)
		...
		t.Run(testName, func(t *testing.T) {
			...
			balanceAfterClose := localBalance.ToLokis() - absoluteFee //nolint:ll
...
func TestRbfCloseClosingNegotiationLocal(t *testing.T) {
	...
	localBalance := lnwire.NewMLokFromLokis(40_000)
	remoteBalance := lnwire.NewMLokFromLokis(50_000)
...
func TestRbfCloseClosingNegotiationRemote(t *testing.T) {
	...
	t.Run("recv_offer_rbf_loop_iterations", func(t *testing.T) {
		...
		// but aren't yet dust.
		closingTerms := *closeTerms
		closingTerms.ShutdownBalances.LocalBalance = lnwire.NewMLokFromLokis( //nolint:ll
			9000,
		)
