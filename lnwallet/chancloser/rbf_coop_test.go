...
func randPubKey(t *testing.T) *crypto.PublicKey {
...
func (r *rbfCloserTestHarness) expectFeeEstimate(absoluteFee chainutil.Amount,
	numTimes int) {
...
func (r *rbfCloserTestHarness) expectNewCloseSig(
	localScript, remoteScript []byte, fee chainutil.Amount,
	closeBalance chainutil.Amount) {
...
func (r *rbfCloserTestHarness) expectRemoteCloseFinalized(
	...
	remoteScript []byte, fee chainutil.Amount,
	balanceAfterClose chainutil.Amount, isLocal bool) {
...
func (r *rbfCloserTestHarness) expectHalfSignerIteration(
	initEvent ProtocolEvent, balanceAfterClose, absoluteFee chainutil.Amount,
...
func (r *rbfCloserTestHarness) assertSingleRemoteRbfIteration(
	...
	absoluteFee chainutil.Amount, sequence uint32, iteration bool,
...
func TestRbfChannelFlushingTransitions(t *testing.T) {
	...
	localBalance := lnwire.NewMSatFromLokis(10_000)
	remoteBalance := lnwire.NewMSatFromLokis(50_000)

	absoluteFee := chainutil.Amount(10_100)
	...
	t.Run("local_can_pay_for_fee", func(t *testing.T) {
		...
		flushEvent.ShutdownBalances.LocalBalance = lnwire.NewMSatFromLokis( //nolint:ll
			absoluteFee * 3,
		)
		...
		balanceAfterClose := localBalance.ToLokis() - absoluteFee
...
func TestRbfCloseClosingNegotiationLocal(t *testing.T) {
	...
	localBalance := lnwire.NewMSatFromLokis(40_000)
	remoteBalance := lnwire.NewMSatFromLokis(50_000)
	...
	t.Run("send_offer_iteration_local_dust", func(t *testing.T) {
		...
		dustBalance := chainutil.Amount(100)
		closeHarness.expectHalfSignerIteration(
	...
	t.Run("send_offer_cannot_pay_for_fees", func(t *testing.T) {
		...
		closeHarness.expectFeeEstimate(chainutil.LokiPerFlokicoin, 1)
...
func TestRbfCloseClosingNegotiationRemote(t *testing.T) {
	...
	balanceAfterClose := remoteBalance.ToLokis() - absoluteFee
	sequence := uint32(mempool.MaxRBFSequence)
	...
	t.Run("recv_offer_rbf_loop_iterations", func(t *testing.T) {
		...
		// but aren't yet dust.
		closingTerms := *closeTerms
		closingTerms.ShutdownBalances.LocalBalance = lnwire.NewMSatFromLokis( //nolint:ll
			9000,
		)
		...
		// fee by 1k sats.
		feeOffer.SigMsg.FeeLokis += 1000
		absoluteFee = feeOffer.SigMsg.FeeLokis
		closeHarness.assertSingleRemoteRbfIteration(
			feeOffer, balanceAfterClose, absoluteFee, sequence,
