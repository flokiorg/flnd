...
func (m *mockChannel) CreateCloseProposal(fee chainutil.Amount,
...
func (m *mockChannel) CompleteCooperativeClose(localSig,
	...
	_ ...lnwallet.ChanCloseOpt) (*wire.MsgTx, chainutil.Amount, error) {

	return &wire.MsgTx{}, 0, nil
}

func (m *mockChannel) LocalBalanceDust() (bool, chainutil.Amount) {
	return false, 0
}

func (m *mockChannel) RemoteBalanceDust() (bool, chainutil.Amount) {
	return false, 0
}

func (m *mockChannel) CommitBalances() (chainutil.Amount, chainutil.Amount) {
	return 0, 0
}

func (m *mockChannel) CommitFee() chainutil.Amount {
	return 0
}
...
func newMockTaprootChan(t *testing.T, initiator bool) *mockChannel {
	...
	localKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)

	remoteKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
...
func TestMaxFeeBailOut(t *testing.T) {
	...
	const (
		absoluteFee = chainutil.Amount(1000)
		idealFee    = chainfee.SatPerKWeight(253)
	)
...
func TestTaprootFastClose(t *testing.T) {
	...
	require.Equal(
		t, aliceClosingSigned.FeeLokis, bobClosingSigned.FeeLokis,
	)

	// If we modify Bob's offer, and try to have Alice process it, then she
	// should reject it.
	ogOffer := bobClosingSigned.FeeLokis
	bobClosingSigned.FeeLokis /= 2
	...
	bobClosingSigned.FeeLokis = ogOffer
