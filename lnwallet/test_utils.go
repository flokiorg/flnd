...
var (
	...
	aliceDustLimit = chainutil.Amount(200)
	bobDustLimit   = chainutil.Amount(1300)
...
// has just completed.  The channel itself is funded with 10 FLC, with 5 FLC
...
func CreateTestChannels(t *testing.T, chanType channeldb.ChannelType,
	...
	*LightningChannel, error) {
	...
	channelCapacity, err := chainutil.NewAmount(testChannelCapacity)
	if err != nil {
		return nil, nil, err
	}
	...
	var (
		aliceKeys []*crypto.PrivateKey
		bobKeys   []*crypto.PrivateKey
	)
	for i := 0; i < 5; i++ {
		...
		aliceKey, _ := crypto.PrivKeyFromBytes(key)
		...
		bobKey, _ := crypto.PrivKeyFromBytes(key)
		bobKeys = append(bobKeys, bobKey)
	}

	aliceCfg := channeldb.ChannelConfig{
		ChannelStateBounds: channeldb.ChannelStateBounds{
			MaxPendingAmount: lnwire.NewMSatFromLokis(channelCapacity),
	...
	var anchorAmt chainutil.Amount
	if chanType.HasAnchors() {
		anchorAmt += 2 * AnchorSize
	}

	aliceBalance := lnwire.NewMSatFromLokis(
		channelBal - commitFee - anchorAmt,
	)
	bobBalance := lnwire.NewMSatFromLokis(channelBal)
...
// pubkeyFromHex parses a Flokicoin public key from a hex encoded string.
func pubkeyFromHex(keyHex string) (*crypto.PublicKey, error) {
	bytes, err := hex.DecodeString(keyHex)
	if err != nil {
		return nil, err
	}
	return crypto.ParsePubKey(bytes)
}

// privkeyFromHex parses a Flokicoin private key from a hex encoded string.
func privkeyFromHex(keyHex string) (*crypto.PrivateKey, error) {
	bytes, err := hex.DecodeString(keyHex)
	if err != nil {
		return nil, err
	}
	key, _ := crypto.PrivKeyFromBytes(bytes)
	return key, nil

}

// blockFromHex parses a full Flokicoin block from a hex encoded string.
func blockFromHex(blockHex string) (*chainutil.Block, error) {
	bytes, err := hex.DecodeString(blockHex)
	if err != nil {
		return nil, err
	}
	return chainutil.NewBlockFromBytes(bytes)
}

// txFromHex parses a full Flokicoin transaction from a hex encoded string.
func txFromHex(txHex string) (*chainutil.Tx, error) {
	bytes, err := hex.DecodeString(txHex)
	if err != nil {
		return nil, err
	}
	return chainutil.NewTxFromBytes(bytes)
}
...
func calcStaticFee(chanType channeldb.ChannelType, numHTLCs int) chainutil.Amount {
	const (
		htlcWeight = 172
		feePerKw   = chainutil.Amount(24/4) * 1000
	)
	htlcsWeight := htlcWeight * int64(numHTLCs)
	totalWeight := CommitWeight(chanType) + lntypes.WeightUnit(htlcsWeight)

	return feePerKw * (chainutil.Amount(totalWeight)) / 1000
}
