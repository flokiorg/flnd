...
type TransactionDetail struct {
	...
	// Value is the net value of this transaction (in loki) from the
	...
	// TotalFees is the total fee in loki paid by this transaction.
	TotalFees int64
...
// such as: uspv, btcwallet, Flokicoin Core, Electrum, etc. This interface then
...
type WalletController interface {
	...
	ConfirmedBalance(confs int32, accountFilter string) (chainutil.Amount,
		error)
	...
	NewAddress(addrType AddressType, change bool,
		account string) (chainutil.Address, error)
	...
	IsOurAddress(a chainutil.Address) bool

	// AddressInfo returns the information about an address, if it's known
	// to this wallet.
	AddressInfo(a chainutil.Address) (waddrmgr.ManagedAddress, error)
	...
	// capped at a maximum.
	RequiredReserve(uint32) chainutil.Amount
	...
	ImportPublicKey(pubKey *crypto.PublicKey,
	...
	// SendOutputs funds, signs, and broadcasts a Flokicoin transaction paying
	...
	// then finally broadcasts the passed transaction to the Flokicoin network.
...
func InternalKeyForAddr(wallet WalletController, netParams *chaincfg.Params,
	...
	// internal key.
	_, isTaproot := addr.(*chainutil.AddressTaproot)
	if !isTaproot {
		return none, nil
	}
