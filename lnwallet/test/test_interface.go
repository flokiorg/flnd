...
import (
	...
	"github.com/flokiorg/flnd/chainntnfs/btcdnotify"
	...
	"github.com/flokiorg/flnd/lntest/unittest"
...
var (
	...
	_, alicePub = crypto.PrivKeyFromBytes(testHdSeed[:])
	_, bobPub   = crypto.PrivKeyFromBytes(bobsPrivKey)
...
func loadTestCredits(miner *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	satoshiPerOutput, err := chainutil.NewAmount(btcPerOutput)
	if err != nil {
	...
	expectedBalance += chainutil.Amount(int64(satoshiPerOutput) * int64(numOutputs))
	addrs := make([]chainutil.Address, 0, numOutputs)
...
func testDualFundingReservationWorkflow(miner *rpctest.Harness,
	...
	fundingAmount, err := chainutil.NewAmount(5)
	require.NoError(t, err, "unable to create amt")

	// In this scenario, we'll test a dual funder reservation, with each
	// side putting in 10 FLC.

	// Alice initiates a channel funded with 5 FLC for each side, so 10 FLC
	// total. She also generates 2 FLC in change.
	...
	bounds := &channeldb.ChannelStateBounds{
		...
		MaxPendingAmount: lnwire.NewMSatFromLokis(fundingAmount),
		MinHTLC:          1,
	...
	// from our HD chain, a change output with 3 FLC, and 2 outputs
	// selected of 4 FLC each. Additionally, the rest of the items needed
...
func testFundingTransactionLockedOutputs(miner *rpctest.Harness,
	...
	// Create a single channel asking for 16 FLC total.
	fundingAmount, err := chainutil.NewAmount(8)
	require.NoError(t, err, "unable to create amt")
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	require.NoError(t, err, "unable to query fee estimator")
	...
	// requesting 900 FLC. We only have around 64BTC worth of outpoints
	// that aren't locked, so this should fail.
	amt, err := chainutil.NewAmount(900)
	require.NoError(t, err, "unable to create amt")
...
func testFundingCancellationNotEnoughFunds(miner *rpctest.Harness,
	...
	// Create a reservation for 44 FLC.
	fundingAmount, err := chainutil.NewAmount(44)
	require.NoError(t, err, "unable to create amt")
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
		NodeID:           bobPub,
	...
	// Attempt to create another channel with 44 FLC, this should fail.
...
func testReservationInitiatorBalanceBelowDustCancel(miner *rpctest.Harness,
	...
	const numBTC = 4
	fundingAmount, err := chainutil.NewAmount(numBTC)
	require.NoError(t, err, "unable to create amt")

	feePerKw := chainfee.SatPerKWeight(
		numBTC * numBTC * chainutil.LokiPerFlokicoin,
	)
	req := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
		NodeID:           bobPub,
...
func testSingleFunderReservationWorkflow(miner *rpctest.Harness,
	...
	// First, Alice will Initialize a reservation for a channel with 4 FLC
	// funded solely by us. We'll also initially push 1 FLC of the channel
	// towards Bob's side.
	fundingAmt, err := chainutil.NewAmount(4)
	require.NoError(t, err, "unable to create amt")
	pushAmt := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	feePerKw, err := alice.Cfg.FeeEstimator.EstimateFeePerKW(1)
	require.NoError(t, err, "unable to query fee estimator")
	aliceReq := &lnwallet.InitFundingReserveMsg{
		ChainHash:        chainHash,
	...
	bounds := &channeldb.ChannelStateBounds{
		...
		MaxPendingAmount: lnwire.NewMSatFromLokis(fundingAmt),
		MinHTLC:          1,
		MaxAcceptedHtlcs: input.MaxHTLCNumber / 2,
	}
	commitParams := &channeldb.CommitmentParams{
...
func testListTransactionDetails(miner *rpctest.Harness,
	...
	const numTxns = 5
	const outputAmt = chainutil.LokiPerFlokicoin
		...
		if txOuts, ok := blockTxOuts[*txDetail.BlockHash][txDetail.Hash]; !ok {
			...
			for i, txOut := range txOuts {
				...
				destinationOutputs = append(destinationOutputs, lnwallet.OutputDetail{
					...
					OutputIndex:  i,
					Value:        chainutil.Amount(txOut.Value),
...
func scriptFromKey(pubkey *crypto.PublicKey) ([]byte, error) {
	pubkeyHash := chainutil.Hash160(pubkey.SerializeCompressed())
	keyAddr, err := chainutil.NewAddressWitnessPubKeyHash(
...
func txFromOutput(tx *wire.MsgTx, signer input.Signer, fromPubKey,
	payToPubKey *crypto.PublicKey, txFee chainutil.Amount,
...
func newTx(t *testing.T, r *rpctest.Harness, pubKey *crypto.PublicKey,
	...
	newOutput := &wire.TxOut{
		Value:    chainutil.LokiPerFlokicoin,
		PkScript: keyScript,
	}
	tx, err := alice.SendOutputs(
	...
	txFee := chainutil.Amount(0.001 * chainutil.LokiPerFlokicoin)
...
func testGetTransactionDetails(r *rpctest.Harness,
	...
	amountSats := chainutil.Amount(chainutil.LokiPerFlokicoin - txFee)
	output := &wire.TxOut{
...
func testPublishTransaction(r *rpctest.Harness,
	...
	var (
		txFee = chainutil.Amount(
			0.005 * chainutil.LokiPerFlokicoin,
		)
...
func testSignOutputUsingTweaks(r *rpctest.Harness,
	...
	commitSecret, commitPoint := crypto.PrivKeyFromBytes(commitPreimage)
	...
	baseKey := pubKey
	for i := 0; i < 2; i++ {
		var tweakedKey *crypto.PublicKey
		if i == 0 {
		...
		pubkeyHash := chainutil.Hash160(tweakedKey.SerializeCompressed())
		keyAddr, err := chainutil.NewAddressWitnessPubKeyHash(pubkeyHash,
		...
		vm, err := txscript.NewEngine(
			...
			nil, nil, int64(chainutil.LokiPerFlokicoin),
			txscript.NewCannedPrevOutputFetcher(
				keyScript, int64(chainutil.LokiPerFlokicoin),
			),
		)
		if err != nil {
...
func testReorgWalletBalance(r *rpctest.Harness, w *lnwallet.LightningWallet,
	...
	for i := 0; i < 5; i++ {
		...
		timeout := time.After(30 * time.Second)
		stillConnected := true
		var peers []chainjson.GetPeerInfoResult
		for stillConnected {
			// Allow for timeout
...
func testChangeOutputSpendConfirmation(r *rpctest.Harness,
	...
	// Assuming a balance of 80 FLC and a transaction fee of 2500 sat/kw,
	...
	// We'll use a transaction fee of 14380 loki, which will allow us to
	...
	require.NoError(t, err, "unable to retrieve alice's balance")
	if aliceBalance != 0 {
		t.Fatalf("expected alice's balance to be 0 FLC, found %v",
			aliceBalance)
	}

	// Now, we'll send an output back to Alice from Bob of 1 FLC.
	...
	output = &wire.TxOut{
		Value:    chainutil.LokiPerFlokicent,
		PkScript: bobPkScript,
	}
	tx = sendCoins(t, r, alice, bob, output, txFeeRate, true, 1)
	txHash = tx.TxHash()
...
func testSpendUnconfirmed(miner *rpctest.Harness,
	...
	output = &wire.TxOut{
		Value:    chainutil.LokiPerFlokicoin * 0.5,
		PkScript: alicePkScript,
	}
	...
	output = &wire.TxOut{
		Value:    int64(bobBalance) - (chainutil.LokiPerFlokicoin * 0.4),
		PkScript: bobPkScript,
	}
...
func waitForMempoolTx(r *rpctest.Harness, txid *chainhash.Hash) error {
	...
	for !found {
		...
		if err != nil {
			switch e := err.(type) {
			case *chainjson.RPCError:
				if e.Code == chainjson.ErrRPCNoTxInfo {
					continue
				}
			default:
			}
...
func testSingleFunderExternalFundingTx(miner *rpctest.Harness,
	...
	// We'll now set up for them to open a 4 FLC channel, with 1 FLC pushed
	// to Bob's side.
	chanAmt := 4 * chainutil.LokiPerFlokicoin
	...
	fundingIntent, err := aliceChanFunder.ProvisionChannel(
		&chanfunding.Request{
			LocalAmt: chainutil.Amount(chanAmt),
			MinConfs: 1,
			FeeRate:  253,
			ChangeAddr: func() (chainutil.Address, error) {
	...
	aliceExternalFunder := chanfunding.NewCannedAssembler(
		thawHeight, *chanPoint, chainutil.Amount(chanAmt), &aliceFundingKey,
		bobFundingKey.PubKey, true, false,
	)
	bobShimIntent, err := chanfunding.NewCannedAssembler(
		thawHeight, *chanPoint, chainutil.Amount(chanAmt), &bobFundingKey,
...
func TestLightningWallet(t *testing.T, targetBackEnd string) {
	...
	// up this node with a chain length of 125, so we have plenty of FLC
	// to play around with.
...
func runTests(t *testing.T, walletDriver *lnwallet.WalletDriver,
	...
	case "btcwallet":
		...
		case "bitcoind":
			...
			chainConn := unittest.NewFlokicoindBackend(
