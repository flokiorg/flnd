...
func newTestContext(t *testing.T) *testContext {
	...
	priv := func(v string) *crypto.PrivateKey {
...
type testCase struct {
	...
	DustLimitSatoshis chainutil.Amount
...
func testVectors(t *testing.T, chanType channeldb.ChannelType, test testCase) {
	...
	// capacity.
	require.EqualValues(t, lnwire.NewMLokFromLokis(tc.fundingAmount),
...
func testSpendValidation(t *testing.T, tweakless bool) {
	...
	const channelBalance = chainutil.Amount(1 * 10e8)
	const csvTimeout = 5
	...
	commitSecret, commitPoint := crypto.PrivKeyFromBytes(
		revocationPreimage,
	)
	...
	localSigner := input.NewMockSigner([]*crypto.PrivateKey{bobKeyPriv}, nil)
...
func createTestChannelsForVectors(tc *testContext, chanType channeldb.ChannelType,
	feeRate chainutil.Amount, remoteBalance, localBalance chainutil.Amount) (
	*LightningChannel, *LightningChannel) {
	...
	// set.
	var (
		remoteDummy1, remoteDummy2 *crypto.PrivateKey
		localDummy2, localDummy1   *crypto.PrivateKey
	)
	generateKeys := []**crypto.PrivateKey{
	...
	remoteCommit := channeldb.ChannelCommitment{
		...
		LocalBalance:  lnwire.NewMLokFromLokis(remoteBalance),
		RemoteBalance: lnwire.NewMLokFromLokis(localBalance - commitFee - anchorAmt),
		CommitFee:     commitFee,
	...
	localCommit := channeldb.ChannelCommitment{
		CommitHeight:  commitHeight,
		LocalBalance:  lnwire.NewMLokFromLokis(localBalance - commitFee - anchorAmt),
		RemoteBalance: lnwire.NewMLokFromLokis(remoteBalance),
		CommitFee:     commitFee,
		FeePerKw:      chainutil.Amount(feePerKw),
