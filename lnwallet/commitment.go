...
const AnchorSize = chainutil.Amount(330)
...
type CommitmentKeyRing struct {
	...
	RemoteHtlcKey *crypto.PublicKey
	...
	RevocationKey *crypto.PublicKey
}
...
// peer's.
func DeriveCommitmentKeys(commitPoint *crypto.PublicKey,
	...
	var (
		toLocalBasePoint    *crypto.PublicKey
		toRemoteBasePoint   *crypto.PublicKey
		revocationBasePoint *crypto.PublicKey
	)
	if whoseCommit.IsLocal() {
...
func CommitScriptToSelf(chanType channeldb.ChannelType, initiator bool,
	selfKey, revokeKey *crypto.PublicKey, csvDelay, leaseExpiry uint32,
...
func CommitScriptToRemote(chanType channeldb.ChannelType, initiator bool,
	remoteKey *crypto.PublicKey, leaseExpiry uint32,
...
func SecondLevelHtlcScript(chanType channeldb.ChannelType, initiator bool,
	revocationKey, delayKey *crypto.PublicKey, csvDelay, leaseExpiry uint32,
	auxLeaf input.AuxTapLeaf) (input.ScriptDescriptor, error) {
...
// HtlcTimeoutFee returns the fee in loki required for an HTLC timeout
// transaction based on the current fee rate.
func HtlcTimeoutFee(chanType channeldb.ChannelType,
	feePerKw chainfee.SatPerKWeight) chainutil.Amount {
...
// HtlcSuccessFee returns the fee in loki required for an HTLC success
// transaction based on the current fee rate.
...
func CommitScriptAnchors(chanType channeldb.ChannelType,
	...
	var (
		anchorScript func(
			key *crypto.PublicKey) (input.ScriptDescriptor, error)
...
func (cb *CommitmentBuilder) createUnsignedCommitmentTx(ourBalance,
	theirBalance lnwire.MilliLoki, whoseCommit lntypes.ChannelParty,
	...
	switch {
	case cb.chanState.IsInitiator && commitFee > ourBalance.ToLokis():
		ourBalance = 0

	case cb.chanState.IsInitiator:
		ourBalance -= commitFeeMSat

	case !cb.chanState.IsInitiator && commitFee > theirBalance.ToLokis():
		theirBalance = 0
	...
	if whoseCommit.IsLocal() {
		commitTx, err = CreateCommitTx(
			...
			ourBalance.ToLokis(), theirBalance.ToLokis(),
		...
		commitTx, err = CreateCommitTx(
			...
			theirBalance.ToLokis(), ourBalance.ToLokis(),
			numHTLCs, !cb.chanState.IsInitiator, leaseExpiry,
			auxResult.AuxLeaves,
		)
	}
...
func CreateCommitTx(chanType channeldb.ChannelType,
	...
	localChanCfg, remoteChanCfg *channeldb.ChannelConfig,
	amountToLocal, amountToRemote chainutil.Amount,
...
func CoopCloseBalance(chanType channeldb.ChannelType, isInitiator bool,
	coopCloseFee, ourBalance, theirBalance, commitFee chainutil.Amount,
...
func addHTLC(commitTx *wire.MsgTx, whoseCommit lntypes.ChannelParty,
	...
	amountPending := int64(paymentDesc.Amount.ToLokis())
...
func findOutputIndexesFromRemote(revocationPreimage *chainhash.Hash,
	...
	_, commitmentPoint := crypto.PrivKeyFromBytes(revocationPreimage[:])
