...
var (
	...
	// be charged up to.
	RoutingFee100PercentUpTo = lnwire.NewMLokFromLokis(1_000)
)
...
const (
	...
	// size.
	DefaultRoutingFeePercentage lnwire.MilliLoki = 5
)
...
func DefaultRoutingFeeLimitForAmount(a lnwire.MilliLoki) lnwire.MilliLoki {
...
func DustLimitForSize(scriptSize int) chainutil.Amount {
	var (
	...
	dustlimit = chainutil.Amount(mempool.GetDustThreshold(txout))

	return dustlimit
}

// DustLimitUnknownWitness returns the dust limit for an UnknownWitnessSize.
func DustLimitUnknownWitness() chainutil.Amount {
