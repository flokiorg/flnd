...
type commitment struct {
	...
	ourBalance   lnwire.MilliLoki
	theirBalance lnwire.MilliLoki
...
func locateOutputIndex(p *paymentDescriptor, tx *wire.MsgTx,
	...
	for i, txOut := range tx.TxOut {
		...
		if bytes.Equal(txOut.PkScript, pkScript) &&
			txOut.Value == int64(p.Amount.ToLokis()) &&
			cltv == p.Timeout {
...
func (c *commitment) populateHtlcIndexes(chanType channeldb.ChannelType,
	...
	populateIndex := func(htlc *paymentDescriptor, incoming bool) error {
		isDust := HtlcIsDust(
			...
			htlc.Amount.ToLokis(), c.dustLimit,
		)
...
func (c *commitment) toDiskCommit(
	...
	commit := &channeldb.ChannelCommitment{
		...
		CommitFee:       c.fee,
		FeePerKw:        chainutil.Amount(c.feePerKw),
		CommitTx:        c.txn,
		CommitSig:       c.sig,
...
func (lc *LightningChannel) diskHtlcToPayDesc(feeRate chainfee.SatPerKWeight,
	...
	isDustLocal := HtlcIsDust(
		...
		htlc.Amt.ToLokis(), lc.channelState.LocalChanCfg.DustLimit,
	)
	...
	isDustRemote := HtlcIsDust(
		...
		htlc.Amt.ToLokis(), lc.channelState.RemoteChanCfg.DustLimit,
	)
...
func (lc *LightningChannel) diskCommitToMemCommit(
	...
	remoteCommitPoint *crypto.PublicKey) (*commitment, error) {
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	remoteDustLimit chainutil.Amount,
	...
	case *lnwire.UpdateAddHTLC:
		...
		isDustRemote := HtlcIsDust(
			...
			feeRate, wireMsg.Amount.ToLokis(), remoteDustLimit,
		)
		if !isDustRemote {
			auxLeaf := fn.FlatMapOption(
				func(l CommitAuxLeaves) input.AuxTapLeaf {
	...
	case *lnwire.UpdateFee:
		pd = &paymentDescriptor{
			ChanID:   wireMsg.ChanID,
			LogIndex: logUpdate.LogIndex,
			Amount: lnwire.NewMSatFromLokis(
				chainutil.Amount(wireMsg.FeePerKw),
			),
			EntryType: FeeUpdate,
			addCommitHeights: lntypes.Dual[uint64]{
				Remote: commitHeight,
...
func NewBreachRetribution(chanState *channeldb.OpenChannel, stateNum uint64,
	...
	commitmentSecret, commitmentPoint := crypto.PrivKeyFromBytes(
		revocationPreimage[:],
	)
...
func createHtlcRetribution(chanState *channeldb.OpenChannel,
	...
	commitmentSecret *crypto.PrivateKey, leaseExpiry uint32,
...
func createBreachRetribution(revokedLog *channeldb.RevocationLog,
	...
	keyRing *CommitmentKeyRing, commitmentSecret *crypto.PrivateKey,
	leaseExpiry uint32,
...
func createBreachRetributionLegacy(revokedLog *channeldb.ChannelCommitment,
	...
	commitmentSecret *crypto.PrivateKey,
	...
	// Compute the balances in loki.
	ourAmt := int64(revokedLog.LocalBalance.ToLokis())
	theirAmt := int64(revokedLog.RemoteBalance.ToLokis())
...
func HtlcIsDust(chanType channeldb.ChannelType,
	...
	feePerKw chainfee.SatPerKWeight, htlcAmt, dustLimit chainutil.Amount,
) bool {
...
func (lc *LightningChannel) fetchCommitmentView(
	...
	var totalOut chainutil.Amount
	for _, txOut := range commitTx.txn.TxOut {
		totalOut += chainutil.Amount(txOut.Value)
	}
	...
	uTx := chainutil.NewTx(commitTx.txn)
...
func balanceAboveReserve(party lntypes.ChannelParty, delta int64,
	...
	localReserve := lnwire.NewMSatFromLokis(c.LocalChanCfg.ChanReserve)
	remoteReserve := lnwire.NewMSatFromLokis(c.RemoteChanCfg.ChanReserve)
	...
	case party.IsLocal():
		...
		if delta >= 0 {
			totalLocal += lnwire.MilliLoki(delta)
		} else {
			totalLocal -= lnwire.MilliLoki(-1 * delta)
		}
	...
	case party.IsRemote():
		...
		if delta >= 0 {
			totalRemote += lnwire.MilliLoki(delta)
		} else {
			totalRemote -= lnwire.MilliLoki(-1 * delta)
		}
...
func genRemoteHtlcSigJobs(keyRing *CommitmentKeyRing,
	...
	for _, htlc := range remoteCommitView.incomingHTLCs {
		...
		htlcFee := HtlcTimeoutFee(chanType, feePerKw)
		outputAmt := htlc.Amount.ToLokis() - htlcFee
		...
		prevFetcher := txscript.NewCannedPrevOutputFetcher(
			txOut.PkScript, int64(htlc.Amount.ToLokis()),
		)
...
func CalcFeeBuffer(feePerKw chainfee.SatPerKWeight,
	commitWeight lntypes.WeightUnit) lnwire.MilliLoki {

	// Account for a 100% in fee rate increase.
	bufferFeePerKw := 2 * feePerKw

	feeBuffer := lnwire.NewMSatFromLokis(
...
func (lc *LightningChannel) applyCommitFee(
	balance lnwire.MilliLoki, commitWeight lntypes.WeightUnit,
	feePerKw chainfee.SatPerKWeight,
	buffer BufferType) (lnwire.MilliLoki, lnwire.MilliLoki,
	lnwire.MilliLoki, error) {

	commitFee := feePerKw.FeeForWeight(commitWeight)
	commitFeeMsat := lnwire.NewMSatFromLokis(commitFee)

	var bufferAmt lnwire.MilliLoki
	switch buffer {
	...
	// peer can add.
	case AdditionalHtlc:
		additionalHtlcFee := lnwire.NewMSatFromLokis(
...
func (lc *LightningChannel) validateCommitmentSanity(theirLogCounter,
	...
	ourReserve := lnwire.NewMSatFromLokis(
		lc.channelState.LocalChanCfg.ChanReserve,
	)
	theirReserve := lnwire.NewMSatFromLokis(
	...
	validateUpdates := func(updates []*paymentDescriptor,
		...
		var amtInFlight lnwire.MilliLoki
...
func (lc *LightningChannel) ProcessChanSyncMsg(ctx context.Context,
	...
	// is valid.
	var commitPoint *crypto.PublicKey
	switch {
...
func (lc *LightningChannel) computeView(view *HtlcView,
	...
	dryRunFee fn.Option[chainfee.SatPerKWeight]) (lnwire.MilliLoki,
	lnwire.MilliLoki, lntypes.WeightUnit, *HtlcView, error) {
	...
	if lc.channelState.IsInitiator {
		ourBalance += lnwire.NewMSatFromLokis(
			commitChain.tip().fee)
	} else if !lc.channelState.IsInitiator {
		theirBalance += lnwire.NewMSatFromLokis(
			commitChain.tip().fee)
	}
	nextHeight := commitChain.tip().height + 1
	...
	// state.
	if deltas.Local >= 0 {
		ourBalance += lnwire.MilliLoki(deltas.Local)
	} else {
		ourBalance -= lnwire.MilliLoki(-1 * deltas.Local)
	}
	if deltas.Remote >= 0 {
		theirBalance += lnwire.MilliLoki(deltas.Remote)
	} else {
		theirBalance -= lnwire.MilliLoki(-1 * deltas.Remote)
	}
	...
	for _, htlc := range filteredHTLCView.Updates.Local {
		if HtlcIsDust(
			...
			feePerKw, htlc.Amount.ToLokis(), dustLimit,
		) {
...
func (lc *LightningChannel) recordSettlement(
	redeemer lntypes.ChannelParty, amt lnwire.MilliLoki) {
	...
	for index := range localCommitmentView.txn.TxOut {
		...
		case localCommitmentView.incomingHTLCIndex[outputIndex] != nil:
			...
			sigHash = func() ([]byte, error) {
				...
				htlcAmt := int64(htlc.Amount.ToLokis())
...
// new commitment.
func (lc *LightningChannel) NextRevocationKey() (*crypto.PublicKey, error) {
	lc.RLock()
	defer lc.RUnlock()
...
// any new states.
func (lc *LightningChannel) InitNextRevocation(revKey *crypto.PublicKey) error {
	lc.Lock()
	defer lc.Unlock()
...
func (lc *LightningChannel) MayAddOutgoingHtlc(amt lnwire.MilliLoki) error {
	lc.Lock()
	defer lc.Unlock()

	var mockHtlcAmt lnwire.MilliLoki
	switch {
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, //nolint:funlen
	...
	remoteCommit channeldb.ChannelCommitment, commitPoint *crypto.PublicKey,
	leafStore fn.Option[AuxLeafStore],
	...
	closeSummary := channeldb.ChannelCloseSummary{
		...
		SettledBalance:          chainutil.Amount(localBalance),
...
func newOutgoingHtlcResolution(signer input.Signer,
	...
	if whoseCommit.IsRemote() {
		...
		prevFetcher := txscript.NewCannedPrevOutputFetcher(
			htlcPkScript, int64(htlc.Amt.ToLokis()),
		)
		signDesc := input.SignDescriptor{
			KeyDesc:       localChanCfg.HtlcBasePoint,
			SingleTweak:   keyRing.LocalHtlcKeyTweak,
	...
	htlcFee := HtlcTimeoutFee(chanType, feePerKw)
	secondLevelOutputAmt := htlc.Amt.ToLokis() - htlcFee
	...
	resolveRes := fn.MapOptionZ(
		auxResolver, func(a AuxContractResolver) fn.Result[tlv.Blob] {
			...
			resReq := ResolutionReq{
				...
				KeyRing:             keyRing,
				CsvDelay:            htlcCsvDelay,
				HtlcAmt:             chainutil.Amount(txOut.Value),
...
// returned.
func (lc *LightningChannel) CreateCloseProposal(proposedFee chainutil.Amount,
	...
	ourBalance, theirBalance, err := CoopCloseBalance(
		...
		proposedFee,
		lc.channelState.LocalCommitment.LocalBalance.ToLokis(),
		lc.channelState.LocalCommitment.RemoteBalance.ToLokis(),
...
func (lc *LightningChannel) CompleteCooperativeClose(
	...
	proposedFee chainutil.Amount,
	closeOpts ...ChanCloseOpt) (*wire.MsgTx, chainutil.Amount, error) {
...
func (lc *LightningChannel) AvailableBalance() lnwire.MilliLoki {
	lc.RLock()
	defer lc.RUnlock()
...
func (lc *LightningChannel) availableBalance(
	buffer BufferType) (lnwire.MilliLoki, lntypes.WeightUnit) {
...
func (lc *LightningChannel) availableCommitmentBalance(view *HtlcView,
	...
	lnwire.MilliLoki, lntypes.WeightUnit) {
	...
	dustlimit := lnwire.NewMSatFromLokis(
...
func (lc *LightningChannel) validateFeeRate(feePerKw chainfee.SatPerKWeight) error {
	...
	oldFee := lnwire.NewMSatFromLokis(
		lc.commitChains.Local.tip().feePerKw.FeeForWeight(txWeight),
	)

	// Our base balance is the total amount of loki we can commit
	...
	// this new fee update.
	newFee := lnwire.NewMSatFromLokis(
...
func (lc *LightningChannel) UpdateFee(feePerKw chainfee.SatPerKWeight) error {
	...
	pd := &paymentDescriptor{
		ChanID:    lc.ChannelID(),
		LogIndex:  lc.updateLogs.Local.logIndex,
		Amount:    lnwire.NewMSatFromLokis(chainutil.Amount(feePerKw)),
		EntryType: FeeUpdate,
	}
...
func (lc *LightningChannel) CommitFeeTotalAt(
	feePerKw chainfee.SatPerKWeight) (chainutil.Amount, chainutil.Amount,
	error) {
	...
	var localCommitFee, remoteCommitFee chainutil.Amount
...
func CreateCooperativeCloseTx(fundingTxIn wire.TxIn,
	localDust, remoteDust, ourBalance, theirBalance chainutil.Amount,
	...
	for _, extraTxOut := range opts.extraCloseOutputs {
		...
		case extraTxOut.IsLocal:
			...
			localOutputIdx.WhenSome(func(idx int) {
				...
				// extra output.
				amtAfterOutput := chainutil.Amount(
...
// outputs.
func (lc *LightningChannel) LocalBalanceDust() (bool, chainutil.Amount) {
	lc.RLock()
	defer lc.RUnlock()

	chanState := lc.channelState
	localBalance := chanState.LocalCommitment.LocalBalance.ToLokis()
...
// outputs.
func (lc *LightningChannel) RemoteBalanceDust() (bool, chainutil.Amount) {
	lc.RLock()
	defer lc.RUnlock()

	chanState := lc.channelState
	remoteBalance := chanState.RemoteCommitment.RemoteBalance.ToLokis()
...
// commitment state.
func (lc *LightningChannel) CommitBalances() (chainutil.Amount, chainutil.Amount) {
	lc.RLock()
	defer lc.RUnlock()
	...
	localBalance := localCommit.LocalBalance.ToLokis()
	remoteBalance := localCommit.RemoteBalance.ToLokis()
...
func (lc *LightningChannel) CommitFee() chainutil.Amount {
	lc.RLock()
	defer lc.RUnlock()
...
// (fee-per-kw).
func (lc *LightningChannel) CalcFee(feeRate chainfee.SatPerKWeight) chainutil.Amount {
...
func (lc *LightningChannel) MaxFeeRate(
	...
	baseBalance := availableBalance.ToLokis() + currentFee
...
func (lc *LightningChannel) RemoteNextRevocation() *crypto.PublicKey {
	lc.RLock()
	defer lc.RUnlock()
...
// the channel.
func (lc *LightningChannel) MarkDataLoss(commitPoint *crypto.PublicKey) error {
	lc.Lock()
	defer lc.Unlock()
...
func (lc *LightningChannel) LocalChanReserve() chainutil.Amount {
...
func (lc *LightningChannel) FwdMinHtlc() lnwire.MilliLoki {
