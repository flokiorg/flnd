...
func testAddSettleWorkflow(t *testing.T, tweakless bool,
	...
	htlcAmt := lnwire.NewMLokFromLokis(chainutil.LokiPerFlokicoin)
	...
	mSatTransferred := lnwire.NewMLokFromLokis(chainutil.LokiPerFlokicoin)
...
func testForceClose(t *testing.T, testCase *forceCloseTestCase) {
	...
	htlcAmount := lnwire.NewMLokFromLokis(20000)
...
func TestForceCloseDustOutput(t *testing.T) {
	...
	htlcAmount := lnwire.NewMLokFromLokis(500)
...
func TestDustHTLCFees(t *testing.T) {
	...
	htlcAmount := lnwire.NewMLokFromLokis(100)
	...
	totalSatoshisAlice := (aliceChannel.channelState.LocalCommitment.LocalBalance +
		...
		lnwire.NewMLokFromLokis(aliceChannel.channelState.LocalCommitment.CommitFee))
	if totalSatoshisAlice+htlcAmount != lnwire.NewMLokFromLokis(aliceChannel.Capacity) {
	...
	totalSatoshisBob := (bobChannel.channelState.LocalCommitment.LocalBalance +
		...
		lnwire.NewMLokFromLokis(bobChannel.channelState.LocalCommitment.CommitFee))
	if totalSatoshisBob+htlcAmount != lnwire.NewMLokFromLokis(bobChannel.Capacity) {
...
func TestHTLCDustLimit(t *testing.T) {
	...
	htlcAmount := lnwire.NewMLokFromLokis(htlcSat)
	...
	createChanWithHTLC := func(htlcValues ...chainutil.Amount) (
		*LightningChannel, *LightningChannel) {
		...
		for i, htlcSat := range htlcValues {
			htlcMsat := lnwire.NewMLokFromLokis(htlcSat)
...
func TestStateUpdatePersistence(t *testing.T) {
	...
	htlcAmt := lnwire.NewMLokFromLokis(5000)
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	aliceBal := lnwire.NewMLokFromLokis(chainutil.LokiPerFlokicoin)
	bobBal := lnwire.NewMLokFromLokis(250)
	...
	lowBalance := lnwire.NewMLokFromLokis(aliceFee) + 1000
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	htlcAmt = lnwire.NewMLokFromLokis(2 * chainutil.LokiPerFlokicoin)
...
func testChanSyncOweCommitment(t *testing.T, chanType channeldb.ChannelType) {
	...
	// single state update.
	htlcAmt := lnwire.NewMLokFromLokis(20000)
	const numBobHtlcs = 3
	var bobPreimage [32]byte
	copy(bobPreimage[:], bytes.Repeat([]byte{0xbb}, 32))
	for i := 0; i < 3; i++ {
...
func TestAuxSignerShutdown(t *testing.T) {
	...
	htlcAmt := lnwire.NewMLokFromLokis(0.01 * chainutil.LokiPerFlokicoin)
...
func TestChannelRetransmissionFeeUpdate(t *testing.T) {
	...
	bobHtlc := &lnwire.UpdateAddHTLC{
		PaymentHash: rHash,
		Amount:      lnwire.NewMLokFromLokis(20000),
		Expiry:      uint32(10),
	}
	addAndReceiveHTLC(t, bobChannel, aliceChannel, bobHtlc, nil)
	if err := ForceStateTransition(bobChannel, aliceChannel); err != nil {
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMLokFromLokis(0.1 * chainutil.LokiPerFlokicoin)
	const numHTLCs = 30
...
func TestChanAvailableBandwidth(t *testing.T) {
	...
	aliceReserve := lnwire.NewMLokFromLokis(
	...
	assertBandwidthEstimateCorrect := func(aliceInitiate bool,
		...
		commitFee := lnwire.NewMLokFromLokis(aliceState.CommitFee)
	...
	// transaction.
	htlcAmt := lnwire.NewMLokFromLokis(30000)
	for i := 0; i < numHtlcs; i++ {
...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	aliceBalance := lnwire.NewMLokFromLokis(5 * chainutil.LokiPerFlokicoin)
	bobBalance := lnwire.NewMLokFromLokis(5 * chainutil.LokiPerFlokicoin)

	aliceReserve := lnwire.NewMLokFromLokis(
		aliceChannel.channelState.LocalChanCfg.ChanReserve,
	)
	bobReserve := lnwire.NewMLokFromLokis(
		bobChannel.channelState.LocalChanCfg.ChanReserve,
	)

	aliceDustlimit := lnwire.NewMLokFromLokis(
	...
	htlcFee := lnwire.NewMLokFromLokis(
		feeRate.FeeForWeight(input.HTLCWeight),
	)
	commitFee := lnwire.NewMLokFromLokis(
		aliceChannel.channelState.LocalCommitment.CommitFee,
	)
	htlcTimeoutFee := lnwire.NewMLokFromLokis(
		HtlcTimeoutFee(aliceChannel.channelState.ChanType, feeRate),
	)
	htlcSuccessFee := lnwire.NewMLokFromLokis(
...
func TestChanCommitWeightDustHtlcs(t *testing.T) {
	...
	bobDustlimit := lnwire.NewMLokFromLokis(
...
func TestDesyncHTLCs(t *testing.T) {
	...
	htlcAmt := lnwire.NewMLokFromLokis(4.1 * chainutil.LokiPerFlokicoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	htlcAmt = lnwire.NewMLokFromLokis(1 * chainutil.LokiPerFlokicoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	maxPending := lnwire.NewMLokFromLokis(chainutil.LokiPerFlokicoin * 3)
	...
	const numHTLCs = 2
	htlcAmt := lnwire.NewMLokFromLokis(1.5 * chainutil.LokiPerFlokicoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		addAndReceiveHTLC(t, aliceChannel, bobChannel, htlc, nil)
	}
...
func TestChanReserve(t *testing.T) {
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMLokFromLokis(0.5 * chainutil.LokiPerFlokicoin)
	...
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMLokFromLokis(3.5 * chainutil.LokiPerFlokicoin)
...
func TestChanReserveLocalInitiatorDustHtlc(t *testing.T) {
	...
	htlcDustAmt := lnwire.NewMLokFromLokis(htlcSat)
...
func TestMinHTLC(t *testing.T) {
	...
	minValue := lnwire.NewMLokFromLokis(0.1 * chainutil.LokiPerFlokicoin)
...
func TestNewBreachRetributionSkipsDustHtlcs(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		...
		h := &lnwire.UpdateAddHTLC{
			PaymentHash: rHash,
			Amount:      lnwire.NewMLokFromLokis(dustValue),
			Expiry:      uint32(10),
			OnionBlob:   fakeOnionBlob,
		}
...
func TestChannelRestoreCommitHeight(t *testing.T) {
	...
	htlcAmount := lnwire.NewMLokFromLokis(100000000)
	htlcAlice, _ := createHTLC(0, htlcAmount)
	addAndReceiveHTLC(t, aliceChannel, bobChannel, htlcAlice, nil)
...
func TestApplyCommitmentFee(t *testing.T) {
	var (
		...
		additionalHtlc = lnwire.NewMLokFromLokis(
...
func TestAsynchronousSendingContraint(t *testing.T) {
	...
	// all its local funds.
	// -----add------>|
	htlc1, _ := createHTLC(0, lnwire.NewMLokFromLokis(htlcAmount))
	...
	// one.
	htlcAmt2 := lnwire.NewMLokFromLokis(
...
func TestAsynchronousSendingWithFeeBuffer(t *testing.T) {
	...
	htlcAmt3 := lnwire.NewMLokFromLokis(
		aliceChannel.channelState.LocalChanCfg.DustLimit + htlcFee,
	)
	...
	require.Equal(t, aliceChannel.channelState.LocalCommitment.LocalBalance,
		lnwire.NewMLokFromLokis(aliceReserve))
}
...
func TestEnforceFeeBuffer(t *testing.T) {
	...
	commitFeeMsat := lnwire.NewMLokFromLokis(commitFee)
	...
	//	<----sig-------	|---------------
	//	---------------	|-----rev------>
	htlc1, _ := createHTLC(0, lnwire.NewMLokFromLokis(htlcAmt1))
	...
	//	---------------	|-----sig------>
	//	<----rev-------	|---------------
	htlcAmt2 := lnwire.NewMLokFromLokis(chainutil.LokiPerFlokicent)
	htlc2, _ := createHTLC(0, htlcAmt2)
