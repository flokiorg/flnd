...
func createHTLC(id int, amount lnwire.MilliLoki) (*lnwire.UpdateAddHTLC, [32]byte) {
...
func testAddSettleWorkflow(t *testing.T, tweakless bool,
	...
	// unittest. The channel will be funded evenly with Alice having 5 FLC,
	// and Bob having 5 FLC.
	...
	htlcAmt := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	...
	// At this point, Bob should have 6 FLC settled, with Alice still
	// having 4 FLC. Alice's channel should show 1 FLC sent and Bob's
	// channel should show 1 FLC received. They should also be at
	// commitment height two, with the revocation window extended by 1 (5).
	mSatTransferred := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	if aliceChannel.channelState.TotalMSatSent != mSatTransferred {
		t.Fatalf("alice loki sent incorrect %v vs %v expected",
			aliceChannel.channelState.TotalMSatSent,
			mSatTransferred)
	}
	if aliceChannel.channelState.TotalMSatReceived != 0 {
		t.Fatalf("alice loki received incorrect %v vs %v expected",
			aliceChannel.channelState.TotalMSatReceived, 0)
	}
	if bobChannel.channelState.TotalMSatReceived != mSatTransferred {
		t.Fatalf("bob loki received incorrect %v vs %v expected",
			bobChannel.channelState.TotalMSatReceived,
			mSatTransferred)
	}
	if bobChannel.channelState.TotalMSatSent != 0 {
		t.Fatalf("bob loki sent incorrect %v vs %v expected",
...
func TestChannelZeroAddLocalHeight(t *testing.T) {
	...
	htlc, _ := createHTLC(0, lnwire.MilliLoki(500000))
	...
	htlc2, _ := createHTLC(0, lnwire.MilliLoki(500000))
...
func TestCheckCommitTxSize(t *testing.T) {
	...
	checkSize := func(channel *LightningChannel, count int) {
		...
		actualCost := blockchain.GetTransactionWeight(chainutil.NewTx(commitTx))
	...
	// error window.
	for i := 0; i <= 10; i++ {
		htlc, _ := createHTLC(i, lnwire.MilliLoki(1e7))
	...
	// HTLCs properly.
	for i := 10; i >= 0; i-- {
		_, preimage := createHTLC(i, lnwire.MilliLoki(1e7))
...
func testCommitHTLCSigTieBreak(t *testing.T, restart bool) {
	...
	const (
		htlcAmt  = lnwire.MilliLoki(20000000)
		numHtlcs = 2
	)
...
func testCoopClose(t *testing.T, testCase coopCloseTestCase) {
	...
	type chanFees struct {
		alice chainutil.Amount
		bob   chainutil.Amount
	}
	...
	closeFees := fn.MapOption(func(payer lntypes.ChannelParty) chanFees {
		var alice, bob chainutil.Amount
	...
	expBalanceBob := bobChannel.channelState.LocalCommitment.
		LocalBalance.ToLokis() - closeFees.bob
...
func testForceClose(t *testing.T, testCase *forceCloseTestCase) {
	...
	htlcAmount := lnwire.NewMSatFromLokis(20000)
	...
	expectedAmount := (aliceChannel.Capacity / 2) -
		htlcAmount.ToLokis() - commitFee - testCase.anchorAmt
	...
	if bobCommitResolution.SelfOutputSignDesc.Output.Value !=
		int64(bobAmount.ToLokis()-htlcAmount.ToLokis()) {
...
func TestForceCloseDustOutput(t *testing.T) {
	...
	htlcAmount := lnwire.NewMSatFromLokis(500)
	...
	if commitResolution.SelfOutputSignDesc.Output.Value !=
		int64(aliceAmount.ToLokis()) {
		t.Fatalf("alice incorrect output value in SelfOutputSignDesc, "+
			"expected %v, got %v",
			aliceChannel.channelState.LocalCommitment.LocalBalance.ToLokis(),
...
func TestDustHTLCFees(t *testing.T) {
	...
	htlcAmount := lnwire.NewMSatFromLokis(100)
	...
	// sides.
	totalLokisAlice := (aliceChannel.channelState.LocalCommitment.LocalBalance +
		aliceChannel.channelState.LocalCommitment.RemoteBalance +
		lnwire.NewMSatFromLokis(aliceChannel.channelState.LocalCommitment.CommitFee))
	if totalLokisAlice+htlcAmount != lnwire.NewMSatFromLokis(aliceChannel.Capacity) {
		t.Fatalf("alice's funds leaked: total loki are %v, but channel "+
			"capacity is %v", int64(totalLokisAlice),
			int64(aliceChannel.Capacity))
	}
	totalLokisBob := (bobChannel.channelState.LocalCommitment.LocalBalance +
		bobChannel.channelState.LocalCommitment.RemoteBalance +
		lnwire.NewMSatFromLokis(bobChannel.channelState.LocalCommitment.CommitFee))
	if totalLokisBob+htlcAmount != lnwire.NewMSatFromLokis(bobChannel.Capacity) {
		t.Fatalf("bob's funds leaked: total loki are %v, but channel "+
			"capacity is %v", int64(totalLokisBob),
...
func TestHTLCDustLimit(t *testing.T) {
	...
	// Bob's dust limit.
	htlcSat := (chainutil.Amount(500) + HtlcTimeoutFee(
		aliceChannel.channelState.ChanType,
		chainfee.SatPerKWeight(
			aliceChannel.channelState.LocalCommitment.FeePerKw,
		),
	))
	htlcAmount := lnwire.NewMSatFromLokis(htlcSat)
	...
	// should have been added to Alice's balance and TotalLokisSent.
	...
	if aliceChannel.channelState.TotalMSatSent != htlcAmount {
		t.Fatalf("alice loki sent incorrect: expected %v, got %v",
...
func TestHTLCSigNumber(t *testing.T) {
	...
	createChanWithHTLC := func(htlcValues ...chainutil.Amount) (
		*LightningChannel, *LightningChannel) {

		// Create a test channel funded evenly with Alice having 5 FLC,
		// and Bob having 5 FLC. Alice's dustlimit is 200 sat, while
		// Bob has 1300 sat.
		aliceChannel, bobChannel, err := CreateTestChannels(
			t, channeldb.SingleFunderTweaklessBit,
		)
		...
		for i, htlcSat := range htlcValues {
			htlcMsat := lnwire.NewMSatFromLokis(htlcSat)
	...
	belowDust := chainutil.Amount(500) + HtlcTimeoutFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
	aboveDust := chainutil.Amount(1400) + HtlcSuccessFee(
		channeldb.SingleFunderTweaklessBit, feePerKw,
	)
...
func TestChannelBalanceDustLimit(t *testing.T) {
	...
	aliceBalance := aliceChannel.channelState.LocalCommitment.LocalBalance.ToLokis()
...
func TestStateUpdatePersistence(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromLokis(5000)
...
func TestCancelHTLC(t *testing.T) {
	...
	// of the new HTLC.
	aliceExpectedBalance := chainutil.Amount(chainutil.LokiPerFlokicoin*4) -
		calcStaticFee(channeldb.SingleFunderTweaklessBit, 1)
	if aliceChannel.channelState.LocalCommitment.LocalBalance.ToLokis() !=
	...
	expectedBalance := chainutil.Amount(chainutil.LokiPerFlokicoin * 5)
	...
	if aliceChannel.channelState.LocalCommitment.RemoteBalance.ToLokis() !=
		expectedBalance {

		t.Fatalf("balance is wrong: expected %v, got %v",
			aliceChannel.channelState.LocalCommitment.RemoteBalance.ToLokis(),
			expectedBalance)
	}
	if bobChannel.channelState.LocalCommitment.LocalBalance.ToLokis() !=
		expectedBalance {

		t.Fatalf("balance is wrong: expected %v, got %v",
			bobChannel.channelState.LocalCommitment.LocalBalance.ToLokis(),
			expectedBalance)
	}
	if bobChannel.channelState.LocalCommitment.RemoteBalance.ToLokis() !=
		expectedBalance-staticFee {

		t.Fatalf("balance is wrong: expected %v, got %v",
			bobChannel.channelState.LocalCommitment.RemoteBalance.ToLokis(),
			expectedBalance-staticFee)
	}
}
...
func TestCooperativeCloseDustAdherence(t *testing.T) {
	...
	setDustLimit := func(dustVal chainutil.Amount) {
	...
	setBalances := func(aliceBalance, bobBalance lnwire.MilliLoki) {
	...
	// loki.
	dustLimit := chainutil.Amount(10000)
	setDustLimit(dustLimit)

	// Both sides currently have over 1 FLC settled as part of their
	...
	aliceFee := chainutil.Amount(aliceChannel.CalcFee(aliceFeeRate)) + 1000
	aliceSig, _, _, err := aliceChannel.CreateCloseProposal(
		aliceFee, aliceDeliveryScript, bobDeliveryScript,
	)
	require.NoError(t, err, "unable to close channel")

	bobFee := chainutil.Amount(bobChannel.CalcFee(bobFeeRate)) + 1000
	bobSig, _, _, err := bobChannel.CreateCloseProposal(
		bobFee, bobDeliveryScript, aliceDeliveryScript,
	)
	require.NoError(t, err, "unable to close channel")
	...
	aliceBal := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	bobBal := lnwire.NewMSatFromLokis(250)
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceExpectedBalance := aliceBal.ToLokis() - aliceFee + commitFee
	...
	lowBalance := lnwire.NewMSatFromLokis(aliceFee) + 1000
	...
	if closeTx.TxOut[0].Value != int64(aliceBal.ToLokis()) {
		t.Fatalf("bob's balance is incorrect: expected %v, got %v",
			aliceBal.ToLokis(), closeTx.TxOut[0].Value)
	}
}
...
func TestUpdateFeeConcurrentSig(t *testing.T) {
	...
	htlc := &lnwire.UpdateAddHTLC{
		PaymentHash: paymentHash,
		Amount:      chainutil.LokiPerFlokicoin,
		Expiry:      uint32(5),
	}
...
func TestAddHTLCNegativeBalance(t *testing.T) {
	...
	// First, we'll add 3 HTLCs of 1 FLC each to Alice's commitment.
	const numHTLCs = 3
	htlcAmt := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	for i := 0; i < numHTLCs; i++ {
	...
	// Alice now has an available balance of 2 FLC. We'll add a new HTLC of
	// value 2 FLC, which should make Alice's balance negative (since she
	// has to pay a commitment fee).
	htlcAmt = lnwire.NewMSatFromLokis(2 * chainutil.LokiPerFlokicoin)
...
func testChanSyncOweCommitment(t *testing.T,
	...
	// single state update.
	htlcAmt := lnwire.NewMSatFromLokis(20000)
	const numBobHtlcs = 3
	var bobPreimage [32]byte
	copy(bobPreimage[:], bytes.Repeat([]byte{0xbb}, 32))
	for i := 0; i < 3; i++ {
...
func TestAuxSignerShutdown(t *testing.T) {
	...
	// Each HTLC amount is 0.01 FLC.
	htlcAmt := lnwire.NewMSatFromLokis(0.01 * chainutil.LokiPerFlokicoin)
...
func TestChanSyncFailure(t *testing.T) {
	...
	p[4] ^= 0x01
	modCommitPoint, err := crypto.ParsePubKey(p)
...
func TestChannelRetransmissionFeeUpdate(t *testing.T) {
	...
	bobHtlc := &lnwire.UpdateAddHTLC{
		PaymentHash: rHash,
		Amount:      lnwire.NewMSatFromLokis(20000),
		Expiry:      uint32(10),
	}
	addAndReceiveHTLC(t, bobChannel, aliceChannel, bobHtlc, nil)
	if err := ForceStateTransition(bobChannel, aliceChannel); err != nil {
...
func TestFeeUpdateOldDiskFormat(t *testing.T) {
	...
	htlcAmt := lnwire.NewMSatFromLokis(0.1 * chainutil.LokiPerFlokicoin)
	const numHTLCs = 30
...
func TestChanAvailableBandwidth(t *testing.T) {
	...
	aliceReserve := lnwire.NewMSatFromLokis(
	...
	assertBandwidthEstimateCorrect := func(aliceInitiate bool,
		...
		commitFee := lnwire.NewMSatFromLokis(aliceState.CommitFee)
	...
	const numHtlcs = 3
	var dustAmt lnwire.MilliLoki = 100000
	...
	// transaction.
	htlcAmt := lnwire.NewMSatFromLokis(30000)
	for i := 0; i < numHtlcs; i++ {
...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	aliceBalance := lnwire.NewMSatFromLokis(5 * chainutil.LokiPerFlokicoin)
	bobBalance := lnwire.NewMSatFromLokis(5 * chainutil.LokiPerFlokicoin)

	aliceReserve := lnwire.NewMSatFromLokis(
		aliceChannel.channelState.LocalChanCfg.ChanReserve,
	)
	bobReserve := lnwire.NewMSatFromLokis(
		bobChannel.channelState.LocalChanCfg.ChanReserve,
	)

	aliceDustlimit := lnwire.NewMSatFromLokis(
	...
	htlcFee := lnwire.NewMSatFromLokis(
		feeRate.FeeForWeight(input.HTLCWeight),
	)
	commitFee := lnwire.NewMSatFromLokis(
		aliceChannel.channelState.LocalCommitment.CommitFee,
	)
	htlcTimeoutFee := lnwire.NewMSatFromLokis(
		HtlcTimeoutFee(aliceChannel.channelState.ChanType, feeRate),
	)
	htlcSuccessFee := lnwire.NewMSatFromLokis(
	...
	checkBalance := func(t *testing.T, expBalanceAlice,
		expBalanceBob lnwire.MilliLoki) {
	...
	// balance.
	htlcIndex := uint64(0)
	sendHtlc := func(htlcAmt lnwire.MilliLoki) {
		t.Helper()
...
func TestChanCommitWeightDustHtlcs(t *testing.T) {
	...
	bobDustlimit := lnwire.NewMSatFromLokis(
	...
	htlcIndex := uint64(0)
	addHtlc := func(htlcAmt lnwire.MilliLoki) lntypes.Preimage {
		t.Helper()
...
func TestLockedInHtlcForwardingSkipAfterRestart(t *testing.T) {
	...
	// a state transition.
	var htlcAmt lnwire.MilliLoki = 100000
...
func TestDesyncHTLCs(t *testing.T) {
	...
	// First add one HTLC of value 4.1 FLC.
	htlcAmt := lnwire.NewMSatFromLokis(4.1 * chainutil.LokiPerFlokicoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	// Alice now has gotten all her original balance (5 FLC) back, however,
	...
	// available.
	//
	// We try adding an HTLC of value 1 FLC, which should fail because the
	// balance is unavailable.
	htlcAmt = lnwire.NewMSatFromLokis(1 * chainutil.LokiPerFlokicoin)
...
func TestMaxPendingAmount(t *testing.T) {
	...
	// We set the remote required MaxPendingAmount to 3 FLC. We will
	// attempt to overflow this value and see if it gives us the
	// ErrMaxPendingAmount error.
	maxPending := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin * 3)
	...
	// First, we'll add 2 HTLCs of 1.5 FLC each to Alice's commitment.
	// This won't trigger Alice's ErrMaxPendingAmount error.
	const numHTLCs = 2
	htlcAmt := lnwire.NewMSatFromLokis(1.5 * chainutil.LokiPerFlokicoin)
	for i := 0; i < numHTLCs; i++ {
		htlc, _ := createHTLC(i, htlcAmt)
		addAndReceiveHTLC(t, aliceChannel, bobChannel, htlc, nil)
	}

	// We finally add one more HTLC of 0.1 FLC to Alice's commitment. This
...
func assertChannelBalances(t *testing.T, alice, bob *LightningChannel,
	aliceBalance, bobBalance chainutil.Amount) {

	_, _, line, _ := runtime.Caller(1)

	aliceSelfBalance := alice.channelState.LocalCommitment.LocalBalance.ToLokis()
	aliceBobBalance := alice.channelState.LocalCommitment.RemoteBalance.ToLokis()
	...
	bobSelfBalance := bob.channelState.LocalCommitment.LocalBalance.ToLokis()
	bobAliceBalance := bob.channelState.LocalCommitment.RemoteBalance.ToLokis()
...
func TestChanReserve(t *testing.T) {
	...
	setupChannels := func() (*LightningChannel, *LightningChannel) {
		...
		// We set the remote required ChanReserve to 0.5 FLC. We will
		// attempt to cause Alice's balance to dip below this amount
		// and test whether it triggers the ErrBelowChanReserve error.
		aliceMinReserve := chainutil.Amount(0.5 *
			chainutil.LokiPerFlokicoin)
		...
		bobMinReserve := chainutil.Amount(6 * chainutil.LokiPerFlokicoin)
	...
	//	Alice:	4.5
	//	Bob:	5.0
	htlcAmt := lnwire.NewMSatFromLokis(0.5 * chainutil.LokiPerFlokicoin)
	...
	assertChannelBalances(
		...
		chainutil.LokiPerFlokicoin*4.5-commitFee, chainutil.LokiPerFlokicoin*5,
	)
	...
	// Now we'll add HTLC of 3.5 FLC to Alice's commitment, this should put
	// Alice's balance at 1.5 FLC.
	//
	// Resulting balances:
	//	Alice:	1.5
	//	Bob:	9.5
	htlcAmt = lnwire.NewMSatFromLokis(3.5 * chainutil.LokiPerFlokicoin)
	...
	// Add a second HTLC of 1 FLC. This should fail because it will take
	...
	// Add a HTLC of 2 FLC to Alice, and the settle it.
	// Resulting balances:
	//	Alice:	3.0
	//	Bob:	7.0
	htlcAmt = lnwire.NewMSatFromLokis(2 * chainutil.LokiPerFlokicoin)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		chainutil.LokiPerFlokicoin*3-commitFee, chainutil.LokiPerFlokicoin*5,
	)
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		chainutil.LokiPerFlokicoin*3-commitFee, chainutil.LokiPerFlokicoin*7,
	)

	// And now let Bob add an HTLC of 1 FLC. This will take Bob's balance
	...
	assertChannelBalances(
		t, aliceChannel, bobChannel,
		chainutil.LokiPerFlokicoin*3-commitFee, chainutil.LokiPerFlokicoin*6,
	)
}
...
func TestChanReserveRemoteInitiator(t *testing.T) {
	...
	// We start out with a channel where both parties have 5 FLC.
	aliceChannel, bobChannel, err := CreateTestChannels(
		t, channeldb.SingleFunderTweaklessBit,
	)
	if err != nil {
	...
	// Set Alice's channel reserve to be 5 FLC-commitfee. This means she
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*chainutil.LokiPerFlokicoin - commitFee

	aliceChannel.channelState.LocalChanCfg.ChanReserve = aliceMinReserve
	bobChannel.channelState.RemoteChanCfg.ChanReserve = aliceMinReserve

	// Now let Bob attempt to add an HTLC of 0.1 FLC. He has plenty of
...
func TestChanReserveLocalInitiatorDustHtlc(t *testing.T) {
	...
	commitFee := aliceChannel.channelState.LocalCommitment.CommitFee
	aliceMinReserve := 5*chainutil.LokiPerFlokicoin - commitFee - htlcSat

	aliceChannel.channelState.LocalChanCfg.ChanReserve = aliceMinReserve
	bobChannel.channelState.RemoteChanCfg.ChanReserve = aliceMinReserve

	htlcDustAmt := lnwire.NewMSatFromLokis(htlcSat)
...
func TestMinHTLC(t *testing.T) {
	...
	// We set Alice's MinHTLC to 0.1 FLC. We will attempt to send an
	// HTLC BELOW this value to trigger the ErrBelowMinHTLC error.
	minValue := lnwire.NewMSatFromLokis(0.1 * chainutil.LokiPerFlokicoin)
	...
	// First, we will add an HTLC of 0.5 FLC. This will not trigger
	// ErrBelowMinHTLC.
	htlcAmt := lnwire.NewMSatFromLokis(0.5 * chainutil.LokiPerFlokicoin)
	htlc, _ := createHTLC(0, htlcAmt)
	addAndReceiveHTLC(t, aliceChannel, bobChannel, htlc, nil)
...
func TestNewBreachRetributionSkipsDustHtlcs(t *testing.T) {
	...
	dustValue := chainutil.Amount(200)
	...
	for i := 0; i < 3; i++ {
		...
		h := &lnwire.UpdateAddHTLC{
			PaymentHash: rHash,
			Amount:      lnwire.NewMSatFromLokis(dustValue),
			Expiry:      uint32(10),
			OnionBlob:   fakeOnionBlob,
		}
...
func TestChannelRestoreCommitHeight(t *testing.T) {
	...
	htlcAmount := lnwire.NewMSatFromLokis(100000000)
	htlcAlice, _ := createHTLC(0, htlcAmount)
	addAndReceiveHTLC(t, aliceChannel, bobChannel, htlcAlice, nil)
...
func TestIdealCommitFeeRate(t *testing.T) {
	...
	maxFeeRate := func(c *LightningChannel,
		...
		maxBalance := balance.ToLokis() + currentFee
...
func TestEvaluateView(t *testing.T) {
	const (
		...
		// ourFeeUpdatePerSat is the fee rate *in loki* that we
		...
		// theirFeeUpdatePerSat is the fee rate *in loki* that we
		// expect if we update to ourFeeUpdateAmt.
...
func TestChannelSignedAckRegression(t *testing.T) {
	...
	htlc, preimage := createHTLC(0, lnwire.MilliLoki(5000000))
	...
	htlc2, _ := createHTLC(0, lnwire.MilliLoki(5000000))
...
func TestMayAddOutgoingHtlc(t *testing.T) {
	...
	chanBal, err := chainutil.NewAmount(testChannelCapacity)
	require.NoError(t, err)

	// Each side should be able to add 1/4 of total channel balance since
	// we're 50/50 split.
	mayAdd := lnwire.MilliLoki(chanBal / 4 * 1000)
	...
	mayNotAdd := lnwire.MilliLoki(chanBal * 1000)
...
func testGetDustSum(t *testing.T, chantype channeldb.ChannelType) {
	...
	htlc1Amt := lnwire.MilliLoki(700_000)
	...
	checkDust(aliceChannel, lnwire.MilliLoki(0), htlc1Amt)

	// Assert that GetDustSum from Bob's perspective results in the same
	// conditions above holding.
	checkDust(bobChannel, htlc1Amt, lnwire.MilliLoki(0))
	...
	require.NoError(t, err)
	checkDust(aliceChannel, lnwire.MilliLoki(0), htlc1Amt)
	checkDust(bobChannel, lnwire.MilliLoki(0), lnwire.MilliLoki(0))

	// Alice now sends an HTLC of 100sats, which is below both sides' dust
	// limits.
	htlc2Amt := lnwire.MilliLoki(100_000)
	...
	htlc3Amt := lnwire.MilliLoki(400_000)
...
func TestCreateHtlcRetribution(t *testing.T) {
	...
	dummyPrivate, _ := crypto.PrivKeyFromBytes([]byte{1})
	testAmt := chainutil.Amount(100)
...
func TestCreateBreachRetribution(t *testing.T) {
	...
	htlc := &channeldb.HTLCEntry{
		Amt: tlv.NewRecordT[tlv.TlvType4](
			tlv.NewBigSizeT(chainutil.Amount(testAmt)),
		),
		Incoming: tlv.NewPrimitiveRecord[tlv.TlvType3](true),
	...
	ourAmtMsat := lnwire.MilliLoki(ourAmt * 1000)
	theirAmtMsat := lnwire.MilliLoki(theirAmt * 1000)
...
func TestCreateBreachRetributionLegacy(t *testing.T) {
	...
	// present).
	require.EqualValues(t, revokedLog.LocalBalance.ToLokis(), ourAmt)
...
func createRandomHTLC(t *testing.T, incoming bool) channeldb.HTLC {
	...
	return channeldb.HTLC{
		Signature:     sig,
		RHash:         rHash,
		Amt:           lnwire.MilliLoki(rand.Uint64()),
...
func TestApplyCommitmentFee(t *testing.T) {
	var (
		...
		additionalHtlc = lnwire.NewMSatFromLokis(
	...
	testCases := []struct {
		...
		channel           *LightningChannel
		buffer            BufferType
		balance           lnwire.MilliLoki
		expectedBalance   lnwire.MilliLoki
		expectedBufferAmt lnwire.MilliLoki
		expectedCommitFee lnwire.MilliLoki
		bufferAmt         lnwire.MilliLoki
		expectedErr       error
	}{
		{
...
func TestAsynchronousSendingContraint(t *testing.T) {
	...
	// capactiy is 10 FLC with every side having 5 FLC at start. The fee
	...
	// all its local funds.
	// -----add------>|
	htlc1, _ := createHTLC(0, lnwire.NewMSatFromLokis(htlcAmount))
	...
	// one.
	htlcAmt2 := lnwire.NewMSatFromLokis(
...
func TestAsynchronousSendingWithFeeBuffer(t *testing.T) {
	...
	htlcAmount := capacity/2 - aliceReserve - feeBuffer.ToLokis()
	...
	htlcAmt3 := lnwire.NewMSatFromLokis(
		aliceChannel.channelState.LocalChanCfg.DustLimit + htlcFee,
	)
	...
	require.Equal(t, aliceChannel.channelState.LocalCommitment.LocalBalance,
		lnwire.NewMSatFromLokis(aliceReserve))
}
...
func TestEnforceFeeBuffer(t *testing.T) {
	...
	commitFeeMsat := lnwire.NewMSatFromLokis(commitFee)
	...
	htlcAmt1 := capacity/2 - aliceReserve - feeBuffer.ToLokis()
	...
	//	<----sig-------	|---------------
	//	---------------	|-----rev------>
	htlc1, _ := createHTLC(0, lnwire.NewMSatFromLokis(htlcAmt1))
	...
	//	---------------	|-----sig------>
	//	<----rev-------	|---------------
	htlcAmt2 := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicent)
	htlc2, _ := createHTLC(0, htlcAmt2)
...
func TestBlindingPointPersistence(t *testing.T) {
	// Create a test channel which will be used for the duration of this
	// test. The channel will be funded evenly with Alice having 5 FLC,
	// and Bob having 5 FLC.
	aliceChannel, bobChannel, err := CreateTestChannels(
		t, channeldb.SingleFunderTweaklessBit,
	)
...
func TestCreateCooperativeCloseTx(t *testing.T) {
	...
	localDust := chainutil.Amount(400)
	remoteDust := chainutil.Amount(400)
	...
	tests := []struct {
		...
		localBalance  chainutil.Amount
		remoteBalance chainutil.Amount
...
func TestNoopAddBelowReserve(t *testing.T) {
	...
	const (
		...
		// milli-loki.
		htlcAmt = lnwire.MilliLoki(500_000)
	...
	require.Equal(
		t, bobBalanceFinal.ToLokis(),
		bobChan.LocalChanReserve()+htlcAmt.ToLokis(),
	)
}
...
func TestEvaluateNoOpHtlc(t *testing.T) {
	testCases := []struct {
		name                        string
		localBalance, remoteBalance chainutil.Amount
		localReserve, remoteReserve chainutil.Amount
	...
	for _, testCase := range testCases {
		...
		if tc.localBalance != 0 && tc.localReserve != 0 {
			...
			aliceChan.channelState.LocalCommitment.LocalBalance =
				lnwire.NewMSatFromLokis(tc.localBalance)
		}
		...
		if tc.remoteBalance != 0 && tc.remoteReserve != 0 {
			...
			aliceChan.channelState.RemoteCommitment.RemoteBalance =
				lnwire.NewMSatFromLokis(tc.remoteBalance)
		}
