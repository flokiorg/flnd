...
const (
	...
	// set fee rate.
	AnchorChanReservedValue = chainutil.Amount(10_000)
...
type InitFundingReserveMsg struct {
	...
	// to this channel.
	RemoteFundingAmt chainutil.Amount
	...
	// CommitFeePerKw is the starting accepted loki/Kw fee for the set
	...
	// PushMSat is the number of milli-loki that should be pushed over
	// the responder as part of the initial channel creation.
	PushMSat lnwire.MilliLoki
...
// LightningWallet is a domain specific, yet general Flokicoin wallet capable of
...
// general purpose Flokicoin wallet within it. Therefore, it is also able to
// serve as a regular Flokicoin wallet which uses HD keys. The wallet is highly
...
// Flokicoin Core + ZeroMQ, etc. Eventually, the wallet won't require a full-node
...
func (l *LightningWallet) ConfirmedBalance(confs int32,
	account string) (chainutil.Amount, error) {
...
func (l *LightningWallet) handleFundingReserveRequest(req *InitFundingReserveMsg) {
	...
	if !ok {
		...
		fundingReq := &chanfunding.Request{
			...
			PushAmt: lnwire.MilliLoki.ToLokis(
				req.PushMSat,
			),
...
func (l *LightningWallet) CheckReservedValue(in []wire.OutPoint,
	out []*wire.TxOut, numAnchorChans int) (chainutil.Amount, error) {
	...
	// them.
	var walletBalance chainutil.Amount
	...
	for _, txOut := range out {
		...
		for _, addr := range addrs {
			...
			walletBalance += chainutil.Amount(txOut.Value)
...
func (l *LightningWallet) initOurContribution(reservation *ChannelReservation,
	...
	nodeID *crypto.PublicKey, keyRing keychain.KeyRing) error {
...
// version of the commitment transaction.
func CreateCommitmentTxns(localBalance, remoteBalance chainutil.Amount,
	ourChanCfg, theirChanCfg *channeldb.ChannelConfig,
	localCommitPoint, remoteCommitPoint *crypto.PublicKey,
	...
	otxn := chainutil.NewTx(ourCommitTx)
	...
	ttxn := chainutil.NewTx(theirCommitTx)
...
func (l *LightningWallet) handleChanPointReady(req *continueContributionMsg) {
	...
	localBalance := pendingReservation.partialState.LocalCommitment.LocalBalance.ToLokis()
	remoteBalance := pendingReservation.partialState.LocalCommitment.RemoteBalance.ToLokis()
...
func DeriveStateHintObfuscator(key1, key2 *crypto.PublicKey) [StateHintSize]byte {
	h := sha256.New()
