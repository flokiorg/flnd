...
func fetchFlokicoindFilters(client *rpcclient.Client) ([]SatPerKWeight, error) {
	...
	for _, peerResp := range peerResps {
		...
		// is in fractions of a bitcoin that represents the loki
		// per KvB. We need to convert this fraction to whole loki
...
func isWithinBounds(filter SatPerKVByte) bool {
	// Ignore values of 0 and MaxLoki. A value of 0 likely means that
	// the peer hasn't sent over a feefilter and a value of MaxLoki
