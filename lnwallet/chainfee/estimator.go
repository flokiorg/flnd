...
type StaticEstimator struct {
	// feePerKW is the static fee rate in loki-per-vbyte that will be
...
func (b *BtcdEstimator) fetchMinRelayFee() (SatPerKWeight, error) {
	...
	relayFee, err := chainutil.NewAmount(info.RelayFee)
	if err != nil {
		return 0, err
	}
...
func (b *BtcdEstimator) fetchEstimateInner(confTarget uint32) (SatPerKWeight,
	...
	// Next, we'll convert the returned value to loki, as it's
	// currently returned in FLC.
	satPerKB, err := chainutil.NewAmount(btcPerKB)
	if err != nil {
		return 0, err
	}
...
// FlokicoindEstimator is an implementation of the Estimator interface backed by
...
type FlokicoindEstimator struct {
...
// NewFlokicoindEstimator creates a new FlokicoindEstimator given a fully populated
// rpc config that is able to successfully connect and authenticate with the
...
// for a fee estimate.
func NewFlokicoindEstimator(rpcConfig rpcclient.ConnConfig, feeMode string,
	fallBackFeeRate SatPerKWeight) (*FlokicoindEstimator, error) {
	...
	fetchCb := func() ([]SatPerKWeight, error) {
		return fetchFlokicoindFilters(chainConn)
	}

	return &FlokicoindEstimator{
		fallbackFeePerKW: fallBackFeeRate,
...
//
// NOTE: This method is part of the Estimator interface.
func (b *FlokicoindEstimator) Start() error {
...
func (b *FlokicoindEstimator) fetchMinMempoolFee() (SatPerKWeight, error) {
	...
	minMempoolFee, err := chainutil.NewAmount(info.MempoolMinFee)
	if err != nil {
		return 0, err
	}
...
//
// NOTE: This method is part of the Estimator interface.
func (b *FlokicoindEstimator) Stop() error {
	b.filterManager.Stop()
	return nil
}
...
//
// NOTE: This method is part of the Estimator interface.
func (b *FlokicoindEstimator) EstimateFeePerKW(
...
//
// NOTE: This method is part of the Estimator interface.
func (b *FlokicoindEstimator) RelayFeePerKW() SatPerKWeight {
	// Get a suitable minimum feerate to use. This may optionally use the
	// median of our peers' feefilter values.
	feeCapClosure := func() (SatPerKWeight, error) {
...
// fetchEstimate returns a fee estimate for a transaction to be confirmed in
// confTarget blocks. The estimate is returned in sat/kw.
func (b *FlokicoindEstimator) fetchEstimate(confTarget uint32, feeMode string) (
	SatPerKWeight, error) {
...
func (b *FlokicoindEstimator) fetchEstimateInner(confTarget uint32,
	...
	// Next, we'll parse the response to get the FLC per KB.
	feeEstimate := struct {
	...
	// Next, we'll convert the returned value to loki, as it's currently
	// returned in FLC.
	satPerKB, err := chainutil.NewAmount(feeEstimate.FeeRate)
	if err != nil {
		return 0, err
	}

	// Flokicoind will not report any fee estimation if it has not enough
...
// A compile-time assertion to ensure that FlokicoindEstimator implements the
// Estimator interface.
var _ Estimator = (*FlokicoindEstimator)(nil)
...
// a user-specified fee estimation API for Flokicoin. It expects the response
