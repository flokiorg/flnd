...
func getInvreqChain(ir *InvoiceRequest) [32]byte {
	chain := flokicoinMainnetGenesisHash
...
func getInvoiceRequestOfferChains(ir *InvoiceRequest) [][32]byte {
	...
	if len(chains) == 0 {
		chains = [][32]byte{flokicoinMainnetGenesisHash}
	}
...
// defaults to Flokicoin mainnet, and the reader must reject offers that do not
...
func ValidateOfferRead(o *Offer, now time.Time, activeChain [32]byte) error {
	...
	// offer_chains TLV means "Flokicoin mainnet" per spec, normalised by
	// getOfferChains.
...
// flokicoinMainnetGenesisHash is the genesis hash for Flokicoin mainnet, used as
// the default when offer_chains is absent per the spec.
var flokicoinMainnetGenesisHash = [32]byte(*chaincfg.MainNetParams.GenesisHash)

// getOfferChains returns the chains an offer is valid for. If offer_chains is
// absent, the spec defaults to Flokicoin mainnet.
...
func checkPubKeyNotNil[T tlv.TlvType](
	opt tlv.OptionalRecordT[T, *crypto.PublicKey], name string) error {

	return fn.MapOptionZ(opt.ValOpt(), func(pk *crypto.PublicKey) error {
		if pk == nil {
