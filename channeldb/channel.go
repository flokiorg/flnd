...
type ChannelStateBounds struct {
	...
	// particular time.
	MaxPendingAmount lnwire.MilliLoki
...
type CommitmentParams struct {
	// DustLimit is the threshold (in loki) below which any outputs
...
type OpenChannel struct {
	...
	// push amount.
	InitialLocalBalance lnwire.MilliLoki

	// InitialRemoteBalance is the balance they have during the channel
	// opening.
	InitialRemoteBalance lnwire.MilliLoki
	...
	RemoteCurrentRevocation *crypto.PublicKey

	// RemoteNextRevocation is the revocation key to be used for the *next*
	// commitment transaction we create for the local node. Within the
	// specification, this value is referred to as the
	// per-commitment-point.
	RemoteNextRevocation *crypto.PublicKey
...
func (c *OpenChannel) amendTlvData(auxData openChannelTlvData) {
	...
	c.InitialLocalBalance = lnwire.MilliLoki(
		auxData.initialLocalBalance.Val,
	)
	c.InitialRemoteBalance = lnwire.MilliLoki(
...
func fetchChanBucket(tx kvdb.RTx, nodeKey *crypto.PublicKey,
...
func fetchChanBucketRw(tx kvdb.RwTx, nodeKey *crypto.PublicKey,
...
// the channel.
func (c *OpenChannel) MarkDataLoss(commitPoint *crypto.PublicKey) error {
	c.Lock()
	defer c.Unlock()
...
func (c *OpenChannel) DataLossCommitPoint() (*crypto.PublicKey, error) {
...
func (c *OpenChannel) SecondCommitmentPoint() (*crypto.PublicKey, error) {
	c.RLock()
	defer c.RUnlock()
...
func NewMusigVerificationNonce(pubKey *crypto.PublicKey, targetHeight uint64,
...
func (c *OpenChannel) InsertNextRevocation(revKey *crypto.PublicKey) error {
	c.Lock()
	defer c.Unlock()
...
type ChannelCloseSummary struct {
	...
	SettledBalance chainutil.Amount
	...
	TimeLockedBalance chainutil.Amount
