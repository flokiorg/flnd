...
type (
	// BigSizeAmount is a type alias for a TLV record of a chainutil.Amount.
	BigSizeAmount = tlv.BigSizeT[chainutil.Amount]

	// BigSizeMilliLoki is a type alias for a TLV record of a
	// lnwire.MilliLoki.
	BigSizeMilliLoki = tlv.BigSizeT[lnwire.MilliLoki]
)
...
type HTLCEntry struct {
	...
	// Amt is the amount of loki this HTLC escrows.
	Amt tlv.RecordT[tlv.TlvType4, tlv.BigSizeT[chainutil.Amount]]
...
func NewHTLCEntryFromHTLC(htlc HTLC) (*HTLCEntry, error) {
	h := &HTLCEntry{
		...
		Amt: tlv.NewRecordT[tlv.TlvType4](
			tlv.NewBigSizeT(htlc.Amt.ToLokis()),
		),
...
type RevocationLog struct {
	...
	// all revocation logs.
	OurBalance tlv.OptionalRecordT[tlv.TlvType3, BigSizeMilliLoki]
	...
	// this field, it could be the case that the field is not present for
	// all revocation logs.
	TheirBalance tlv.OptionalRecordT[tlv.TlvType4, BigSizeMilliLoki]
...
func NewRevocationLog(ourOutputIndex uint16, theirOutputIndex uint16,
	...
	theirBalance fn.Option[lnwire.MilliLoki], htlcs []*HTLCEntry,
	...
	ourBalance.WhenSome(func(balance lnwire.MilliLoki) {
		rl.OurBalance = tlv.SomeRecordT(tlv.NewRecordT[tlv.TlvType3](
			tlv.NewBigSizeT(balance),
		))
	})

	theirBalance.WhenSome(func(balance lnwire.MilliLoki) {
...
func serializeRevocationLog(w io.Writer, rl *RevocationLog) error {
	...
	rl.OurBalance.WhenSome(
		func(r tlv.RecordT[tlv.TlvType3, BigSizeMilliLoki]) {
			records = append(records, r.Record())
		},
	)

	rl.TheirBalance.WhenSome(
		func(r tlv.RecordT[tlv.TlvType4, BigSizeMilliLoki]) {
			records = append(records, r.Record())
		},
	)
