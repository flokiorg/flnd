...
import (
	...
	mig25 "github.com/flokiorg/flnd/channeldb/migration25"
	mig26 "github.com/flokiorg/flnd/channeldb/migration26"
	mig "github.com/flokiorg/flnd/channeldb/migration_01_to_11"
...
var (
	...
	localBalance1 = lnwire.MilliLoki(990_950_000)

	// remoteBalance1 is the RemoteBalance used in oldLog1.
	remoteBalance1 = lnwire.MilliLoki(0)
	...
	htlc = mig.HTLC{
		RHash:         rHash,
		Amt:           lnwire.MilliLoki(100_000_000),
		RefundTimeout: 489,
		OutputIndex:   0,
		Incoming:      false,
	...
	localBalance2 = lnwire.MilliLoki(888_800_000)

	// remoteBalance2 is the RemoteBalance used in oldLog2.
	remoteBalance2 = lnwire.MilliLoki(0)
	...
	newLog3 = mig.ChannelCommitment{
		...
		RemoteLogIndex:  0,
		RemoteHtlcIndex: 0,
		LocalBalance:    lnwire.MilliLoki(888_800_000),
		RemoteBalance:   0,
		CommitTx:        commitTx2,
		Htlcs:           []mig.HTLC{htlc},
	}
	...
	localMusigKey, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	localRevocationBasePoint, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	localPaymentBasePoint, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	localDelayBasePoint, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	localHtlcBasePoint, _ = crypto.ParsePubKey([]byte{
		0x3,
	...
	remoteMultiSigKey, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	remoteRevocationBasePoint, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	remotePaymentBasePoint, _ = crypto.ParsePubKey([]byte{
		0x3,
	...
	remoteDelayBasePoint, _ = crypto.ParsePubKey([]byte{
		0x2,
	...
	remoteHtlcBasePoint, _ = crypto.ParsePubKey([]byte{
		0x3,
...
func createTestChannel(nodePub *crypto.PublicKey) *mig26.OpenChannel {
	...
	localCfg := mig.ChannelConfig{
		ChannelConstraints: mig.ChannelConstraints{
			DustLimit:        chainutil.Amount(354),
