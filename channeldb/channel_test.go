...
var (
	...
	privKey, pubKey = crypto.PrivKeyFromBytes(key[:])
...
func pubKeyOption(pubKey *crypto.PublicKey) testChannelOption {
	return func(params *testChannelParams) {
...
func createTestChannelState(t *testing.T, cdb *ChannelStateDB) *OpenChannel {
	...
	localStateBounds := ChannelStateBounds{
		...
		ChanReserve:      chainutil.Amount(rand.Int63()),
		MinHTLC:          lnwire.MilliLoki(rand.Int63()),
		MaxAcceptedHtlcs: uint16(rand.Int31()),
	}

	localRenderingParams := CommitmentParams{
		DustLimit: chainutil.Amount(rand.Int63()),
	...
	return &OpenChannel{
		...
		LocalCommitment: ChannelCommitment{
			CommitHeight:  0,
			LocalBalance:  lnwire.MilliLoki(9000),
			RemoteBalance: lnwire.MilliLoki(3000),
			CommitFee:     chainutil.Amount(rand.Int63()),
			FeePerKw:      chainutil.Amount(5000),
		...
		RemoteCommitment: ChannelCommitment{
			CommitHeight:  0,
			LocalBalance:  lnwire.MilliLoki(3000),
			RemoteBalance: lnwire.MilliLoki(9000),
			CommitFee:     chainutil.Amount(rand.Int63()),
			FeePerKw:      chainutil.Amount(5000),
			CommitTx:      channels.TestFundingTx,
			CommitSig:     bytes.Repeat([]byte{1}, 71),
		...
		InitialLocalBalance:     lnwire.MilliLoki(9000),
		InitialRemoteBalance:    lnwire.MilliLoki(3000),
		Memo:                    []byte("test"),
...
func TestOpenChannelPutGetDelete(t *testing.T) {
	...
	nextRevKey, err := crypto.NewPrivateKey()
	...
	closeSummary := &ChannelCloseSummary{
		...
		SettledBalance:    chainutil.Amount(500),
		TimeLockedBalance: chainutil.Amount(10000),
		IsPending:         false,
...
func assertRevocationLogEntryEqual(t *testing.T, c *ChannelCommitment,
	...
	for i, rHtlc := range r.HTLCEntries {
		...
		require.Equal(
			t, rHtlc.Amt.Val.Int(), cHtlc.Amt.ToLokis(), "Amt",
		)
		require.Equal(
...
func TestChannelStateTransition(t *testing.T) {
	...
	commitment := ChannelCommitment{
		...
		RemoteLogIndex:  2,
		RemoteHtlcIndex: 1,
		LocalBalance:    lnwire.MilliLoki(1e8),
		RemoteBalance:   lnwire.MilliLoki(1e8),
		CommitFee:       55,
		FeePerKw:        99,
		CommitTx:        newTx,
		CommitSig:       newSig,
	...
	remoteCommit.LocalBalance = lnwire.MilliLoki(2e8)
	remoteCommit.RemoteBalance = lnwire.MilliLoki(3e8)
		...
		LogUpdates: []LogUpdate{
			{
				...
				UpdateMsg: &lnwire.UpdateAddHTLC{
					ID:     1,
					Amount: lnwire.NewMLokFromLokis(100),
					Expiry: 25,
				},
			},
			{
				...
				UpdateMsg: &lnwire.UpdateAddHTLC{
					ID:     2,
					Amount: lnwire.NewMLokFromLokis(200),
					Expiry: 50,
				},
			},
		},
...
func TestFetchClosedChannels(t *testing.T) {
	...
	summary := &ChannelCloseSummary{
		...
		SettledBalance:    state.LocalCommitment.LocalBalance.ToLokis(),
		TimeLockedBalance: state.RemoteCommitment.LocalBalance.ToLokis() + 10000,
		CloseType:         RemoteForceClose,
		IsPending:         true,
...
func TestOnionBlobIncorrectLength(t *testing.T) {
	...
	require.NoError(t, WriteElements(
		...
		testSig.Serialize(), false, lnwire.MilliLoki(10), key,
		uint32(1),
