...
func WriteElement(w io.Writer, element interface{}) error {
	...
	case chainutil.Amount:
		if err := binary.Write(w, byteOrder, uint64(e)); err != nil {
			return err
		}

	case lnwire.MilliLoki:
		if err := binary.Write(w, byteOrder, uint64(e)); err != nil {
			return err
		}
...
func ReadElement(r io.Reader, element interface{}) error {
	...
	case *lnwire.MilliLoki:
		...
		*e = lnwire.MilliLoki(a)

	case **crypto.PrivateKey:
		var b [crypto.PrivKeyBytesLen]byte
		if _, err := io.ReadFull(r, b[:]); err != nil {
			return err
		}

		priv, _ := crypto.PrivKeyFromBytes(b[:])
		*e = priv

	case **crypto.PublicKey:
		var b [crypto.PubKeyBytesLenCompressed]byte
		if _, err := io.ReadFull(r, b[:]); err != nil {
			return err
		}

		pubKey, err := crypto.ParsePubKey(b[:])
		if err != nil {
			return err
		}
		*e = pubKey
