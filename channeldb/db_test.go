...
func TestFetchClosedChannelForID(t *testing.T) {
	...
	for i := uint32(0); i < numChans; i++ {
		...
		closeSummary := &ChannelCloseSummary{
			...
			SettledBalance: chainutil.Amount(500 + i),
		}
	...
	for i := uint32(0); i < numChans; i++ {
		...
		// SettledBalance.
		if fetchedSummary.SettledBalance != chainutil.Amount(500+i) {
			t.Fatalf("summaries don't match: expected %v got %v",
				chainutil.Amount(500+i),
...
func genRandomChannelShell() (*ChannelShell, error) {
	...
	_, pub := crypto.PrivKeyFromBytes(testPriv[:])
...
func createLightningNode(priv *crypto.PrivateKey) *models.LightningNode {
