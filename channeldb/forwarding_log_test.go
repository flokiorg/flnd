...
func TestForwardingLogBasicStorageAndQuery(t *testing.T) {
	...
	for i := 0; i < numEvents; i++ {
		events[i] = ForwardingEvent{
			...
			AmtIn:          lnwire.MilliLoki(rand.Int63()),
			AmtOut:         lnwire.MilliLoki(rand.Int63()),
...
func TestForwardingLogDeletion(t *testing.T) {
	...
	for i := range numEvents {
		amtIn := lnwire.MilliLoki(10000 + rand.Intn(5000))
		amtOut := lnwire.MilliLoki(9000 + rand.Intn(4000))
		events[i] = ForwardingEvent{
			Timestamp:      timestamp,
...
func TestForwardingLogPartialDeletion(t *testing.T) {
	...
	for i := range numEvents {
		events[i] = ForwardingEvent{
			...
			AmtIn:          lnwire.MilliLoki(10000),
			AmtOut:         lnwire.MilliLoki(9500),
			IncomingHtlcID: fn.Some(uint64(i)),
			OutgoingHtlcID: fn.Some(uint64(i)),
		}
		timestamp = timestamp.Add(time.Minute * 10)
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		for i := range numEvents {
			...
			events[i] = ForwardingEvent{
				...
				AmtIn:          lnwire.MilliLoki(amtIn),
				AmtOut:         lnwire.MilliLoki(amtOut),
				IncomingHtlcID: fn.Some(uint64(i)),
				OutgoingHtlcID: fn.Some(uint64(i)),
			}
