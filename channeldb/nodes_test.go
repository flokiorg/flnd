...
func TestLinkNodeEncodeDecode(t *testing.T) {
	...
	// LinkNode instances.
	_, pub1 := crypto.PrivKeyFromBytes(key[:])
	_, pub2 := crypto.PrivKeyFromBytes(rev[:])
...
func TestFindMissingLinkNodes(t *testing.T) {
	...
	_, pub3 := crypto.PrivKeyFromBytes(testKey[:])

	// Test 1: All nodes missing (empty database).
	allPubs := []*crypto.PublicKey{pub1, pub2, pub3}
...
func TestCreateLinkNodes(t *testing.T) {
	...
	_, pub4 := crypto.PrivKeyFromBytes(testKey4[:])
