...
// channel open with. Information such as the Flokicoin network the node
...
// Flokicoin's addrmanager. The TCP address information stored within the struct
...
type LinkNode struct {
	// Network indicates the Flokicoin network that the LinkNode advertises
...
func NewLinkNode(db *LinkNodeDB, bitNet wire.FlokicoinNet, pub *crypto.PublicKey,
...
func (l *LinkNodeDB) FindMissingLinkNodes(tx kvdb.RTx,
	pubKeys []*crypto.PublicKey) ([]*crypto.PublicKey, error) {
...
// database.
func (l *LinkNodeDB) DeleteLinkNode(identity *crypto.PublicKey) error {
	return kvdb.Update(l.backend, func(tx kvdb.RwTx) error {
		return deleteLinkNode(tx, identity)
	}, func() {})
}

func deleteLinkNode(tx kvdb.RwTx, identity *crypto.PublicKey) error {
	nodeMetaBucket := tx.ReadWriteBucket(nodeInfoBucket)
	if nodeMetaBucket == nil {
		return ErrLinkNodesNotFound
	}
...
func (l *LinkNodeDB) FetchLinkNode(identity *crypto.PublicKey) (*LinkNode, error) {
...
func fetchLinkNode(tx kvdb.RTx, targetPub *crypto.PublicKey) (*LinkNode, error) {
...
func deserializeLinkNode(r io.Reader) (*LinkNode, error) {
	...
	node.Network = wire.FlokicoinNet(byteOrder.Uint32(buf[:4]))

	var pub [33]byte
	if _, err := io.ReadFull(r, pub[:]); err != nil {
		return nil, err
	}
	node.IdentityPub, err = crypto.ParsePubKey(pub[:])
	if err != nil {
		return nil, err
	}
