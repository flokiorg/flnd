...
import (
	...
	switchhop "github.com/flokiorg/flnd/htlcswitch/hop"
...
const (
	...
	noFeeLimit = lnwire.MilliLoki(math.MaxUint32)
)
...
var (
	...
	testAuthProof = models.ChannelAuthProof{
		...
		FlokicoinSig1Bytes: testSig.Serialize(),
		FlokicoinSig2Bytes: testSig.Serialize(),
	}
)

// noProbabilitySource is used in testing to return the same probability 1 for
// all edges.
func noProbabilitySource(route.Vertex, route.Vertex, lnwire.MilliLoki,
	chainutil.Amount) float64 {
...
func parseTestGraph(t *testing.T, useCache bool, path string) (
	*testGraphInstance, error) {
	...
	for _, node := range g.Nodes {
		...
		if len(privBytes) > 0 {
			key, derivedPub := crypto.PrivKeyFromBytes(
				privBytes,
			)
	...
	for _, edge := range g.Edges {
		...
		edgeInfo := models.ChannelEdgeInfo{
			...
			Capacity:     chainutil.Amount(edge.Capacity),
		}

		copy(edgeInfo.NodeKey1Bytes[:], node1Bytes)
		copy(edgeInfo.NodeKey2Bytes[:], node2Bytes)
		copy(edgeInfo.FlokicoinKey1Bytes[:], node1Bytes)
		copy(edgeInfo.FlokicoinKey2Bytes[:], node2Bytes)

		shortID := lnwire.NewShortChanIDFromInt(edge.ChannelID)
		links[shortID] = &mockLink{
			bandwidth: lnwire.MilliLoki(
		...
		edgePolicy := &models.ChannelEdgePolicy{
			...
			MinHTLC:                   lnwire.MilliLoki(edge.MinHTLC),
			MaxHTLC:                   lnwire.MilliLoki(edge.MaxHTLC),
			FeeBaseMSat:               lnwire.MilliLoki(edge.FeeBaseMsat),
			FeeProportionalMillionths: lnwire.MilliLoki(edge.FeeRate),
			ToNode:                    targetNode,
		}
...
type testChannelPolicy struct {
	Expiry             uint16
	MinHTLC            lnwire.MilliLoki
	MaxHTLC            lnwire.MilliLoki
	FeeBaseMsat        lnwire.MilliLoki
	FeeRate            lnwire.MilliLoki
...
func symmetricTestChannel(alias1, alias2 string, capacity chainutil.Amount,
...
func asymmetricTestChannel(alias1, alias2 string, capacity chainutil.Amount,
...
type testGraphInstance struct {
	...
	privKeyMap map[string]*crypto.PrivateKey
...
func createTestGraphFromChannels(t *testing.T, useCache bool,
	...
	addNodeWithAlias := func(alias string,
		...
		privKey, pubKey := crypto.PrivKeyFromBytes(keyBytes)
	...
	for _, testChannel := range testChannels {
		...
		capacity := lnwire.MilliLoki(testChannel.Capacity * 1000)
		...
		edgeInfo := models.ChannelEdgeInfo{
			...
			FlokicoinKey1Bytes: node1Vertex,
			NodeKey2Bytes:      node2Vertex,
			FlokicoinKey2Bytes: node2Vertex,
		}
...
func runFindPathWithMetadata(t *testing.T, useCache bool) {
	...
	paymentAmt := lnwire.NewMSatFromLokis(100)
...
type expectedHop struct {
	alias     string
	fee       lnwire.MilliLoki
	fwdAmount lnwire.MilliLoki
	timeLock  uint32
}

type basicGraphPathFindingTestCase struct {
	target                string
	paymentAmt            chainutil.Amount
	feeLimit              lnwire.MilliLoki
	expectedTotalAmt      lnwire.MilliLoki
...
func testBasicGraphPathFindingCase(t *testing.T, graphInstance *testGraphInstance,
	...
	paymentAmt := lnwire.NewMSatFromLokis(test.paymentAmt)
	...
	var expectedTotalFee lnwire.MilliLoki
...
func runPathFindingWithBlindedPathDuplicateHop(t *testing.T, useCache bool) {
	...
	songokuPubKey, err := crypto.ParsePubKey(songokuPubKeyBytes[:])
	require.NoError(t, err, "unable to parse public key from bytes")

	_, pkb1 := crypto.PrivKeyFromBytes([]byte{2})
	_, pkb2 := crypto.PrivKeyFromBytes([]byte{3})
	_, blindedPoint := crypto.PrivKeyFromBytes([]byte{5})
...
func TestNewRoute(t *testing.T) {
	...
	createHop := func(baseFee lnwire.MilliLoki,
		feeRate lnwire.MilliLoki,
		bandwidth lnwire.MilliLoki,
	...
	testCases := []struct {
		...
		// indicated by hops.
		paymentAmount lnwire.MilliLoki
		...
		expectedTotalAmount lnwire.MilliLoki
	...
	}{
		{
			...
			metadata:              []byte{1, 2, 3},
			expectedFees:          []lnwire.MilliLoki{0},
			...
			expectedFees:          []lnwire.MilliLoki{130, 0},
			...
			expectedFees:          []lnwire.MilliLoki{1, 1, 0},
			...
			expectedFees:          []lnwire.MilliLoki{1010, 1000, 0},
			...
			expectedFees:          []lnwire.MilliLoki{101, 1000, 0},
...
func runNewRoutePathTooLong(t *testing.T, useCache bool) {
	...
	payAmt := lnwire.MilliLoki(100001)
...
func runPathInsufficientCapacity(t *testing.T, useCache bool) {
	...
	// To test his we'll attempt to make a payment of 1 FLC, or 100 million
	// loki. The largest channel in the basic graph is of size 100k
	// loki, so we shouldn't be able to find a path to sophon even
	// though we have a 2-hop link.
	target := graph.aliasMap["sophon"]

	payAmt := lnwire.NewMSatFromLokis(chainutil.LokiPerFlokicoin)
	_, err = dbFindPath(
		graph.graph, nil, &mockBandwidthHints{},
		noRestrictions, testPathFindingConfig,
...
func runRouteFailDisabledEdge(t *testing.T, useCache bool) {
	...
	target := graph.aliasMap["sophon"]
	payAmt := lnwire.NewMSatFromLokis(105000)
	_, err = dbFindPath(
		graph.graph, nil, &mockBandwidthHints{},
		noRestrictions, testPathFindingConfig,
		sourceNode.PubKeyBytes, target, payAmt, 0, 0,
...
func runPathSourceEdgesBandwidth(t *testing.T, useCache bool) {
	...
	// cheapest path.
	target := graph.aliasMap["sophon"]
	payAmt := lnwire.NewMSatFromLokis(50000)
	path, err := dbFindPath(
		graph.graph, nil, &mockBandwidthHints{},
		noRestrictions, testPathFindingConfig,
		sourceNode.PubKeyBytes, target, payAmt, 0, 0,
	...
	bandwidths := &mockBandwidthHints{
		hints: map[uint64]lnwire.MilliLoki{
			roasToSongoku: 0,
			roasToPham:    0,
		},
	}
...
func TestPathFindSpecExample(t *testing.T) {
	...
	carol := ctx.aliases["C"]
	const amt lnwire.MilliLoki = 4999999
	...
	expectedAmt := lnwire.MilliLoki(5010198)
...
func testProbabilityRouting(t *testing.T, useCache bool,
	paymentAmt chainutil.Amount, timePref float64,
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a1", "a2", 100000, &testChannelPolicy{
			Expiry:      144,
			FeeBaseMsat: lnwire.NewMSatFromLokis(5),
			MinHTLC:     1,
		}, 10),
		symmetricTestChannel("a2", "target", 100000, &testChannelPolicy{
			Expiry:      144,
			FeeBaseMsat: lnwire.NewMSatFromLokis(8),
			MinHTLC:     1,
		}, 11),
		symmetricTestChannel("b", "target", 100000, &testChannelPolicy{
			Expiry:      100,
			FeeBaseMsat: lnwire.NewMSatFromLokis(25),
			MinHTLC:     1,
		}, 20),
	}
	...
	ctx.restrictParams.ProbabilitySource = func(fromNode,
		toNode route.Vertex, amt lnwire.MilliLoki,
		capacity chainutil.Amount) float64 {
	...
	ctx.pathFindingConfig = PathFindingConfig{
		AttemptCost:    lnwire.NewMSatFromLokis(9),
	...
	path, err := ctx.findPath(
		target, lnwire.NewMSatFromLokis(paymentAmt),
	)
	if expectedChan == 0 {
		if err != errNoPathFound {
...
func runEqualCostRouteSelection(t *testing.T, useCache bool) {
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a", "target", 100000, &testChannelPolicy{
			Expiry:      144,
			FeeBaseMsat: lnwire.NewMSatFromLokis(6),
			MinHTLC:     1,
		}, 1),
		symmetricTestChannel("b", "target", 100000, &testChannelPolicy{
			Expiry:      100,
	...
	ctx.pathFindingConfig = PathFindingConfig{
		AttemptCost: lnwire.NewMSatFromLokis(1),
	}
...
func runInboundFees(t *testing.T, useCache bool) {
	...
	chanCapSat := chainutil.Amount(100000)
	...
	testChannels := []*testChannel{
		asymmetricTestChannel("a", "b", chanCapSat,
			&testChannelPolicy{
				MinHTLC:  lnwire.NewMSatFromLokis(2),
				Features: features,
			},
			&testChannelPolicy{
				Features:           features,
	...
	paymentAmt := lnwire.MilliLoki(100_000)
...
func (c *pathFindingTestContext) findPath(target route.Vertex,
	amt lnwire.MilliLoki) ([]*unifiedEdge,
	error) {
...
func dbFindPath(graph *graphdb.ChannelGraph,
	...
	source, target route.Vertex, amt lnwire.MilliLoki, timePref float64,
...
func TestBlindedRouteConstruction(t *testing.T) {
	...
	var (
		...
		_, alicePk       = crypto.PrivKeyFromBytes([]byte{1})
		_, bobPk         = crypto.PrivKeyFromBytes([]byte{2})
		_, carolPk       = crypto.PrivKeyFromBytes([]byte{3})
		_, daveBlindedPk = crypto.PrivKeyFromBytes([]byte{4})
		_, eveBlindedPk  = crypto.PrivKeyFromBytes([]byte{5})

		_, blindingPk = crypto.PrivKeyFromBytes([]byte{9})
	...
	totalAmount := totalAmt + bobCarolEdge.FeeBaseMSat +
		lnwire.MilliLoki(blindedPayment.BaseFee)
	...
	bobAmount := totalAmt + lnwire.MilliLoki(
...
func TestLastHopPayloadSize(t *testing.T) {
	...
	var (
		...
		amtToForward             = lnwire.MilliLoki(10000)
