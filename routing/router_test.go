...
func TestFindRoutesWithFeeLimit(t *testing.T) {
	...
	restrictions := &RestrictParams{
		FeeLimit:          lnwire.NewMLokFromLokis(10),
...
func TestSendPaymentRouteFailureFallback(t *testing.T) {
	...
	paymentAmt := lnwire.NewMLokFromLokis(1000)
...
func TestChannelUpdateValidation(t *testing.T) {
	...
	testChannels := []*testChannel{
		symmetricTestChannel("a", "b", chanCapSat, &testChannelPolicy{
			...
			FeeRate: feeRate,
			MinHTLC: 1,
			MaxHTLC: lnwire.NewMLokFromLokis(chanCapSat),
		}, 1),
...
func TestSendPaymentErrorRepeatedFeeInsufficient(t *testing.T) {
	...
	amt := lnwire.NewMLokFromLokis(1000)
	payment := createDummyLightningPayment(
...
func TestUnknownErrorSource(t *testing.T) {
	...
	payment := createDummyLightningPayment(
		t, ctx.aliases["c"], lnwire.NewMLokFromLokis(1000),
	)
...
func TestBuildRoute(t *testing.T) {
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a", "b", chanCapSat, &testChannelPolicy{
			Expiry:  144,
			FeeRate: 20000,
			MinHTLC: lnwire.NewMLokFromLokis(5),
			MaxHTLC: lnwire.NewMLokFromLokis(chanCapSat),
		}, 1),
		symmetricTestChannel("a", "b", chanCapSat/2, &testChannelPolicy{
			...
			FeeRate: 20000,
			MinHTLC: lnwire.NewMLokFromLokis(5),
			MaxHTLC: lnwire.NewMLokFromLokis(chanCapSat / 2),
		}, 6),
		...
		symmetricTestChannel("b", "c", chanCapSat, &testChannelPolicy{
			Expiry:   144,
			FeeRate:  50000,
			MinHTLC:  lnwire.NewMLokFromLokis(20),
			MaxHTLC:  lnwire.NewMLokFromLokis(120),
		...
		symmetricTestChannel("b", "z", chanCapSat, &testChannelPolicy{
			...
			FeeRate:  50000,
			MinHTLC:  lnwire.NewMLokFromLokis(20),
			MaxHTLC:  lnwire.NewMLokFromLokis(25),
			Features: paymentAddrFeatures,
		}, 3),
		symmetricTestChannel("b", "z", chanCapSat, &testChannelPolicy{
			Expiry:   144,
		...
		symmetricTestChannel("a", "d", chanCapSat, &testChannelPolicy{
			...
			FeeRate: 20000,
			MinHTLC: lnwire.NewMLokFromLokis(5),
			MaxHTLC: lnwire.NewMLokFromLokis(
				chanCapSat,
			),
	...
	amt := lnwire.NewMLokFromLokis(100)
