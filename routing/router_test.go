...
var (
	...
	priv1, _    = crypto.NewPrivateKey()
	bitcoinKey1 = priv1.PubKey()

	priv2, _    = crypto.NewPrivateKey()
...
// network.
func signErrChanUpdate(t *testing.T, key *crypto.PrivateKey,
...
func TestFindRoutesWithFeeLimit(t *testing.T) {
	...
	// loki with a fee limit of 10 loki. There are two routes from
	// roasbeef to sophon:
	...
	restrictions := &RestrictParams{
		FeeLimit:          lnwire.NewMSatFromLokis(10),
...
func TestSendPaymentRouteFailureFallback(t *testing.T) {
	...
	// to luo ji for 1000 loki, with a maximum of 1000 loki in fees.
	paymentAmt := lnwire.NewMSatFromLokis(1000)
...
func TestSendPaymentRouteInfiniteLoopWithBadHopHint(t *testing.T) {
	...
	sourceNodeID, err := crypto.ParsePubKey(source[:])
	require.NoError(t, err)
...
func TestChannelUpdateValidation(t *testing.T) {
	...
	feeRate := lnwire.MilliLoki(400)
	testChannels := []*testChannel{
		symmetricTestChannel("a", "b", chanCapSat, &testChannelPolicy{
			Expiry:  144,
			FeeRate: feeRate,
			MinHTLC: 1,
			MaxHTLC: lnwire.NewMSatFromLokis(chanCapSat),
		}, 1),
...
func TestSendPaymentErrorRepeatedFeeInsufficient(t *testing.T) {
	...
	// Craft a LightningPayment struct that'll send a payment from roasbeef
	// to sophon for 1000 loki.
	amt := lnwire.NewMSatFromLokis(1000)
	payment := createDummyLightningPayment(
...
func TestSendPaymentErrorFeeInsufficientPrivateEdge(t *testing.T) {
	...
	sgNodeID, err := crypto.ParsePubKey(sgNode[:])
	require.NoError(t, err)

	// Craft a LightningPayment struct that'll send a payment from roasbeef
	// to elst, through a private channel between songoku and elst for
	// 1000 loki. This route has lowest fees compared with the rest.
	...
	require.Equal(t,
		lnwire.MilliLoki(updatedFeeBaseMSat).String(),
...
func TestSendPaymentPrivateEdgeUpdateFeeExceedsLimit(t *testing.T) {
	...
	var (
		...
		expiryDelta      = uint16(32)
		sgNode           = ctx.aliases["songoku"]
		feeLimit         = lnwire.MilliLoki(500000)
	)
...
func TestUnknownErrorSource(t *testing.T) {
	...
	payment := createDummyLightningPayment(
		t, ctx.aliases["c"], lnwire.NewMSatFromLokis(1000),
	)
...
func TestSendToRouteStructuredError(t *testing.T) {
	...
	// in a call to SendToRoute. SendToRoute also applies channel updates,
	// but it saves us from including RequestRoute in the test scope too.
	const payAmt = lnwire.MilliLoki(10000)
	hop1 := ctx.aliases["b"]
	hop2 := ctx.aliases["c"]
	hops := []*route.Hop{
		{
...
func TestBuildRoute(t *testing.T) {
	...
	testChannels := []*testChannel{
		...
		symmetricTestChannel("a", "b", chanCapSat, &testChannelPolicy{
			Expiry:  144,
			FeeRate: 20000,
			MinHTLC: lnwire.NewMSatFromLokis(5),
			MaxHTLC: lnwire.NewMSatFromLokis(chanCapSat),
		}, 1),
		symmetricTestChannel("a", "b", chanCapSat/2, &testChannelPolicy{
			...
			FeeRate: 20000,
			MinHTLC: lnwire.NewMSatFromLokis(5),
			MaxHTLC: lnwire.NewMSatFromLokis(chanCapSat / 2),
		}, 6),
		...
		symmetricTestChannel("b", "c", chanCapSat, &testChannelPolicy{
			Expiry:   144,
			FeeRate:  50000,
			MinHTLC:  lnwire.NewMSatFromLokis(20),
			MaxHTLC:  lnwire.NewMSatFromLokis(120),
		...
		symmetricTestChannel("b", "z", chanCapSat, &testChannelPolicy{
			...
			FeeRate:  50000,
			MinHTLC:  lnwire.NewMSatFromLokis(20),
			MaxHTLC:  lnwire.NewMSatFromLokis(25),
			Features: paymentAddrFeatures,
		}, 3),
		symmetricTestChannel("b", "z", chanCapSat, &testChannelPolicy{
			Expiry:   144,
			FeeRate:  60000,
			MinHTLC:  lnwire.NewMSatFromLokis(20),
			MaxHTLC:  lnwire.MilliLoki(20100),
			Features: paymentAddrFeatures,
		}, 8),
		...
		symmetricTestChannel("a", "d", chanCapSat, &testChannelPolicy{
			...
			FeeRate: 20000,
			MinHTLC: lnwire.NewMSatFromLokis(5),
			MaxHTLC: lnwire.NewMSatFromLokis(
				chanCapSat,
			),
	...
	noAmt := fn.None[lnwire.MilliLoki]()
	...
	amt := lnwire.NewMSatFromLokis(100)
	...
	checkHops(rt, []uint64{1, 7}, payAddr)
	require.Equal(t, lnwire.MilliLoki(21200), rt.TotalAmount)

	// The receiver gets sent the minimal HTLC amount.
	require.Equal(t, lnwire.MilliLoki(20000), rt.Hops[1].AmtToForward)
...
func TestSenderAmtBackwardPass(t *testing.T) {
	...
	var (
		capacity        chainutil.Amount = 1_000_000
		testReceiverAmt lnwire.MilliLoki = 1_000_000
		minHTLC         lnwire.MilliLoki = 1_000
	)
	...
	_, senderAmount, err := senderAmtBackwardPass(
		edgeUnifiers, fn.None[lnwire.MilliLoki](), &bandwidthHints,
	)
	require.NoError(t, err)
...
func testInboundOutboundFee(t *testing.T, outgoingAmt uint64, inBase,
	...
	outgoingEdge := &unifiedEdge{
		policy: &models.CachedEdgePolicy{
			...
			FeeProportionalMillionths: lnwire.MilliLoki(
				outRate,
			),
		},
	}
	...
	incomingAmt := incomingFromOutgoing(
		lnwire.MilliLoki(outgoingAmt), incomingEdge,
		outgoingEdge,
	)
...
func TestAddEdgeUnknownVertexes(t *testing.T) {
	...
	edge := &models.ChannelEdgeInfo{
		...
		NodeKey1Bytes:      pub1,
		NodeKey2Bytes:      pub2,
		FlokicoinKey1Bytes: pub1,
		FlokicoinKey2Bytes: pub2,
	...
	connectNodeKey, err := crypto.ParsePubKey(connectNode[:])
	require.NoError(t, err)

	var (
		pubKey1 *crypto.PublicKey
		pubKey2 *crypto.PublicKey
	)
	...
	copy(edge.FlokicoinKey1Bytes[:], node1Bytes)
	edge.FlokicoinKey2Bytes = node2Bytes
...
func createDummyLightningPayment(t *testing.T,
	target route.Vertex, amt lnwire.MilliLoki) *LightningPayment {
...
func (m *mockGraphBuilder) ApplyChannelUpdate(msg *lnwire.ChannelUpdate1) bool {
	...
	err := m.updateEdge(&models.ChannelEdgePolicy{
		...
		FeeBaseMSat:               lnwire.MilliLoki(msg.BaseFee),
		FeeProportionalMillionths: lnwire.MilliLoki(msg.FeeRate),
...
func createChannelEdge(bitcoinKey1, bitcoinKey2 []byte,
	chanValue chainutil.Amount, fundingHeight uint32) (*wire.MsgTx,
...
func TestFindBlindedPathsWithMC(t *testing.T) {
	...
	probabilitySrc := func(from route.Vertex, to route.Vertex,
		amt lnwire.MilliLoki, capacity chainutil.Amount) float64 {
