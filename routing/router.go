...
type FeeSchema struct {
	// BaseFee is the base amount of milli-loki that will be chained
...
// ChannelRouter is the HtlcSwitch, and below that is the Flokicoin blockchain
...
type RouteRequest struct {
	...
	// Amount is the Amount in milliloki to be delivered to the target
	// node.
...
func NewRouteRequest(source route.Vertex, target *route.Vertex,
	amount lnwire.MilliLoki, timePref float64,
...
// given vertices.
type probabilitySource func(route.Vertex, route.Vertex, lnwire.MilliLoki,
...
func (r *ChannelRouter) FindBlindedPaths(destination route.Vertex,
	amt lnwire.MilliLoki, probabilitySrc probabilitySource,
...
// payment attempt.
func generateNewSessionKey() (*crypto.PrivateKey, error) {
...
type LightningPayment struct {
	...
	// FeeLimit is the maximum fee in milliloki that the payment should
...
func (r *ChannelRouter) sendPayment(ctx context.Context,
	feeLimit lnwire.MilliLoki, identifier lntypes.Hash,
...
func (r *ChannelRouter) BuildRoute(amt fn.Option[lnwire.MilliLoki],
	...
	*route.Route, error) {
	...
	var (
		receiverAmt lnwire.MilliLoki
		senderAmt   lnwire.MilliLoki
	...
	receiverAmt, err = fn.ElimOption(
		amt,
		func() fn.Result[lnwire.MilliLoki] {
			return fn.NewResult(
				receiverAmtForwardPass(senderAmt, pathEdges),
			)
		},
		fn.Ok[lnwire.MilliLoki],
	).Unpack()
	if err != nil {
		return nil, err
	}
...
func senderAmtBackwardPass(unifiers []*edgeUnifier,
	amt fn.Option[lnwire.MilliLoki],
	bandwidthHints bandwidthHints) ([]*unifiedEdge, lnwire.MilliLoki,
	error) {
	...
	for i := len(unifiers) - 2; i >= 0; i-- {
		...
		fee := lnwire.MilliLoki(int64(outboundFee) + inboundFee)
...
func receiverAmtForwardPass(runningAmt lnwire.MilliLoki,
	unifiedEdges []*unifiedEdge) (lnwire.MilliLoki, error) {
...
func incomingFromOutgoing(outgoingAmt lnwire.MilliLoki,
	incoming, outgoing *unifiedEdge) lnwire.MilliLoki {
	...
	return netAmount + lnwire.MilliLoki(inboundFee)
}
...
func outgoingFromIncoming(incomingAmt lnwire.MilliLoki,
	incoming, outgoing *unifiedEdge) lnwire.MilliLoki {
	...
	if lnwire.MilliLoki(testAmt.Int64()) > incomingAmt {
		...
		// MilliLoki, but this helps to ensure that we don't hit min
		...
		return lnwire.MilliLoki(ceil.Int64())
	}
