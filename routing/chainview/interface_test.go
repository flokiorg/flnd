...
import (
	...
	_ "github.com/flokiorg/walletd/walletdb/bdb" // Required to register the boltdb walletdb implementation.
...
var (
	...
	privKey, pubKey = crypto.PrivKeyFromBytes(testPrivKey)
	addrPk, _       = chainutil.NewAddressPubKey(pubKey.SerializeCompressed(),
		netParams)
...
func setupMinerAndChainView(t *testing.T, chainViewInit chainViewInitFunc) (
	*rpctest.Harness, FilteredChainView) {
	...
	// this node with a chain length of 125, so we have plenty of FLC to
	// play around with.
...
func testFilterSingleBlock(t *testing.T, chainViewInit chainViewInitFunc) {
	...
	require.NoError(t, err, "unable to create spending tx")
	txns := []*chainutil.Tx{chainutil.NewTx(spendingTx1), chainutil.NewTx(spendingTx2)}
...
var interfaceImpls = []struct {
...
}{
	{
		...
		chainViewInit: func(t *testing.T, _ rpcclient.ConnConfig,
			...
			chainView := NewFlokicoindFilteredChainView(
	...
	},
	{
		...
		chainViewInit: func(t *testing.T, _ rpcclient.ConnConfig,
			...
			// in `FlokicoindClient`'s `ntfnHandler` may be invoked to
