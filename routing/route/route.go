...
// Vertex is a simple alias for the serialization of a compressed Flokicoin
// public key.
type Vertex [VertexSize]byte

// NewVertex returns a new Vertex given a public key.
func NewVertex(pub *crypto.PublicKey) Vertex {
	var v Vertex
...
func (h *Hop) PayloadSize(nextChanID uint64) uint64 {
	...
	if h.BlindingPoint != nil {
		addRecord(
			...
			crypto.PubKeyBytesLenCompressed,
		)
	}
...
type Route struct {
	...
	// route will need to have at least this many loki, otherwise the
	...
	FirstHopAmount tlv.RecordT[
		tlv.TlvType0, tlv.BigSizeT[lnwire.MilliLoki],
	]
...
//	outgoingAmt !=0
//	-> Fee = 0
func (r *Route) HopFee(hopIndex int) lnwire.MilliLoki {
	var incomingAmt lnwire.MilliLoki
	if hopIndex == 0 {
...
func (r *Route) TotalFees() lnwire.MilliLoki {
	if len(r.Hops) == 0 {
		return 0
	}
...
func (r *Route) ReceiverAmt() lnwire.MilliLoki {
	if len(r.Hops) == 0 {
		return 0
	}
...
func NewRouteFromHops(amtToSend lnwire.MilliLoki, timeLock uint32,
...
func (r *Route) ToSphinxPath() (*sphinx.PaymentPath, error) {
	...
	for i, hop := range r.Hops {
		pub, err := crypto.ParsePubKey(hop.PubKeyBytes[:])
		if err != nil {
			return nil, err
		}
