...
var (
	...
	_, testPubKey       = crypto.PrivKeyFromBytes(testPrivKeyBytes)
...
func TestPayloadSize(t *testing.T) {
	...
	blindedHops := []*Hop{
		...
		},
		{
			...
			Metadata:         []byte{10, 11},
			EncryptedData:    []byte{12, 13},
			TotalAmtMsat:     lnwire.MilliLoki(900),
		},
	}
...
func TestBlindedHopFee(t *testing.T) {
	...
	require.Equal(t, lnwire.MilliLoki(50), route.HopFee(0))
	require.Equal(t, lnwire.MilliLoki(150), route.HopFee(1))
	require.Equal(t, lnwire.MilliLoki(300), route.HopFee(2))
	require.Equal(t, lnwire.MilliLoki(0), route.HopFee(3))
	require.Equal(t, lnwire.MilliLoki(0), route.HopFee(4))
}
