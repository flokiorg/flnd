...
func createDummyRoute(t *testing.T, amt lnwire.MilliLoki) *route.Route {
	t.Helper()
	...
	hopFee := lnwire.NewMLokFromLokis(3)
	hops := []*route.Hop{
		{
			ChannelID:     1,
			PubKeyBytes:   hop1,
...
func makeAttemptInfo(t *testing.T, amt int) channeldb.HTLCAttemptInfo {
	rt := createDummyRoute(t, lnwire.MilliLoki(amt))
...
func TestResumePaymentFailOnTimeout(t *testing.T) {
	...
	paymentAmt := lnwire.MilliLoki(10000)
