...
const (
	...
	fakeHopHintCapacity = chainutil.Amount(10 * chainutil.LokiPerFlokicoin)
)

// pathFinder defines the interface of a path finding algorithm.
type pathFinder = func(g *graphParams, r *RestrictParams,
	cfg *PathFindingConfig, self, source, target route.Vertex,
	amt lnwire.MilliLoki, timePref float64, finalHtlcExpiry int32) (
...
var (
	...
	DefaultAttemptCost = lnwire.NewMSatFromLokis(100)
...
func newRoute(sourceVertex route.Vertex,
	...
	var (
		...
		nextIncomingAmount lnwire.MilliLoki
	...
	for i := pathLength - 1; i >= 0; i-- {
		...
		var (
			amtToForward        lnwire.MilliLoki
			fee                 int64
			totalAmtMsatBlinded lnwire.MilliLoki
		...
		nextIncomingAmount = amtToForward + lnwire.MilliLoki(fee)
	}
...
func edgeWeight(lockedAmt lnwire.MilliLoki, fee lnwire.MilliLoki,
...
type RestrictParams struct {
	...
	ProbabilitySource func(route.Vertex, route.Vertex,
		lnwire.MilliLoki, chainutil.Amount) float64
...
func getOutgoingBalance(node route.Vertex, outgoingChans map[uint64]struct{},
	bandwidthHints bandwidthHints,
	g Graph) (lnwire.MilliLoki, lnwire.MilliLoki, error) {

	var max, total lnwire.MilliLoki
	cb := func(channel *graphdb.DirectedChannel) error {
		...
		if !ok {
			bandwidth = lnwire.NewMSatFromLokis(channel.Capacity)
		...
		if overflow {
			...
			// maximum value. Which is more milli-loki than are
			// in existence anyway, so the actual value is
			// irrelevant.
			total = lnwire.MilliLoki(math.MaxUint64)
		}
...
func findPath(g *graphParams, r *RestrictParams, cfg *PathFindingConfig,
	self, source, target route.Vertex, amt lnwire.MilliLoki,
	...
	defaultAttemptCost := float64(
		cfg.AttemptCost +
			amt*lnwire.MilliLoki(cfg.AttemptCostPPM)/1000000,
	)
	...
	processEdge := func(fromVertex route.Vertex,
		...
		if totalFee > 0 && lnwire.MilliLoki(totalFee) > r.FeeLimit {
			return
		}
		...
		fee := lnwire.MilliLoki(0)
		if signedFee > 0 {
			fee = lnwire.MilliLoki(signedFee)
		}
		...
		withDist := &nodeWithDist{
			...
			outboundFee:       lnwire.MilliLoki(outboundFee),
...
func lastHopPayloadSize(r *RestrictParams, finalHtlcExpiry int32,
	amount lnwire.MilliLoki) (uint64, error) {
...
// overflowSafeAdd adds two MilliLoki values and returns the result. If an
// overflow could occur, zero is returned instead and the boolean is set to
// true.
func overflowSafeAdd(x, y lnwire.MilliLoki) (lnwire.MilliLoki, bool) {
