...
func (p *AprioriEstimator) getNodeProbability(now time.Time,
	results NodeResults, amt lnwire.MilliLoki,
...
func capacityFactor(amt lnwire.MilliLoki, capacity chainutil.Amount,
	...
	capMsat := float64(lnwire.NewMLokFromLokis(capacity))
	amtMsat := float64(amt)
...
func (p *AprioriEstimator) LocalPairProbability(
	...
	return p.calculateProbability(
		now, results, nodeProbability, toNode, lnwire.MaxMilliLoki,
	)
}
