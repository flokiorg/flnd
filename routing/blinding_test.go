...
func TestBlindedPaymentToHints(t *testing.T) {
	...
	policy1 := &models.CachedEdgePolicy{
		...
		MinHTLC:       lnwire.MilliLoki(htlcMin),
		MaxHTLC:       lnwire.MilliLoki(htlcMax),
		FeeBaseMSat:   lnwire.MilliLoki(baseFee),
...
func TestBlindedPaymentDeepCopy(t *testing.T) {
	_, pkBlind1 := crypto.PrivKeyFromBytes([]byte{1})
	_, blindingPoint := crypto.PrivKeyFromBytes([]byte{2})
	_, pkBlind2 := crypto.PrivKeyFromBytes([]byte{3})
