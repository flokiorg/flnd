...
const (
	...
	DefaultBimodalScaleMsat = lnwire.MilliLoki(300_000_000)
	...
	BimodalScaleMsatMax = lnwire.MilliLoki(
...
func (p *BimodalEstimator) PairProbability(now time.Time,
	results NodeResults, toNode route.Vertex, amt lnwire.MilliLoki,
	...
	directProbability := p.directProbability(
		now, results, toNode, amt, lnwire.NewMSatFromLokis(capacity),
	)
...
func (p *BimodalEstimator) directProbability(now time.Time,
	results NodeResults, toNode route.Vertex, amt lnwire.MilliLoki,
	capacity lnwire.MilliLoki) float64 {
	...
	successAmount := lnwire.MilliLoki(0)
...
// time.
func canSend(successAmount lnwire.MilliLoki, now, successTime time.Time,
	decayConstant time.Duration) lnwire.MilliLoki {
	...
	return lnwire.MilliLoki(canSend)
}
...
// than the decay time.
func cannotSend(failAmount, capacity lnwire.MilliLoki, now,
	failTime time.Time, decayConstant time.Duration) lnwire.MilliLoki {
	...
	cannotSend := capacity - lnwire.MilliLoki(
...
func (p *BimodalEstimator) probabilityFormula(capacityMsat, successAmountMsat,
	failAmountMsat, amountMsat lnwire.MilliLoki) (float64, error) {
