...
func TestManager(t *testing.T) {
	...
	var (
		...
		chanCap            = chainutil.Amount(1000)
		chanPointMissing   = wire.OutPoint{Hash: chainhash.Hash{2}, Index: 2}
		maxPendingAmount   = lnwire.MilliLoki(999000)
		minHTLC            = lnwire.MilliLoki(2000)
	...
	selfpub, _ := crypto.ParsePubKey(sp[:])
	remotepub, _ := crypto.ParsePubKey(rp[:])
	localMultisigPrivKey, _ := crypto.NewPrivateKey()
	localMultisigKey := localMultisigPrivKey.PubKey()
	remoteMultisigPrivKey, _ := crypto.NewPrivateKey()
	...
	manager := Manager{
		...
		DefaultRoutingPolicy: models.ForwardingPolicy{
			...
			BaseFee:       lnwire.MilliLoki(1000),
			FeeRate:       lnwire.MilliLoki(0),
