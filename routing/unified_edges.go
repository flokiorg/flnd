...
func (u *nodeEdgeUnifier) addPolicy(fromNode route.Vertex,
	...
	capacity chainutil.Amount, hopPayloadSizeFn PayloadSizeFunc,
...
func newUnifiedEdge(policy *models.CachedEdgePolicy, capacity chainutil.Amount,
...
// channel.
func (u *unifiedEdge) amtInRange(amt lnwire.MilliLoki) bool {
	// If the capacity is available (non-light clients), skip channels that
	// are too small.
	if u.capacity > 0 &&
		amt > lnwire.NewMLokFromLokis(u.capacity) {
...
// channels.
func (u *edgeUnifier) getEdge(netAmtReceived lnwire.MilliLoki,
	bandwidthHints bandwidthHints,
	nextOutFee lnwire.MilliLoki) *unifiedEdge {
...
func calcCappedInboundFee(edge *unifiedEdge, amt lnwire.MilliLoki,
	nextOutFee lnwire.MilliLoki) int64 {
...
func (u *edgeUnifier) getEdgeLocal(netAmtReceived lnwire.MilliLoki,
	bandwidthHints bandwidthHints,
	nextOutFee lnwire.MilliLoki) *unifiedEdge {

	var (
		bestEdge     *unifiedEdge
		maxBandwidth lnwire.MilliLoki
	)
	...
	for _, edge := range u.edges {
		...
		// local channel.
		amt := netAmtReceived + lnwire.MilliLoki(inboundFee)
...
// forwarding context.
func (u *edgeUnifier) getEdgeNetwork(netAmtReceived lnwire.MilliLoki,
	nextOutFee lnwire.MilliLoki) *unifiedEdge {
	...
	for _, edge := range u.edges {
		...
		capMsat := lnwire.NewMLokFromLokis(edge.capacity)
	...
	modifiedEdge := newUnifiedEdge(
		&policyCopy, maxCapMsat.ToLokis(), bestPolicy.inboundFees,
...
func (u *edgeUnifier) minAmt() lnwire.MilliLoki {
	minAmount := lnwire.MaxMilliLoki
	for _, edge := range u.edges {
