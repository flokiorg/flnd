...
type BuildBlindedPathCfg struct {
	...
	// construction.
	FindRoutes func(value lnwire.MilliLoki) ([]*route.Route, error)
	...
	// ValueMsat is the payment amount in milli-loki that must be
	// routed. This will be used for selecting appropriate routes to use for
	// the blinded path.
	ValueMsat lnwire.MilliLoki
...
func collectRelayInfo(cfg *BuildBlindedPathCfg, path *candidatePath) (
	[]*hopRelayInfo, lnwire.MilliLoki, lnwire.MilliLoki, error) {
...
func buildDummyRouteData(node route.Vertex, relayInfo *record.PaymentRelayInfo,
	...
	nodeID, err := crypto.ParsePubKey(node[:])
	if err != nil {
		return nil, err
	}
...
func computeDummyHopPolicy(defaultPolicy *BlindedHopPolicy,
	...
	if numPolicies == 0 {
		...
		// MaxHTLC value.
		var maxHTLC chainutil.Amount
		for _, c := range chans {
			maxHTLC += c.Capacity
		}

		maxHTLC = chainutil.Amount(float64(maxHTLC) / float64(len(chans)))
		...
		return &BlindedHopPolicy{
			...
			MaxHTLCMsat:     lnwire.NewMSatFromLokis(maxHTLC),
		}, nil
	}
	...
	avgPolicy.MinHTLCMsat = lnwire.MilliLoki(
		float64(avgPolicy.MinHTLCMsat) / float64(numPolicies),
	)
	avgPolicy.MaxHTLCMsat = lnwire.MilliLoki(
		float64(avgPolicy.MaxHTLCMsat) / float64(numPolicies),
	)
	avgPolicy.BaseFee = lnwire.MilliLoki(
...
type BlindedHopPolicy struct {
	...
	MinHTLCMsat     lnwire.MilliLoki
	MaxHTLCMsat     lnwire.MilliLoki
}
...
func calcBlindedPathPolicies(relayInfo []*record.PaymentRelayInfo,
	ourMinFinalCLTVDelta uint16) (lnwire.MilliLoki, uint32, uint16) {

	var (
		totalFeeBase lnwire.MilliLoki
		totalFeeProp uint32
...
func calcNextTotalBaseFee(currentTotal, hopBaseFee lnwire.MilliLoki,
	hopFeeRate uint32) lnwire.MilliLoki {

	numerator := (uint32(hopBaseFee) * oneMillion) +
		(uint32(currentTotal) * (oneMillion + hopFeeRate)) +
		oneMillion - 1

	return lnwire.MilliLoki(numerator / oneMillion)
}
