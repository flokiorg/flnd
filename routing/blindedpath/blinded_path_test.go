...
func TestBlindedPathAccumulatedPolicyCalc(t *testing.T) {
	...
	require.Equal(t, lnwire.MilliLoki(201), totalBase)
	...
	sometimesDo(func() {
		data.Constraints = tlv.SomeRecordT(
			tlv.NewRecordT[tlv.TlvType12](record.PaymentConstraints{
				...
				HtlcMinimumMsat: lnwire.MilliLoki(
					rand.Uint32(),
				),
			}),
		)
...
func TestBuildBlindedPath(t *testing.T) {
	...
	var (
		privC, pkC = crypto.PrivKeyFromBytes([]byte{1})
		privB, pkB = crypto.PrivKeyFromBytes([]byte{2})
		privA, pkA = crypto.PrivKeyFromBytes([]byte{3})
	...
	paths, err := BuildBlindedPaymentPaths(&BuildBlindedPathCfg{
		FindRoutes: func(_ lnwire.MilliLoki) ([]*route.Route,
			error) {
		...
		AddPolicyBuffer: func(_ *BlindedHopPolicy) (
			*BlindedHopPolicy, error) {
			...
			return &BlindedHopPolicy{
				...
				CLTVExpiryDelta: 144,
				MinHTLCMsat:     1000,
				MaxHTLCMsat:     lnwire.MaxMilliLoki,
			}, nil
		},
		PathID:                  []byte{1, 2, 3},
		ValueMsat:               1000,
	...
	require.EqualValues(t, lnwire.MaxMilliLoki, path.HTLCMaxMsat)
...
func TestSingleHopBlindedPath(t *testing.T) {
	var (
		_, pkC = crypto.PrivKeyFromBytes([]byte{1})
		carol  = route.NewVertex(pkC)
	)
...
func decryptAndDecodeHopData(t *testing.T, priv *crypto.PrivateKey,
	ephem *crypto.PublicKey, cipherText []byte) (*record.BlindedRouteData,
	*crypto.PublicKey) {
