...
const (
	smallAmount = lnwire.MilliLoki(400_000_000)
	largeAmount = lnwire.MilliLoki(5_000_000_000)
	capacity    = lnwire.MilliLoki(10_000_000_000)
	scale       = lnwire.MilliLoki(400_000_000)
...
func TestSuccessProbability(t *testing.T) {
	...
	tests := []struct {
		...
		successAmount       lnwire.MilliLoki
		failAmount          lnwire.MilliLoki
...
func TestSmallScale(t *testing.T) {
	var (
		...
		// distribution.
		scale    lnwire.MilliLoki = 1
		capacity lnwire.MilliLoki = 7e+09
		...
		successAmount lnwire.MilliLoki = 1.0e+09
		failAmount    lnwire.MilliLoki = 4.0e+09
	)
...
func TestIntegral(t *testing.T) {
	...
	defaultScale := lnwire.NewMSatFromLokis(300_000)
...
func TestCanSend(t *testing.T) {
	...
	successAmount := lnwire.MilliLoki(1_000_000)
	...
	// value.
	decayAmount := lnwire.MilliLoki(float64(successAmount) / math.E)
	...
	require.Equal(t, lnwire.MilliLoki(0), canSend(
...
func TestCannotSend(t *testing.T) {
	...
	failAmount := lnwire.MilliLoki(1_000_000)
	failTime := time.Unix(1_000, 0)
	now := time.Unix(2_000, 0)
	decayTime := time.Duration(1_000) * time.Second
	infinity := time.Unix(10_000_000_000, 0)
	capacity := lnwire.MilliLoki(3_000_000)
	...
	// the capacity.
	summand := lnwire.MilliLoki(float64(capacity-failAmount) / math.E)
...
func FuzzProbability(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, capacity, successAmt, failAmt, amt uint64) {
		...
		_, err := estimator.probabilityFormula(
			lnwire.MilliLoki(capacity),
			lnwire.MilliLoki(successAmt),
			lnwire.MilliLoki(failAmt), lnwire.MilliLoki(amt),
		)
