...
const (
	...
	testCapacity         = chainutil.Amount(100_000)
	testAmount           = lnwire.MilliLoki(90_000_000)
...
func (c *estimatorTestContext) assertPairProbability(now time.Time,
	toNode byte, amt lnwire.MilliLoki, capacity chainutil.Amount,
...
func TestProbabilityEstimatorMix(t *testing.T) {
	...
	ctx.results = map[int]TimedPairResult{
		node1: {
			SuccessAmt: lnwire.MilliLoki(1000),
		},
		node2: {
...
func TestCapacityCutoff(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			got := capacityFactor(
				lnwire.MilliLoki(test.amountMsat),
				chainutil.Amount(capacitySat),
