...
func (s *BlindedPaymentPathSet) TargetPubKey() *crypto.PublicKey {
...
func (b *BlindedPayment) toRouteHints() (RouteHints, error) {
	...
	edgePolicy := &models.CachedEdgePolicy{
		...
		MinHTLC:       lnwire.MilliLoki(b.HtlcMinimum),
		MaxHTLC:       lnwire.MilliLoki(b.HtlcMaximum),
		FeeBaseMSat:   lnwire.MilliLoki(b.BaseFee),
...
func copyPublicKey(pk *crypto.PublicKey) *crypto.PublicKey {
	var result secp256k1.JacobianPoint
	pk.AsJacobian(&result)
	result.ToAffine()

	return crypto.NewPublicKey(&result.X, &result.Y)
}
