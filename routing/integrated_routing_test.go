...
func twoPathGraph(g *mockGraph, capacityOut, capacityIn chainutil.Amount) {
	// Create the following network of nodes:
	// source -> intermediate1 -> target
...
var mppTestCases = []mppSendTestCase{
	...
	// payment of 30k loki with a max shard size of 10k loki should
...
func TestBadFirstHopHint(t *testing.T) {
	...
	sourcePubKey, _ := crypto.ParsePubKey(ctx.source.pubkey[:])
	...
	ctx.amt = lnwire.NewMSatFromLokis(100)
...
func testMppSend(t *testing.T, testCase *mppSendTestCase) {
	...
	ctx.amt = lnwire.NewMSatFromLokis(testCase.amt)

	if testCase.maxShardSize != 0 {
		shardAmt := lnwire.NewMSatFromLokis(testCase.maxShardSize)
...
func TestPaymentAddrOnlyNoSplit(t *testing.T) {
	...
	// We'll make a payment of 1.5 mil loki our single chan sizes,
	...
	// ignored.
	const maxParts = 5
	ctx.amt = lnwire.NewMSatFromLokis(1_500_000)
