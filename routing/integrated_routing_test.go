...
func TestBadFirstHopHint(t *testing.T) {
	...
	ctx.amt = lnwire.NewMLokFromLokis(100)
...
func testMppSend(t *testing.T, testCase *mppSendTestCase) {
	...
	ctx.amt = lnwire.NewMLokFromLokis(testCase.amt)

	if testCase.maxShardSize != 0 {
		shardAmt := lnwire.NewMLokFromLokis(testCase.maxShardSize)
...
func TestPaymentAddrOnlyNoSplit(t *testing.T) {
	...
	// ignored.
	const maxParts = 5
	ctx.amt = lnwire.NewMLokFromLokis(1_500_000)
