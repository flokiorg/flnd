...
func TestNodeEdgeUnifier(t *testing.T) {
	...
	c1 := chainutil.Amount(7)
	c2 := chainutil.Amount(8)
	...
	tests := []struct {
		...
		expectedFeeBase    lnwire.MilliLoki
		expectedFeeRate    lnwire.MilliLoki
		expectedInboundFee models.InboundFee
		expectedTimeLock   uint16
		expectNoPolicy     bool
		expectedCapacity   chainutil.Amount
		nextOutFee         lnwire.MilliLoki
	}{
		{
		...
		// maxHTLC value.
		{
			...
			expectedFeeRate:  p2.FeeProportionalMillionths,
			expectedTimeLock: p1.TimeLockDelta,
			expectedCapacity: p1.MaxHTLC.ToLokis(),
		},
		{
			name:             "no info",
