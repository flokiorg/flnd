...
type BlindedPaymentPath struct {
	// FeeBaseMsat is the total base fee for the path in milli-loki.
	FeeBaseMsat uint32
	...
	// HTLCMinMsat is the minimum number of milli-loki that any hop in
	// the path will route.
	HTLCMinMsat uint64

	// HTLCMaxMsat is the maximum number of milli-loki that a hop in the
	// path will route.
	HTLCMaxMsat uint64
	...
	FirstEphemeralBlindingPoint *crypto.PublicKey
...
func DecodeBlindedPayment(r io.Reader) (*BlindedPaymentPath, error) {
	...
	var blindingPointBytes [crypto.PubKeyBytesLenCompressed]byte
	_, err = r.Read(blindingPointBytes[:])
	if err != nil {
		return nil, err
	}

	blinding, err := crypto.ParsePubKey(blindingPointBytes[:])
	if err != nil {
		return nil, err
	}
...
func DecodeBlindedHop(r io.Reader) (*sphinx.BlindedHopInfo, error) {
	var nodeIDBytes [crypto.PubKeyBytesLenCompressed]byte
	_, err := r.Read(nodeIDBytes[:])
	if err != nil {
		return nil, err
	}

	nodeID, err := crypto.ParsePubKey(nodeIDBytes[:])
	if err != nil {
		return nil, err
	}
