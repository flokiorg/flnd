...
func parseDestination(data []byte) (*crypto.PublicKey, error) {
	...
	return crypto.ParsePubKey(base256Data)
}
...
// on-chain address.
func parseFallbackAddr(data []byte, net *chaincfg.Params) (chainutil.Address, error) { // nolint:dupl
	...
	case fallbackVersionWitness:
		...
		switch len(witness) {
		case 20:
			addr, err = chainutil.NewAddressWitnessPubKeyHash(witness, net)
		case 32:
			addr, err = chainutil.NewAddressWitnessScriptHash(witness, net)
		default:
	...
	case fallbackVersionTaproot:
		...
		addr, err = chainutil.NewAddressTaproot(witness, net)
		if err != nil {
			return nil, err
		}
	case fallbackVersionPubkeyHash:
		...
		addr, err = chainutil.NewAddressPubKeyHash(pubKeyHash, net)
		if err != nil {
			return nil, err
		}
	case fallbackVersionScriptHash:
		...
		addr, err = chainutil.NewAddressScriptHashFromHash(scriptHash, net)
		if err != nil {
			return nil, err
		}
	default:
...
func parseRouteHint(data []byte) ([]HopHint, error) {
	...
	for len(base256Data) > 0 {
		hopHint := HopHint{}
		hopHint.NodeID, err = crypto.ParsePubKey(base256Data[:33])
		if err != nil {
			return nil, err
		}
