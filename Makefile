...
#? build: Build flnd and flncli binaries, place them in project directory
build:
	@$(call print, "Building debug flnd and flncli.")
	$(GOBUILD) -tags="$(DEV_TAGS)" -o flnd-debug $(DEV_GCFLAGS) $(DEV_LDFLAGS) $(PKG)/cmd/flnd
	$(GOBUILD) -tags="$(DEV_TAGS)" -o flncli-debug $(DEV_GCFLAGS) $(DEV_LDFLAGS) $(PKG)/cmd/flncli
...
#? install-binaries: Build and install flnd and flncli binaries, place them in $GOPATH/bin
install-binaries:
	@$(call print, "Installing flnd and flncli.")
	$(GOINSTALL) -tags="${tags}" -ldflags="$(RELEASE_LDFLAGS)" $(PKG)/cmd/flnd
	$(GOINSTALL) -tags="${tags}" -ldflags="$(RELEASE_LDFLAGS)" $(PKG)/cmd/flncli
...
#? release-install: Build and install flnd and flncli release binaries, place them in $GOPATH/bin
release-install:
	@$(call print, "Installing release flnd and flncli.")
	env CGO_ENABLED=0 $(GOINSTALL) -v -trimpath -ldflags="$(RELEASE_LDFLAGS)" -tags="$(RELEASE_TAGS)" $(PKG)/cmd/flnd
	env CGO_ENABLED=0 $(GOINSTALL) -v -trimpath -ldflags="$(RELEASE_LDFLAGS)" -tags="$(RELEASE_TAGS)" $(PKG)/cmd/flncli

#? cross-release-install: Build flnd and flncli release binaries for single/all supported platforms to /tmp (useful for checking cross compilation or priming release build cache).
cross-release-install:
	@$(call print, "Cross compiling release flnd and flncli.")
	for sys in $(BUILD_SYSTEM); do \
		...
		$(GOBUILD) -trimpath -ldflags="$(RELEASE_LDFLAGS)" -tags="$(RELEASE_TAGS)" -o /tmp/flnd-$$sys $(PKG)/cmd/flnd; \
		$(GOBUILD) -trimpath -ldflags="$(RELEASE_LDFLAGS)" -tags="$(RELEASE_TAGS)" -o /tmp/flncli-$$sys $(PKG)/cmd/flncli; \
		echo; \
	done
...
clean:
	...
	$(RM) ./flnd-debug ./flncli-debug
	$(RM) ./flnd-itest ./flncli-itest
