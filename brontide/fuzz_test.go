...
var (
	...
	initEphemeral = EphemeralGenerator(func() (*crypto.PrivateKey, error) {
		...
		priv, _ := crypto.PrivKeyFromBytes(eBytes)

		return priv, nil
	})

	// Returns the responder's ephemeral private key.
	respEphemeral = EphemeralGenerator(func() (*crypto.PrivateKey, error) {
...
func newInsecurePrivateKey(t *testing.T,
	insecureRNG io.Reader) *crypto.PrivateKey {
...
func getBrontideMachines(t *testing.T, seed int64) (*Machine, *Machine) {
	...
	ephGen := EphemeralGenerator(func() (*crypto.PrivateKey, error) {
...
func getStaticBrontideMachines() (*Machine, *Machine) {
	initPriv, _ := crypto.PrivKeyFromBytes(initBytes)
	respPriv, respPub := crypto.PrivKeyFromBytes(respBytes)
