...
func makeListener() (*Listener, *lnwire.NetAddress, error) {
	...
	localPriv, err := crypto.NewPrivateKey()
	if err != nil {
		return nil, nil, err
	}
...
func establishTestConnection(t testing.TB) (net.Conn, net.Conn, error) {
	...
	// within our test.
	remotePriv, err := crypto.NewPrivateKey()
	if err != nil {
		return nil, nil, err
	}
...
func TestBolt0008TestVectors(t *testing.T) {
	...
	initiatorPriv, _ := crypto.PrivKeyFromBytes(
		initiatorKeyBytes,
	)
	...
	require.NoError(t, err, "unable to decode hex")
	responderPriv, responderPub := crypto.PrivKeyFromBytes(
		responderKeyBytes,
	)
	...
	initiatorEphemeral := EphemeralGenerator(func() (*crypto.PrivateKey, error) {
	...
	responderEphemeral := EphemeralGenerator(func() (*crypto.PrivateKey, error) {
