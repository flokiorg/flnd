...
func getInfinityTweak() KeyTweakDesc {
	...
	keySet := []*crypto.PublicKey{generator}
...
func TestMuSig2KeyAggTestVectors(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			explicitKeys: []*secp256k1.PublicKey{crypto.Generator()},
			tweaks: []KeyTweakDesc{
				getInfinityTweak(),
			},
...
func parseKey(xHex string) *crypto.PublicKey {
	xB, err := hex.DecodeString(xHex)
	if err != nil {
		panic(err)
	}

	var x, y crypto.FieldVal
	x.SetByteSlice(xB)
	if !crypto.DecompressY(&x, false, &y) {
		panic("x not on curve")
	}
	y.Normalize()

	return crypto.NewPublicKey(&x, &y)
}

var (
	signSetPrivKey, _ = crypto.PrivKeyFromBytes(
...
func TestMuSig2SigningTestVectors(t *testing.T) {
	...
	for _, testCase := range testCases {
		...
		t.Run(testName, func(t *testing.T) {
			keySet := make([]*crypto.PublicKey, 0, len(testCase.keyOrder))
			for _, keyIndex := range testCase.keyOrder {
...
func aggNonceToPubkey(combinedNonce [66]byte, combinedKey *AggregateKey,
	msg [32]byte) (*crypto.PublicKey, error) {
	...
	var nonce, r1J, r2J crypto.JacobianPoint
	r1.AsJacobian(&r1J)
	r2.AsJacobian(&r2J)
...
func getGBytes() []byte {
	return crypto.Generator().SerializeCompressed()
}
