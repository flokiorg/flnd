...
// EPubKey is an Encoder for *crypto.PublicKey values. An error is returned if
// val is not a **crypto.PublicKey.
func EPubKey(w io.Writer, val interface{}, _ *[8]byte) error {
	if pk, ok := val.(**crypto.PublicKey); ok {
		_, err := w.Write((*pk).SerializeCompressed())
		return err
	}
	return NewTypeForEncodingErr(val, "*crypto.PublicKey")
}

// DPubKey is a Decoder for *crypto.PublicKey values. An error is returned if val
// is not a **crypto.PublicKey.
func DPubKey(r io.Reader, val interface{}, _ *[8]byte, l uint64) error {
	if pk, ok := val.(**crypto.PublicKey); ok && l == 33 {
		var b [33]byte
		_, err := io.ReadFull(r, b[:])
		if err != nil {
			return err
	...
	return NewTypeForDecodingErr(val, "*crypto.PublicKey", l, 33)
}
