...
// mergeMilliLoki merges two msat values, failing if they have different
// non-zero values.
func mergeMilliLoki(name string, current,
	newValue lnwire.MilliLoki) (lnwire.MilliLoki, error) {
...
func mergeResponse(current,
	...
	current.Reserve = chainutil.Amount(reserve)

	current.MinHtlcIn, err = mergeMilliLoki(
	...
	current.InFlightTotal, err = mergeMilliLoki(
