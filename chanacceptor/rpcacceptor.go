	...
	for {
		...
		case resp := <-responses:
			...
			requestInfo.response <- NewChannelAcceptResponse(
				...
				chainutil.Amount(resp.ReserveSat),
				lnwire.MilliLoki(resp.InFlightMaxMsat),
				lnwire.MilliLoki(resp.MinHtlcIn),
				resp.ZeroConf,
			)
...
func (r *RPCAcceptor) validateAcceptorResponse(dustLimit chainutil.Amount,
	...
	// required by BOLT 2.
	reserveSat := chainutil.Amount(req.ReserveSat)
