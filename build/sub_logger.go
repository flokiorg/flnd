...
type SubLogCreator interface {
	...
	Logger(subsystemTag string) flog.Logger
}
...
func NewSubLoggerManager(handlers ...flog.Handler) *SubLoggerManager {
	return &SubLoggerManager{
		loggers: make(SubLoggers),
		genLogger: newSubLogGenerator(
			newHandlerSet(flog.LevelInfo, handlers...),
		),
	}
}
...
func (r *SubLoggerManager) setLogLevelUnsafe(subsystemID string,
	...
	level, _ := flog.LevelFromString(logLevel)
...
// subsystem name.
type SubLoggers map[string]flog.Logger
