...
func (cfg *LoggerConfig) HandlerOptions() []flog.HandlerOption {
	opts := []flog.HandlerOption{
		...
		flog.WithCallSiteSkipDepth(flog.DefaultSkipDepth + 1),
	}

	if cfg.NoTimestamps {
		opts = append(opts, flog.WithNoTimestamp())
	}

	switch cfg.CallSite {
	case callSiteShort:
		opts = append(opts, flog.WithCallerFlags(flog.Lshortfile))
	case callSiteLong:
		opts = append(opts, flog.WithCallerFlags(flog.Llongfile))
	}
