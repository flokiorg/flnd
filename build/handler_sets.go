...
func newHandlerSet(level flogv1.Level, set ...flog.Handler) *handlerSet {
	h := &handlerSet{
		set:   set,
		level: level,
	}
...
func (h *handlerSet) SubSystem(tag string) flog.Handler {
	newSet := &handlerSet{set: make([]flog.Handler, len(h.set))}
	for i, handler := range h.set {
...
// level.
//
// NOTE: this is part of the flog.Handler interface.
func (h *handlerSet) SetLevel(level flogv1.Level) {
	for _, handler := range h.set {
...
//
// NOTE: this is part of the flog.Handler interface.
func (h *handlerSet) Level() flogv1.Level {
	return h.level
}
...
//
// NOTE: this is part of the flog.Handler interface.
func (h *handlerSet) WithPrefix(prefix string) flog.Handler {
	newSet := &handlerSet{set: make([]flog.Handler, len(h.set))}
	for i, handler := range h.set {
...
// A compile-time check to ensure that handlerSet implements flog.Handler.
var _ flog.Handler = (*handlerSet)(nil)
...
// produce slog.Handlers and not flog.Handlers. So the reducedSet represents
...
type subLogGenerator struct {
	handler flog.Handler
}

// newSubLogGenerator constructs a new subLogGenerator from a Handler.
func newSubLogGenerator(handler flog.Handler) *subLogGenerator {
...
func (b *subLogGenerator) Logger(subsystemTag string) flog.Logger {
	handler := b.handler.SubSystem(subsystemTag)

	return flog.NewSLogger(handler)
}
