...
func assertInboundConnection(t *testing.T, a *accessMan,
	remotePub *crypto.PublicKey, status peerAccessStatus) {
...
func assertAccessState(t *testing.T, a *accessMan, remotePub *crypto.PublicKey,
...
func TestAccessManRestrictedSlots(t *testing.T) {
	...
	disconnect := func(*crypto.PublicKey) (bool, error) {
		return false, nil
	}
	...
	peerPriv4, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	...
	peerPriv5, err := crypto.NewPrivateKey()
	require.NoError(t, err)
...
func TestAssignPeerPerms(t *testing.T) {
	...
	genPeerPub := func() *crypto.PublicKey {
		peerPriv, err := crypto.NewPrivateKey()
		require.NoError(t, err)

		return peerPriv.PubKey()
	}

	disconnect := func(_ *crypto.PublicKey) (bool, error) {
		return true, nil
	}

	noDisconnect := func(_ *crypto.PublicKey) (bool, error) {
		return false, nil
	}
