...
type HTLCAttemptInfo struct {
	...
	// EC operations used by crypto.PrivKeyFromBytes.
	sessionKey [crypto.PrivKeyBytesLen]byte
	...
	cachedSessionKey *crypto.PrivateKey
...
func NewHtlcAttempt(attemptID uint64, sessionKey *crypto.PrivateKey,
	route route.Route, attemptTime time.Time,
	hash *lntypes.Hash) (*HTLCAttempt, error) {

	var scratch [crypto.PrivKeyBytesLen]byte
...
func (h *HTLCAttemptInfo) SessionKey() *crypto.PrivateKey {
	if h.cachedSessionKey == nil {
		h.cachedSessionKey, _ = crypto.PrivKeyFromBytes(
			h.sessionKey[:],
		)
	}
...
//
//nolint:unused
func (h *HTLCAttemptInfo) setSessionKey(sessionKey *crypto.PrivateKey) {
...
func (m *MPPayment) SentAmt() (lnwire.MilliLoki, lnwire.MilliLoki) {
	var sent, fees lnwire.MilliLoki
	for _, h := range m.HTLCs {
		if h.Failure != nil {
			continue
		}
...
func generateSphinxPacket(rt *route.Route, paymentHash []byte,
	sessionKey *crypto.PrivateKey) ([]byte, *sphinx.Circuit, error) {
