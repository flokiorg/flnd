	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			info := &PaymentCreationInfo{
				Value: lnwire.MilliLoki(tc.totalAmt),
			}
...
func makeAttemptInfo(total, amtForwarded int) HTLCAttemptInfo {
	hop := &route.Hop{AmtToForward: lnwire.MilliLoki(amtForwarded)}
	return HTLCAttemptInfo{
		Route: route.Route{
			TotalAmount: lnwire.MilliLoki(total),
			Hops:        []*route.Hop{hop},
		},
	}
}
...
func makePayment(total lnwire.MilliLoki,
...
func TestEmptyRoutesGenerateSphinxPacket(t *testing.T) {
	...
	sessionKey, _ := crypto.NewPrivateKey()
