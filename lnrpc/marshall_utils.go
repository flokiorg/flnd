...
// CalculateFeeLimit returns the fee limit in MilliLokis. If a percentage
...
func CalculateFeeLimit(feeLimit *FeeLimit,
	...
	case *FeeLimit_Fixed:
		return lnwire.NewMLokFromLokis(
			chainutil.Amount(feeLimit.GetFixed()),
		)

	case *FeeLimit_FixedMsat:
		return lnwire.MilliLoki(feeLimit.GetFixedMsat())

	case *FeeLimit_Percent:
		return amount * lnwire.MilliLoki(feeLimit.GetPercent()) / 100
...
func UnmarshallAmt(amtSat, amtMsat int64) (lnwire.MilliLoki, error) {
	...
	if amtSat != 0 {
		return lnwire.NewMLokFromLokis(chainutil.Amount(amtSat)), nil
	}

	return lnwire.MilliLoki(amtMsat), nil
}
