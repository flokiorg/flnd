...
func TestIsLsp(t *testing.T) {
	...
	carolPrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	carolPubKey := carolPrivKey.PubKey()

	davePrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	davePubKey := davePrivKey.PubKey()

	evePrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
...
func TestPrepareLspRouteHints(t *testing.T) {
	...
	amt := lnwire.MilliLoki(1_000_000)
...
func TestProbePaymentRequestUsesUniqueHashPerLSP(t *testing.T) {
	...
	destPrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	...
	lspHint := func(pubKey *crypto.PublicKey, chanID uint64,
	...
	invoice, err := zpay32.NewInvoice(
		...
		zpay32.Amount(lnwire.MilliLoki(100_000)),
