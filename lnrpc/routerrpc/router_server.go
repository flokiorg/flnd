...
func (s *Server) probeDestination(dest []byte, amtSat int64) (*RouteFeeResponse,
	...
	// native unit of LN.
	amtMsat := lnwire.NewMLokFromLokis(chainutil.Amount(amtSat))
...
func prepareLspRouteHints(routeHints [][]zpay32.HopHint,
	amt lnwire.MilliLoki) ([][]zpay32.HopHint, *zpay32.HopHint, error) {
...
func maxLspFee(routeHints [][]zpay32.HopHint, amt lnwire.MilliLoki) (uint32,
	uint32) {

	var maxFeePpm uint32
	var maxBaseFee uint32
	var maxTotalFee lnwire.MilliLoki
...
func (s *Server) SetMissionControlConfig(ctx context.Context,
	...
	case MissionControlConfig_BIMODAL:
		...
		bimodalConfig := routing.BimodalConfig{
			...
			BimodalScaleMsat:  lnwire.MilliLoki(bCfg.ScaleMsat),
...
func toRPCPairData(data *routing.TimedPairResult) *PairData {
	rpcData := PairData{
		FailAmtSat:     int64(data.FailAmt.ToLokis()),
		FailAmtMsat:    int64(data.FailAmt),
		SuccessAmtSat:  int64(data.SuccessAmt.ToLokis()),
...
func toPairSnapshot(pairResult *PairHistory) (*routing.MissionControlPairSnapshot,
	...
	failAmt, failTime, err := getPair(
		lnwire.MilliLoki(pairResult.History.FailAmtMsat),
		chainutil.Amount(pairResult.History.FailAmtSat),
	...
	successAmt, successTime, err := getPair(
		lnwire.MilliLoki(pairResult.History.SuccessAmtMsat),
		chainutil.Amount(pairResult.History.SuccessAmtSat),
...
func getPair(amtMsat lnwire.MilliLoki, amtSat chainutil.Amount,
	timestamp int64, isFailure bool) (lnwire.MilliLoki, time.Time,
	error) {
...
func getMsatPairValue(msatValue lnwire.MilliLoki,
	...
	if msatValue == 0 {
		return lnwire.MilliLoki(satValue * 1000), nil
	}
...
func (s *Server) BuildRoute(_ context.Context,
	...
	var amt fn.Option[lnwire.MilliLoki]
	if req.AmtMsat != 0 {
		rpcAmt := lnwire.MilliLoki(req.AmtMsat)
		amt = fn.Some(rpcAmt)
	}
