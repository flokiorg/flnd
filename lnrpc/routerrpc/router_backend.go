...
type RouterBackend struct {
	...
	FetchChannelCapacity func(chanID uint64) (chainutil.Amount, error)
...
func (r *RouterBackend) QueryRoutes(ctx context.Context,
	...
	// can carry `in.Amt` loki _including_ the total fee required on
	// the route
...
func unmarshalBlindedPaymentPaths(rpcPath *lnrpc.BlindedPath) (
	*sphinx.BlindedPath, error) {
	...
	introduction, err := crypto.ParsePubKey(rpcPath.IntroductionNode)
	if err != nil {
		return nil, err
	}

	blinding, err := crypto.ParsePubKey(rpcPath.BlindingPoint)
	if err != nil {
		return nil, err
	}
...
func unmarshalBlindedHop(rpcHop *lnrpc.BlindedHop) (*sphinx.BlindedHopInfo,
	...
	pubkey, err := crypto.ParsePubKey(rpcHop.BlindedNode)
	if err != nil {
		return nil, err
	}
...
func (r *RouterBackend) MarshallRoute(route *route.Route) (*lnrpc.Route, error) {
	resp := &lnrpc.Route{
		...
		TotalFees:          int64(route.TotalFees().ToLokis()),
		TotalFeesMsat:      int64(route.TotalFees()),
		TotalAmt:           int64(route.TotalAmount.ToLokis()),
	...
	for i, hop := range route.Hops {
		...
		if err != nil {
			...
			chanCapacity = incomingAmt.ToLokis()
		}
		...
		resp.Hops[i] = &lnrpc.Hop{
			...
			AmtToForward:     int64(hop.AmtToForward.ToLokis()),
			AmtToForwardMsat: int64(hop.AmtToForward),
			Fee:              int64(fee.ToLokis()),
			FeeMsat:          int64(fee),
...
func UnmarshallHopWithPubkey(rpcHop *lnrpc.Hop, pubkey route.Vertex) (*route.Hop,
	...
	hop := &route.Hop{
		...
		AmtToForward:     lnwire.MilliLoki(rpcHop.AmtToForwardMsat),
		PubKeyBytes:      pubkey,
		...
		TotalAmtMsat:     lnwire.MilliLoki(rpcHop.TotalAmtMsat),
	}

	haveBlindingPoint := len(rpcHop.BlindingPoint) != 0
	if haveBlindingPoint {
		hop.BlindingPoint, err = crypto.ParsePubKey(
...
func (r *RouterBackend) UnmarshallRoute(rpcroute *lnrpc.Route) (
	*route.Route, error) {
	...
	route, err := route.NewRouteFromHops(
		lnwire.MilliLoki(rpcroute.TotalAmtMsat),
...
func (r *RouterBackend) extractIntentFromSendRequest(
	...
	if rpcPayReq.MaxShardSizeMsat > 0 {
		shardAmtMsat := lnwire.MilliLoki(rpcPayReq.MaxShardSizeMsat)
	...
	if payIntent.MaxShardAmt != nil {
		maxPossibleAmount := (*payIntent.MaxShardAmt) *
			lnwire.MilliLoki(payIntent.MaxParts)
...
func unmarshallHopHint(rpcHint *lnrpc.HopHint) (zpay32.HopHint, error) {
	...
	pubkey, err := crypto.ParsePubKey(pubBytes)
	if err != nil {
		return zpay32.HopHint{}, err
	}
...
func UnmarshalMPP(reqMPP *lnrpc.MPPRecord) (*record.MPP, error) {
	...
	total := lnwire.MilliLoki(reqTotal)
...
func (r *RouterBackend) MarshallPayment(payment *paymentsdb.MPPayment) (
	*lnrpc.Payment, error) {
	...
	satValue := int64(payment.Info.Value.ToLokis())
