...
// SendOutputs is similar to the existing sendmany call in Flokicoind, and allows
...
func (w *WalletKit) prepareSweepParams(in *BumpFeeRequest,
	...
	if !ok {
		...
		params := sweep.Params{
			...
			Budget:          chainutil.Amount(in.Budget),
		}
...
func (w *WalletKit) fundPsbtCoinSelect(account string, changeIndex int32,
	...
	var (
		inputSum, outputSum chainutil.Amount
	...
	var changeDustLimit chainutil.Amount
	switch changeType {
...
func (w *WalletKit) handleChange(packet *psbt.Packet, changeIndex int32,
	...
	// Assets).
	pOut := psbt.POutput{}
	_, isTaprootChangeAddr := changeAddr.(*chainutil.AddressTaproot)
...
func (w *WalletKit) SignMessageWithAddr(_ context.Context,
	...
	addr, err := chainutil.DecodeAddress(req.Addr, w.cfg.ChainParams)
	if err != nil {
	...
	// Flokicoin signatures are base64 encoded (being compatible with
...
func (w *WalletKit) VerifyMessageWithAddr(_ context.Context,
	...
	case *chainutil.AddressScriptHash:
		...
		address, err = chainutil.NewAddressScriptHashFromHash(
			chainutil.Hash160(witnessScript), w.cfg.ChainParams,
		)
		if err != nil {
			return nil, err
		}
	...
	case *chainutil.AddressTaproot:
		...
		address, err = chainutil.NewAddressTaproot(
...
func (w *WalletKit) ImportPublicKey(_ context.Context,
	...
	default:
		pubKey, err = crypto.ParsePubKey(req.PublicKey)
	}
	if err != nil {
		return nil, err
	}
