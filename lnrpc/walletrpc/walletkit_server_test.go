...
func TestFundPsbtCoinSelect(t *testing.T) {
	...
	calcFee := func(p2trIn, p2wkhIn, p2trOut, p2wkhOut int,
		dust chainutil.Amount) chainutil.Amount {
	...
	testCases := []struct {
		...
		expectedChangeOutputAmount chainutil.Amount

		// expectedFee is the total amount of fees paid by the funded
		// packet in bytes.
		expectedFee chainutil.Amount
		...
		expectedContainedErrStr: "fee 0.00000111 FLC exceeds max fee " +
			"(0.00000027 FLC) on total output value",
	}, {
