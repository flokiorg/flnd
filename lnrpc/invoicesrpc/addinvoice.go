...
type AddInvoiceConfig struct {
	...
	QueryBlindedRoutes func(lnwire.MilliLoki) ([]*route.Route, error)
}
...
type AddInvoiceData struct {
	...
	// The value of this invoice in milliloki.
...
func AddInvoice(ctx context.Context, cfg *AddInvoiceConfig,
	...
	// We set the max invoice amount to 100k FLC, which itself is several
	// multiples off the current block reward.
	maxInvoiceAmt := chainutil.Amount(chainutil.LokiPerFlokicoin * 100000)
	...
	case invoice.Value.ToLokis() > maxInvoiceAmt:
		return nil, nil, fmt.Errorf("invoice amount %v is "+
			"too large, max is %v", invoice.Value.ToLokis(),
			maxInvoiceAmt)
	}
	...
	// allow the payer to specify the amount of loki they wish to send.
	if amtMSat > 0 {
	...
	payReqString, err := payReq.Encode(zpay32.MessageSigner{
		SignCompact: func(msg []byte) ([]byte, error) {
			...
			// the recipient.
			ephemKey, err := crypto.NewPrivateKey()
			if err != nil {
				return nil, err
			}
...
func sufficientHints(nHintsLeft int, currentAmount,
	targetAmount lnwire.MilliLoki) bool {
...
func PopulateHopHints(cfg *SelectHopHintsCfg, amtMSat lnwire.MilliLoki,
