...
func testOnchainFundRecovery(ht *lntest.HarnessTest) {
	...
	skipAndSend := func(nskip int) func(*node.HarnessNode) {
		return func(node *node.HarnessNode) {
			...
			// Send one FLC to the next P2WKH address.
			ht.FundCoins(chainutil.LokiPerFlokicoin, node)

			// And another to the next NP2WKH address.
			ht.FundCoinsNP2WKH(chainutil.LokiPerFlokicoin, node)

			// Add another whole coin to the P2TR address.
			ht.FundCoinsP2TR(chainutil.LokiPerFlokicoin, node)
		}
	}
	...
	//
	// After, one FLC is sent to both her first external P2WKH and NP2WKH
	// addresses.
	...
	// and send another FLC to the subsequent 10th address in each
	// derivation path.
	restoreCheckBalance(3*chainutil.LokiPerFlokicoin, 3, 1, skipAndSend(9))

	// Check that using a recovery window of 9 does not find the two most
	// recent txns.
	restoreCheckBalance(3*chainutil.LokiPerFlokicoin, 3, 9, nil)

	// Extending our recovery window to 10 should find the most recent
	// transactions, leaving the wallet with 6 FLC total. We should also
	...
	restoreCheckBalance(6*chainutil.LokiPerFlokicoin, 6, 10, skipAndSend(19))

	// Check that recovering with a recovery window of 19 fails to find the
	// most recent transactions.
	restoreCheckBalance(6*chainutil.LokiPerFlokicoin, 6, 19, nil)
	...
	// would've caught the bug earlier. Carol has received 9 FLC so far from
	// the miner, we'll send 8 back to ensure all of her UTXOs get spent to
	// avoid fee discrepancies and a change output is formed.
	const minerAmt = 8 * chainutil.LokiPerFlokicoin
	const finalBalance = 9 * chainutil.LokiPerFlokicoin
	...
	// We should expect a static fee of 36400 loki for spending 9
...
func testRescanAddressDetection(ht *lntest.HarnessTest) {
	...
	pubKeyHash := chainutil.Hash160(keyDesc.RawKeyBytes)
	ghostUtxoAddr, err := chainutil.NewAddressWitnessPubKeyHash(
