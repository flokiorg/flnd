...
func basicChannelFundingTest(ht *lntest.HarnessTest,
	...
	*lnrpc.Channel) {
	...
	// First establish a channel with a capacity of 0.5 FLC between Alice
	// and Bob with Alice pushing 100k loki to Bob's side during
...
func testUnconfirmedChannelFunding(ht *lntest.HarnessTest) {
	...
	// response to a channel balance RPC.
	checkChannelBalance := func(node *node.HarnessNode,
		local, remote, pendingLocal, pendingRemote chainutil.Amount) {

		expectedResponse := &lnrpc.ChannelBalanceResponse{
			LocalBalance: &lnrpc.Amount{
				Sat: uint64(local),
				Msat: uint64(lnwire.NewMSatFromLokis(
					local,
				)),
			},
...
func runChannelFundingInputTypes(ht *lntest.HarnessTest, alice,
	...
	fundMixed := func(amt chainutil.Amount,
	...
	fundMultipleP2TR := func(amt chainutil.Amount,
		target *node.HarnessNode) *wire.MsgTx {
	...
	fundWithTypes := []func(amt chainutil.Amount,
		target *node.HarnessNode) *wire.MsgTx{
	...
	checkChannelBalance := func(node *node.HarnessNode, local,
		remote, pendingLocal, pendingRemote chainutil.Amount) {

		expectedResponse := &lnrpc.ChannelBalanceResponse{
			LocalBalance: &lnrpc.Amount{
				Sat:  uint64(local),
				Msat: uint64(lnwire.NewMSatFromLokis(local)),
			},
			RemoteBalance: &lnrpc.Amount{
				Sat: uint64(remote),
				Msat: uint64(lnwire.NewMSatFromLokis(
...
func runExternalFundingScriptEnforced(ht *lntest.HarnessTest) {
	...
	payAmt := chainutil.Amount(100000)
...
func testChannelFundingPersistence(ht *lntest.HarnessTest) {
	...
	const numConfs = 5
	carolArgs := []string{
		fmt.Sprintf("--flokicoin.defaultchanconfs=%v", numConfs),
	}
	carol := ht.NewNode("Carol", carolArgs)
...
func testBatchChanFunding(ht *lntest.HarnessTest) {
	...
	ensurePolicy(
		...
		lnwire.MilliLoki(batchReq.Channels[0].BaseFee),
		chainreg.DefaultFlokicoinFeeRate,
	)
	ensurePolicy(
		ht, alice, carol, chanPoint2,
		chainreg.DefaultFlokicoinBaseFeeMSat,
		lnwire.MilliLoki(batchReq.Channels[1].FeeRate),
	)
	ensurePolicy(
		ht, alice, dave, chanPoint3,
		lnwire.MilliLoki(batchReq.Channels[2].BaseFee),
		lnwire.MilliLoki(batchReq.Channels[2].FeeRate),
	)
	ensurePolicy(
...
func ensurePolicy(ht *lntest.HarnessTest, alice, peer *node.HarnessNode,
	chanPoint *lnrpc.ChannelPoint, expectedBaseFee lnwire.MilliLoki,
	expectedFeeRate lnwire.MilliLoki) {
...
func testChannelFundingWithUnstableUtxos(ht *lntest.HarnessTest) {
	...
	fundingAmt := chainutil.Amount(3_000_000)
	...
	chanSize := chainutil.Amount(walletUtxo.AmountSat / 3)
	...
	chanSize = chainutil.Amount(carolBalance.TotalBalance) -
		fundingFee(2, false)
