...
// Wumbo nodes should enforce a default soft limit of 1,000,000 FLC by default. This
...
func testMaxChannelSize(ht *lntest.HarnessTest) {
	...
	// maximum channel size (1,000,000 FLC)
	...
	// We'll send 1,000,001 FLC to the wumbo node so it can test the wumbo soft
	// limit.
	ht.FundCoins(1000001*chainutil.LokiPerFlokicoin, wumboNode)
	...
	wumboNode3 := ht.NewNode(
		"wumbo3", []string{
			...
			fmt.Sprintf(
				"--maxchansize=%v",
				int64(funding.MaxFlokicoinFundingAmountWumbo+1),
			),
		},
	)
