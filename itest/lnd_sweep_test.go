...
func testSweepCPFPAnchorIncomingTimeout(ht *lntest.HarnessTest) {
	...
	htlcValue := chainutil.Amount(htlc.Amount)
...
func testSweepCommitOutputAndAnchor(ht *lntest.HarnessTest) {
	...
	fundAmt := chainutil.Amount(1_000_000)
	...
	bobValue := chainutil.Amount(bobToLocal)
...
func runBumpFee(ht *lntest.HarnessTest, alice *node.HarnessNode) {
	...
	tx := ht.SendCoins(alice, alice, chainutil.LokiPerFlokicoin)
	txid := tx.TxHash()
	...
	value := chainutil.Amount(tx.TxOut[0].Value)
	...
	endingFeeRate := chainfee.NewSatPerKWeight(
		chainutil.Amount(smallBudget), txWeight,
	)
...
func testBumpForceCloseFee(ht *lntest.HarnessTest) {
	...
	// calculation.
	pushAmt := chainutil.Amount(50_000)
