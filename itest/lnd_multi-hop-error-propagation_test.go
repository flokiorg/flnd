...
func testHtlcErrorPropagation(ht *lntest.HarnessTest) {
	...
	htlcAmt := lnwire.NewMSatFromLokis(1000)
	sendReq = &routerrpc.SendPaymentRequest{
		PaymentHash: carolInvoice.RHash,
		Dest:        carol.PubKey[:],
		// 10k loki are expected.
		Amt:            int64(htlcAmt.ToLokis()),
		FinalCltvDelta: int32(carolPayReq.CltvExpiry),
		FeeLimitMsat:   noFeeLimitMsat,
		MaxParts:       1,
	}
	...
	// Alice's side, leaving on 10k loki of available balance for bob.
	...
	// At this point, Alice has 50mil loki on her side of the channel,
	// but Bob only has 10k available on his side of the channel. So a
	// payment from Alice to Carol worth 100k loki should fail.
	invoiceReq = &lnrpc.Invoice{
		Value: 100000,
	}
