...
type chanFundMaxTestCase struct {
	...
	initialWalletBalance chainutil.Amount
	...
	expectedBalanceAlice chainutil.Amount
	...
	var testCases = []*chanFundMaxTestCase{
		{
			...
			feeRate:              20,
			expectedErrStr: "output amount(-0.00000435 FLC) " +
				"after subtracting fees(0.00002435 FLC) " +
				"below dust limit(0.00000330 FLC)",
		},
		{
	...
	var testCases = []*chanFundMaxTestCase{
		{
			...
			expectedBalanceAlice: chainutil.Amount(37_000) -
				fundingFee(1, false),
		},
		{
			name: "wallet amount > max chan size " +
	...
	var testCases = []*chanFundMaxTestCase{
		{
			...
			expectedBalanceAlice: chainutil.Amount(100_000) -
...
func runFundMaxTestCase(ht *lntest.HarnessTest, alice, bob *node.HarnessNode,
	testCase *chanFundMaxTestCase, reserveAmount chainutil.Amount) {
...
// wallet.
func fundingFee(numInput int, change bool) chainutil.Amount {
