...
func testSwitchOfflineDelivery(ht *lntest.HarnessTest) {
	...
	// At this point all the channels within our proto network should be
	// shifted by 5k loki in the direction of Carol, the sink within the
...
func testSwitchOfflineDeliveryOutgoingOffline(ht *lntest.HarnessTest) {
	...
	// show a shift of 5k loki towards Carol.  The order of asserts
...
func setupScenarioFourNodes(ht *lntest.HarnessTest) *scenarioFourNodes {
	const (
		...
		pushAmt = chainutil.Amount(900000)
	)
	...
	// loki with a different preimage each time.
