...
type chanFundUtxoSelectionTestCase struct {
	...
	remainingWalletBalance chainutil.Amount
...
func testChannelUtxoSelectionError(ht *lntest.HarnessTest) {
	...
	reserveAmount := chainutil.Amount(reserveResp.RequiredReserve)
	...
	var tcs = []*chanFundUtxoSelectionTestCase{
		...
		// miner fees.
		{
			...
			initialCoins:       []chainutil.Amount{2_000},
			selectedCoins:      []chainutil.Amount{2_000},
			chanOpenShouldFail: true,
			feeRate:            15,
			expectedErrStr: "output amount(0.00000174 FLC) after " +
				"subtracting fees(0.00001826 FLC) below dust " +
				"limit(0.00000330 FLC)",
		},
		// Selected coins don't cover the minimum channel size.
		{
			name: "fundmax, local amount < min chan " +
				"size",
			initialCoins:       []chainutil.Amount{18_000},
			selectedCoins:      []chainutil.Amount{18_000},
			feeRate:            1,
			chanOpenShouldFail: true,
			expectedErrStr: "available funds(0.00017877 FLC) " +
				"below the minimum amount(0.00020000 FLC)",
		},
		// The local amount exceeds the value of the selected coins.
		{
			name: "selected, local amount > " +
				"selected amount",
			initialCoins:       []chainutil.Amount{100_000, 50_000},
			selectedCoins:      []chainutil.Amount{100_000},
			localAmt:           chainutil.Amount(210_337),
			chanOpenShouldFail: true,
			expectedErrStr: "not enough witness outputs to " +
				"create funding transaction, need 0.00210337 " +
				"FLC only have 0.00100000 FLC available",
		},
		...
		// available local amount to fund an anchor channel.
		{
			...
			selectedCoins:  []chainutil.Amount{200_000, 100_000},
			commitmentType: lnrpc.CommitmentType_ANCHORS,
			localAmt: chainutil.Amount(300_000) -
...
func testUtxoSelectionSelectedValidChanSize(ht *lntest.HarnessTest) {
	...
	tc := &chanFundUtxoSelectionTestCase{
		...
		localAmt:        chainutil.Amount(250_000),
		expectedBalance: chainutil.Amount(250_000),
		remainingWalletBalance: chainutil.Amount(350_000) -
			chainutil.Amount(250_000) - fundingFee(2, true),
	}
...
func testUtxoSelectionSelectedValidChanReserve(ht *lntest.HarnessTest) {
	...
	tc := &chanFundUtxoSelectionTestCase{
		...
		expectedBalance: chainutil.Amount(300_000) -
			reserveAmount - fundingFee(2, true),
...
func testUtxoSelectionFundmax(ht *lntest.HarnessTest) {
	...
	tc := &chanFundUtxoSelectionTestCase{
		...
		selectedCoins:  []chainutil.Amount{200_000},
		commitmentType: lnrpc.CommitmentType_ANCHORS,
		expectedBalance: chainutil.Amount(200_000) -
			fundingFee(1, false),
		remainingWalletBalance: reserveAmount,
	}
...
func runUtxoSelectionTestCase(ht *lntest.HarnessTest, alice,
	...
	reserveAmount chainutil.Amount) {
