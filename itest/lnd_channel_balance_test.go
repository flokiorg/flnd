...
func testChannelBalance(ht *lntest.HarnessTest) {
	// Open a channel with 100,000 FLC between Alice and Bob, ensuring the
	...
	// exactly 0.5 FLC since now state transitions have taken place yet.
...
func testChannelUnsettledBalance(ht *lntest.HarnessTest) {
	...
	// response to a channel balance RPC.
	checkChannelBalance := func(node *node.HarnessNode,
		local, remote, unsettledLocal, unsettledRemote chainutil.Amount) {
