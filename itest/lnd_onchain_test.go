...
func testChainKitSendOutputsAnchorReserve(ht *lntest.HarnessTest) {
	...
	fundingAmount := chainutil.Amount(100_000)
	...
	chanAmt := fundingAmount - 2*chainutil.Amount(reserve.RequiredReserve)
...
func testRemoveTx(ht *lntest.HarnessTest) {
	...
	const initialWalletAmt = chainutil.LokiPerFlokicoin
	...
	expectedAmt := chainutil.Amount(initialWalletAmt) - txFee
	...
	aliceBalResp = alice.RPC.WalletBalance()
	expectedAmt = chainutil.Amount(initialWalletAmt)
