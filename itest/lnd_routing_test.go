...
func testSingleHopSendToRouteCase(ht *lntest.HarnessTest,
	...
	const chanAmt = chainutil.Amount(100000)
	...
	// is the sole funder of the channel with 100k loki. The network
	...
	ht.FundCoins(chainutil.LokiPerFlokicoin, carol)

	// Open a channel with 100k loki between Carol and Dave with Carol
	...
	// shifted by 5k loki in the direction of Dave, the sink within the
...
func runMultiHopSendToRoute(ht *lntest.HarnessTest, useGraphCache bool) {
	...
	// Open a channel with 100k loki between Alice and Bob with Alice
	// being the sole funder of the channel.
	...
	// funder of the channel with 100k loki. The network topology
	// should look like:
	// Alice -> Bob -> Carol
	carol := ht.NewNode("Carol", nil)
	...
	// 1k loki with a different preimage each time.
	const (
		numPayments = 5
		paymentAmt  = 1000
	)
	...
	routesReq := &lnrpc.QueryRoutesRequest{
		...
		Amt:            paymentAmt,
		FinalCltvDelta: chainreg.DefaultFlokicoinTimeLockDelta,
	}
	...
	for i, rHash := range rHashes {
		...
		route.Hops[len(route.Hops)-1].MppRecord = &lnrpc.MPPRecord{
			...
			TotalAmtMsat: int64(
				lnwire.NewMSatFromLokis(paymentAmt),
			),
		}
	...
	// When asserting the amount of loki moved, we'll factor in the
	// default base fee, as we didn't modify the fee structure when
	// creating the seed nodes in the network.
	const baseFee = 1

	// At this point all the channels within our proto network should be
	// shifted by 5k loki in the direction of Carol, the sink within
...
func testSendToRouteErrorPropagation(ht *lntest.HarnessTest) {
	...
	ht.FundCoins(chainutil.LokiPerFlokicoin, charlie)
...
func testPrivateChannels(ht *lntest.HarnessTest) {
	...
	// Open a channel with 200k loki between Alice and Bob.
	alice := ht.NewNodeWithCoins("Alice", nil)
	bob := ht.NewNode("Bob", nil)
	ht.EnsureConnected(alice, bob)
	...
	// Create 2 invoices for Bob, each of 70k loki. Since each of
	...
	// Bob should have received 140k loki from Alice.
	ht.AssertAmountPaid("Alice(local) => Bob(remote)", bob,
...
func testScidAliasRoutingHints(ht *lntest.HarnessTest) {
	...
	const chanAmt = chainutil.Amount(800000)
	...
	hopHint := &lnrpc.HopHint{
		...
		FeeBaseMsat: uint32(
			chainreg.DefaultFlokicoinBaseFeeMSat,
		),
		FeeProportionalMillionths: uint32(
			chainreg.DefaultFlokicoinFeeRate,
		),
		CltvExpiryDelta: chainreg.DefaultFlokicoinTimeLockDelta,
	}
...
func testMultiHopOverPrivateChannels(ht *lntest.HarnessTest) {
	...
	// Dave of 20k loki and pay it with Alice. Since there is no public
...
func testQueryRoutes(ht *lntest.HarnessTest) {
	...
	for i, route := range routesRes.Routes {
		expectedTotalFeesMSat :=
			lnwire.MilliLoki(len(route.Hops)-1) * feePerHopMSat
...
func testRouteFeeCutoff(ht *lntest.HarnessTest) {
	...
	// amount and as a fixed amount of loki.
	const chanAmt = chainutil.Amount(100000)
	...
	baseFee := int64(10000)
	feeRate := int64(5)
	timeLockDelta := uint32(chainreg.DefaultFlokicoinTimeLockDelta)
	...
	// other will have it expressed as a fixed amount of loki.
	const paymentAmt = 100
	carolFee := computeFee(lnwire.MilliLoki(baseFee), 1, paymentAmt)
	...
	feeLimitFixed := &lnrpc.FeeLimit{
		Limit: &lnrpc.FeeLimit_Fixed{
			Fixed: int64(carolFee.ToLokis()) - 1,
		},
	}
...
func testFeeLimitAfterQueryRoutes(ht *lntest.HarnessTest) {
	...
	chanAmt := chainutil.Amount(100000)
	...
	expectedPolicy := &lnrpc.RoutingPolicy{
		...
		TimeLockDelta: uint32(
			chainreg.DefaultFlokicoinTimeLockDelta,
		),
		MinHtlc:     1000,
...
func computeFee(baseFee, feeRate, amt lnwire.MilliLoki) lnwire.MilliLoki {
