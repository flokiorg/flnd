...
func newChanRestoreScenario(ht *lntest.HarnessTest, ct lnrpc.CommitmentType,
	...
	const (
		chanAmt = chainutil.Amount(10000000)
		pushAmt = chainutil.Amount(5000000)
	)
...
func testChannelBackupUpdates(ht *lntest.HarnessTest) {
	...
	numChans := 2
	chanAmt := chainutil.Amount(1000000)
...
func testDataLossProtection(ht *lntest.HarnessTest) {
	...
	timeTravelDave := func() (func() error, *lnrpc.ChannelPoint, int64) {
		...
		// We'll first open up a channel between them with a 0.5 FLC
		// value.
		chanPoint := ht.OpenChannel(
		...
		// of 10k loki each, it should now see his balance as being
		// 30k loki.
...
func createLegacyRevocationChannel(ht *lntest.HarnessTest,
	chanAmt, pushAmt chainutil.Amount, from, to *node.HarnessNode) {
