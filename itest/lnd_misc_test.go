...
func testSphinxReplayPersistence(ht *lntest.HarnessTest) {
	...
	ht.FundCoins(chainutil.LokiPerFlokicoin, fred)
	...
	// expects a payment of 1000 loki from Carol paid via a particular
	// preimage.
...
func testMaxPendingChannels(ht *lntest.HarnessTest) {
	...
	carolBalance := chainutil.Amount(maxPendingChannels) * amount
...
func testRejectHTLC(ht *lntest.HarnessTest) {
	...
	// Alice ------> Carol ------> Bob
	//
	const chanAmt = chainutil.Amount(1000000)
	alice := ht.NewNodeWithCoins("Alice", nil)
	bob := ht.NewNodeWithCoins("Bob", nil)
	...
	// Create an invoice from Carol of 100 loki.
	...
	// Create an invoice from Bob of 100 loki.
...
func testSendAllCoins(ht *lntest.HarnessTest) {
	...
	// Next, we'll give Ainz exactly 2 utxos of 1 FLC each, with one of
	// them being p2wkh and the other being a n2wpkh address.
	ht.FundCoins(chainutil.LokiPerFlokicoin, ainz)
	ht.FundCoinsNP2WKH(chainutil.LokiPerFlokicoin, ainz)
...
func testListAddresses(ht *lntest.HarnessTest) {
	...
	// Next, we'll give Alice exactly 1 utxo of 1 FLC.
	ht.FundCoins(chainutil.LokiPerFlokicoin, alice)
...
func testSendSelectedCoins(ht *lntest.HarnessTest) {
	...
	// Next, we'll give Alice exactly 3 utxos of 1 FLC of different address
	// types.
	ht.FundCoins(chainutil.LokiPerFlokicoin, alice)
	ht.FundCoinsNP2WKH(chainutil.LokiPerFlokicoin, alice)
	ht.FundCoinsP2TR(chainutil.LokiPerFlokicoin, alice)
	...
	//
	// We'll send the first utxo with an amount of 0.5 FLC.
	amt := chainutil.Amount(0.5 * chainutil.LokiPerFlokicoin)
	alice.RPC.SendCoins(&lnrpc.SendCoinsRequest{
		Addr: ht.NewMinerAddress().String(),
...
func testSendSelectedCoinsChannelReserve(ht *lntest.HarnessTest) {
	chanAmt := chainutil.Amount(100_000)
...
func testReorgNotifications(ht *lntest.HarnessTest) {
	...
	// Mine tx1 only.
	ht.Miner().MineBlockWithTxes([]*chainutil.Tx{chainutil.NewTx(tx1)})
	...
	block1 := ht.Miner().MineBlockWithTxes(
		[]*chainutil.Tx{chainutil.NewTx(tx2b)},
	)
	...
	ht.Miner().MineBlockWithTxes([]*chainutil.Tx{chainutil.NewTx(tx2a)})
