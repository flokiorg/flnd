...
func testUpdateChannelPolicy(ht *lntest.HarnessTest) {
	...
	defaultMaxHtlc := lntest.CalculateMaxHtlc(funding.MaxFlokicoinFundingAmount)
	...
	payAmt = chainutil.Amount(4)
	amtSat := int64(payAmt)
	amtMSat := int64(lnwire.NewMSatFromLokis(payAmt))
	...
	substrs := []string{
		"AmountBelowMinimum",
		"HtlcMinimumMsat: (lnwire.MilliLoki) 5000 mSAT",
	}
...
func testSendUpdateDisableChannel(ht *lntest.HarnessTest) {
	...
	// Give Eve some coins.
	ht.FundCoins(chainutil.LokiPerFlokicoin, eve)
	...
	expectedPolicy := &lnrpc.RoutingPolicy{
		FeeBaseMsat:      int64(chainreg.DefaultFlokicoinBaseFeeMSat),
		FeeRateMilliMsat: int64(chainreg.DefaultFlokicoinFeeRate),
...
func testUpdateChannelPolicyForPrivateChannel(ht *lntest.HarnessTest) {
	...
	// Open a channel with 100k loki between Alice and Bob.
	...
	// Open a channel with 100k loki between Bob and Carol.
