...
func testQueryBlindedRoutes(ht *lntest.HarnessTest) {
	...
	bobCarolDelta := uint32(chainreg.DefaultFlokicoinTimeLockDelta)
	...
	var (
		_, blindingPoint = crypto.PrivKeyFromBytes([]byte{1})
		_, carolBlinded  = crypto.PrivKeyFromBytes([]byte{2})
		_, blindedHop1   = crypto.PrivKeyFromBytes([]byte{3})
		_, blindedHop2   = crypto.PrivKeyFromBytes([]byte{4})
...
func testMPPToSingleBlindedPath(ht *lntest.HarnessTest) {
	...
	for i := 0; i < 2; i++ {
		ht.FundCoinsUnconfirmed(chainutil.LokiPerFlokicoin, alice)
		ht.FundCoinsUnconfirmed(chainutil.LokiPerFlokicoin, bob)
		ht.FundCoinsUnconfirmed(chainutil.LokiPerFlokicoin, carol)
		ht.FundCoinsUnconfirmed(chainutil.LokiPerFlokicoin, dave)
		ht.FundCoinsUnconfirmed(chainutil.LokiPerFlokicoin, eve)
		ht.MineBlocksAndAssertNumTxes(1, 5)
	}

	const paymentAmt = chainutil.Amount(300000)
