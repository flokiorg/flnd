...
func testUpdateOnFunderPendingOpenChannels(ht *lntest.HarnessTest) {
	...
	params := lntest.OpenChannelParams{
		Amt:     funding.MaxFlokicoinFundingAmount,
		PushAmt: funding.MaxFlokicoinFundingAmount / 2,
	}
...
func testPendingChannelAfterReorg(ht *lntest.HarnessTest) {
	...
	// Alice initiates a channel opening to Bob.
	params := lntest.OpenChannelParams{Amt: funding.MaxFlokicoinFundingAmount}
...
func testOpenChannelLockedBalance(ht *lntest.HarnessTest) {
	...
	openChannelReq := &lnrpc.OpenChannelRequest{
		...
		LocalFundingAmount: int64(funding.MaxFlokicoinFundingAmount),
		TargetConf:         6,
	}
	...
	ht.AssertWalletLockedBalance(alice, chainutil.LokiPerFlokicoin)
...
func testOpenChannelWithShutdownAddr(ht *lntest.HarnessTest) {
	...
	openChannelParams := lntest.OpenChannelParams{
		Amt:     chainutil.Amount(channelAmount),
		PushAmt: chainutil.Amount(paymentAmount),
	}
	...
	// We expect 6 confirmed UTXOs, as 5 UTXOs of 1 FLC each were sent to
