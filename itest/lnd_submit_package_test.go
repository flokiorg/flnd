...
func testSubmitPackage(ht *lntest.HarnessTest) {
	...
	// flokicoind does not implement submitpackage either, so this test is
	// a permanent skip until/unless that RPC lands upstream in
	// go-flokicoin; see lnwallet/btcwallet.BtcWallet.SubmitPackage.
	...
	const (
		fundAmt = int64(chainutil.LokiPerFlokicoin)
	...
	// SignOutputRaw RPC.
	p2wkhKey := func() (*signrpc.KeyDescriptor, *crypto.PublicKey,
		chainutil.Address, []byte) {

		keyDesc := alice.RPC.DeriveNextKey(&walletrpc.KeyReq{
			KeyFamily: p2wkhKeyFamily,
		})

		pubKey, err := crypto.ParsePubKey(keyDesc.RawKeyBytes)
		require.NoError(ht, err)

		addr, err := chainutil.NewAddressWitnessPubKeyHash(
			chainutil.Hash160(pubKey.SerializeCompressed()),
			harnessNetParams,
		)
		require.NoError(ht, err)
	...
	childOutAddr, err := chainutil.DecodeAddress(
