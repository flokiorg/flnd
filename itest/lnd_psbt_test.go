...
// by using a Partially Signed Flokicoin Transaction that funds the channel
...
// and dave by using a Partially Signed Flokicoin Transaction that funds the
...
func runPsbtChanFundingSingleStep(ht *lntest.HarnessTest, private bool,
	...
	reserveAddr, err := chainutil.DecodeAddress(
...
func runSignPsbtSegWitV0P2WKH(ht *lntest.HarnessTest, alice *node.HarnessNode) {
	...
	require.NoError(ht, err)
	pubKeyHash := chainutil.Hash160(addrPubKey.SerializeCompressed())
	witnessAddr, err := chainutil.NewAddressWitnessPubKeyHash(
...
func runSignPsbtSegWitV0NP2WKH(ht *lntest.HarnessTest,
	...
	require.NoError(ht, err)
	np2wkhAddr, err := chainutil.NewAddressScriptHash(
...
func runFundPsbt(ht *lntest.HarnessTest, alice, bob *node.HarnessNode) {
	...
	// 5 million Lokis in a non-obvious way. So Bob selects a UTXO that's
	// bigger than 5 million Lokis and expects the change minus the send
	// amount back. Alice then funds the PSBT with coins of her own and
	// combines her change with the 5 million Lokis from Bob. With this
...
func addressToPkScript(t testing.TB, addr string) []byte {
	parsed, err := chainutil.DecodeAddress(addr, harnessNetParams)
...
func assertPsbtFundSignSpend(ht *lntest.HarnessTest, alice *node.HarnessNode,
	...
	_, err = crypto.ParsePubKey(derivation.PubKey)
	require.NoError(ht, err)
...
func deriveInternalKey(ht *lntest.HarnessTest,
	alice *node.HarnessNode) (*signrpc.KeyDescriptor, *crypto.PublicKey,
	[]uint32) {
	...
	parsedPubKey, err := crypto.ParsePubKey(keyDesc.RawKeyBytes)
	require.NoError(ht, err)
...
func testPsbtChanFundingWithUnstableUtxos(ht *lntest.HarnessTest) {
	fundingAmt := chainutil.Amount(2_000_000)
	...
	chanSize := chainutil.Amount(walletUtxo.AmountSat / 2)
