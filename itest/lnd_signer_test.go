...
func runDeriveSharedKey(ht *lntest.HarnessTest, alice *node.HarnessNode) {
	...
	ephemeralPriv, err := crypto.NewPrivateKey()
	...
	assertECDHMatch := func(pub *crypto.PublicKey,
	...
	nodePub, err := crypto.ParsePubKey(alice.PubKey[:])
	...
	customizedPub, err := crypto.ParsePubKey(resp.RawKeyBytes)
	require.NoError(ht, err, "failed to parse node pubkey")
...
func runSignOutputRaw(ht *lntest.HarnessTest, alice *node.HarnessNode) {
	...
	targetPubKey, err := crypto.ParsePubKey(keyDesc.RawKeyBytes)
...
func assertSignOutputRaw(ht *lntest.HarnessTest,
	alice *node.HarnessNode, targetPubKey *crypto.PublicKey,
	keyDesc *signrpc.KeyDescriptor,
	sigHash txscript.SigHashType) {

	pubKeyHash := chainutil.Hash160(targetPubKey.SerializeCompressed())
	targetAddr, err := chainutil.NewAddressWitnessPubKeyHash(
	...
	p2wkhAdrr, err := chainutil.DecodeAddress(
...
func runSignVerifyMessage(ht *lntest.HarnessTest, alice *node.HarnessNode) {
	...
	deriveCustomizedKey := func() *crypto.PublicKey {
		resp := alice.RPC.DeriveKey(keyLoc)
		pub, err := crypto.ParsePubKey(resp.RawKeyBytes)
		require.NoError(ht, err, "failed to parse node pubkey")
