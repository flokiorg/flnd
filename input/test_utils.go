...
func NewMockSigner(privKeys []*crypto.PrivateKey,
	...
	keyFetcher := func(*keychain.KeyDescriptor) (*crypto.PrivateKey, error) {
...
func (m *MockSigner) SignOutputRaw(tx *wire.MsgTx,
	...
	hash160 := chainutil.Hash160(pubkey.SerializeCompressed())
...
func (m *MockSigner) findKey(needleHash160 []byte, singleTweak []byte,
	doubleTweak *crypto.PrivateKey) *crypto.PrivateKey {

	for _, privkey := range m.Privkeys {
		// First check whether public key is directly derived from
		// private key.
		hash160 := chainutil.Hash160(privkey.PubKey().SerializeCompressed())
...
// pubkeyToHex serializes a Flokicoin public key to a hex encoded string.
func pubkeyToHex(key *crypto.PublicKey) string {
	return hex.EncodeToString(key.SerializeCompressed())
}

// privkeyFromHex serializes a Flokicoin private key to a hex encoded string.
func privkeyToHex(key *crypto.PrivateKey) string {
