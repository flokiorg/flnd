...
func MustParsePubKey(pubStr string) crypto.PublicKey {
	pubBytes, err := hex.DecodeString(pubStr)
	if err != nil {
		panic(err)
	}

	pub, err := crypto.ParsePubKey(pubBytes)
	if err != nil {
		panic(err)
	}
...
type Signature interface {
	...
	Verify([]byte, *crypto.PublicKey) bool
}
...
func WitnessPubKeyHash(pubkey []byte) ([]byte, error) {
	...
	bldr.AddOp(txscript.OP_0)
	pkhash := chainutil.Hash160(pubkey)
	bldr.AddData(pkhash)
	return bldr.Script()
}
...
func GenTaprootFundingScript(aPub, bPub *crypto.PublicKey,
	...
	*wire.TxOut, error) {
	...
	combinedKey, _, _, err := musig2.AggregateKeys(
		[]*crypto.PublicKey{aPub, bPub}, true, muSig2Opt,
	)
	if err != nil {
...
func SenderHTLCScript(senderHtlcKey, receiverHtlcKey,
	revocationKey *crypto.PublicKey, paymentHash []byte,
	...
	builder.AddData(chainutil.Hash160(revocationKey.SerializeCompressed()))
...
func SenderHtlcSpendRevokeWithKey(signer Signer, signDesc *SignDescriptor,
	revokeKey *crypto.PublicKey, sweepTx *wire.MsgTx) (wire.TxWitness, error) {
...
func SenderHTLCScriptTaprootRedeem(signer Signer, signDesc *SignDescriptor,
	sweepTx *wire.MsgTx, preimage []byte, revokeKey *crypto.PublicKey,
...
func ReceiverHTLCScript(cltvExpiry uint32, senderHtlcKey,
	receiverHtlcKey, revocationKey *crypto.PublicKey,
...
func deriveRevokePubKey(signDesc *SignDescriptor) (*crypto.PublicKey, error) {
...
func ReceiverHTLCScriptTaprootRedeem(senderSig Signature,
	...
	htlcSuccessTx *wire.MsgTx, revokeKey *crypto.PublicKey,
	tapscriptTree *txscript.IndexedTapScriptTree) (wire.TxWitness, error) {
...
func ReceiverHTLCScriptTaprootTimeout(signer Signer, signDesc *SignDescriptor,
	sweepTx *wire.MsgTx, cltvExpiry int32, revokeKey *crypto.PublicKey,
	tapscriptTree *txscript.IndexedTapScriptTree) (wire.TxWitness, error) {
...
//   - transition?
//   - covenant output
func SecondLevelHtlcScript(revocationKey, delayKey *crypto.PublicKey,
...
func TaprootSecondLevelHtlcScript(revokeKey, delayKey *crypto.PublicKey,
	csvDelay uint32, auxLeaf AuxTapLeaf) (*crypto.PublicKey, error) {
...
func TaprootHtlcSpendSuccess(signer Signer, signDesc *SignDescriptor,
	sweepTx *wire.MsgTx, revokeKey *crypto.PublicKey,
	tapscriptTree *txscript.IndexedTapScriptTree) (wire.TxWitness, error) {
...
//	 OP_ENDIF
//	 OP_CHECKSIG.
func LeaseSecondLevelHtlcScript(revocationKey, delayKey *crypto.PublicKey,
...
//	OP_ENDIF
//	OP_CHECKSIG
func CommitScriptToSelf(csvTimeout uint32, selfKey, revokeKey *crypto.PublicKey) ([]byte, error) {
...
func TaprootCommitScriptToSelf(csvTimeout uint32,
	selfKey, revokeKey *crypto.PublicKey) (*crypto.PublicKey, error) {
...
// of the leaf.
func MakeTaprootCtrlBlock(leafScript []byte, internalKey *crypto.PublicKey,
...
//	OP_ENDIF
//	OP_CHECKSIG
func LeaseCommitScriptToSelf(selfKey, revokeKey *crypto.PublicKey,
...
func CommitScriptUnencumbered(key *crypto.PublicKey) ([]byte, error) {
	// This script goes to the "other" party, and is spendable immediately.
	builder := txscript.NewScriptBuilder(txscript.WithScriptAllocSize(
		P2WPKHSize,
	))
	builder.AddOp(txscript.OP_0)
	builder.AddData(chainutil.Hash160(key.SerializeCompressed()))
...
func CommitScriptToRemoteConfirmed(key *crypto.PublicKey) ([]byte, error) {
	builder := txscript.NewScriptBuilder(txscript.WithScriptAllocSize(
...
//	1 OP_CHECKSEQUENCEVERIFY OP_DROP
func TaprootCommitScriptToRemote(remoteKey *crypto.PublicKey,
	auxLeaf AuxTapLeaf) (*crypto.PublicKey, error) {
...
//		1 OP_CHECKSEQUENCEVERIFY
func LeaseCommitScriptToRemoteConfirmed(key *crypto.PublicKey,
...
//	  OP_16 OP_CSV
//	OP_ENDIF
func CommitScriptAnchor(key *crypto.PublicKey) ([]byte, error) {
	builder := txscript.NewScriptBuilder(txscript.WithScriptAllocSize(
		AnchorScriptSize,
	))
...
func NewAnchorScriptTree(
	anchorKey *crypto.PublicKey) (*AnchorScriptTree, error) {
...
func TaprootOutputKeyAnchor(key *crypto.PublicKey) (*crypto.PublicKey, error) {
...
func TaprootAnchorSpendAny(anchorKey *crypto.PublicKey) (wire.TxWitness, error) {
...
func SingleTweakBytes(commitPoint, basePoint *crypto.PublicKey) []byte {
	h := sha256.New()
...
func TweakPubKey(basePoint, commitPoint *crypto.PublicKey) *crypto.PublicKey {
...
func TweakPubKeyWithTweak(pubKey *crypto.PublicKey,
	tweakBytes []byte) *crypto.PublicKey {

	var (
		pubKeyJacobian crypto.JacobianPoint
		tweakJacobian  crypto.JacobianPoint
		resultJacobian crypto.JacobianPoint
	)
	tweakKey, _ := crypto.PrivKeyFromBytes(tweakBytes)
	crypto.ScalarBaseMultNonConst(&tweakKey.Key, &tweakJacobian)

	pubKey.AsJacobian(&pubKeyJacobian)
	crypto.AddNonConst(&pubKeyJacobian, &tweakJacobian, &resultJacobian)

	resultJacobian.ToAffine()
	return crypto.NewPublicKey(&resultJacobian.X, &resultJacobian.Y)
}
...
//
// Where N is the order of the sub-group.
func TweakPrivKey(basePriv *crypto.PrivateKey,
	commitTweak []byte) *crypto.PrivateKey {

	// tweakInt := sha256(commitPoint || basePub)
	tweakScalar := new(crypto.ModNScalar)
	tweakScalar.SetByteSlice(commitTweak)

	tweakScalar.Add(&basePriv.Key)

	return &crypto.PrivateKey{Key: *tweakScalar}
}
...
func DeriveRevocationPubkey(revokeBase,
	...
	commitTweakScalar := new(crypto.ModNScalar)
	commitTweakScalar.SetByteSlice(commitTweakBytes)

	var (
		commitPointJacobian crypto.JacobianPoint
		cJacobian           crypto.JacobianPoint
	)
	...
	//
	// P = R + C
	var resultJacobian crypto.JacobianPoint
	crypto.AddNonConst(&rJacobian, &cJacobian, &resultJacobian)
...
//
// Where N is the order of the sub-group.
func DeriveRevocationPrivKey(revokeBasePriv *crypto.PrivateKey,
	commitSecret *crypto.PrivateKey) *crypto.PrivateKey {
	...
	return &crypto.PrivateKey{Key: *revocationPriv}
}
...
// for the state.
func ComputeCommitmentPoint(commitSecret []byte) *crypto.PublicKey {
	_, pubKey := crypto.PrivKeyFromBytes(commitSecret)
	return pubKey
}
