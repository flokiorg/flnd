...
type testSenderHtlcScriptTree struct {
	...
	receiverKey *crypto.PrivateKey

	revokeKey *crypto.PrivateKey
...
func newTestCommitScriptTree(local bool,
	...
	selfKey, err := crypto.NewPrivateKey()
	if err != nil {
		return nil, err
	}
...
func newTestAnchorScripTree() (*testAnchorScriptTree, error) {
	sweepKey, err := crypto.NewPrivateKey()
	if err != nil {
		return nil, err
	}
...
func newTestSecondLevelHtlcTree(t *testing.T,
	...
	delayKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
