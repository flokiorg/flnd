...
func TestRevocationKeyDerivation(t *testing.T) {
	...
	commitSecret, commitPoint := crypto.PrivKeyFromBytes(revocationPreimage)

	// With the commitment secrets generated, we'll now create the base
	// keys we'll use to derive the revocation key from.
	basePriv, basePub := crypto.PrivKeyFromBytes(testWalletPrivKey)
...
func TestTweakKeyDerivation(t *testing.T) {
	...
	basePriv, basePub := crypto.PrivKeyFromBytes(baseSecret)
...
func TestHTLCSenderSpendValidation(t *testing.T) {
	...
	commitSecret, commitPoint := crypto.PrivKeyFromBytes(revokePreimage)
	...
	// the HTLC output.
	aliceKeyPriv, aliceKeyPub := crypto.PrivKeyFromBytes(testWalletPrivKey)
	bobKeyPriv, bobKeyPub := crypto.PrivKeyFromBytes(bobsPrivKey)
	paymentAmt := chainutil.Amount(1 * 10e8)
	...
	bobSigner := &MockSigner{Privkeys: []*crypto.PrivateKey{bobKeyPriv}}
	aliceSigner := &MockSigner{Privkeys: []*crypto.PrivateKey{aliceKeyPriv}}
...
func TestLeaseCommmitSpendToSelf(t *testing.T) {
	...
	const (
		outputVal   = chainutil.Amount(2 * 10e8)
		csvDelay    = 5
		leaseExpiry = 1337
	)
...
func TestCommitSpendToRemoteConfirmed(t *testing.T) {
	...
	const outputVal = chainutil.Amount(2 * 10e8)
...
func BenchmarkScriptBuilderAlloc(t *testing.B) {
	...
	randomPriv, err := crypto.NewPrivateKey()
	require.NoError(t, err)
...
func runScriptAllocTest(dummyData, randomPubBytes []byte,
	randomPub *crypto.PublicKey) error {
