...
var (
	...
	testPubkey = &crypto.PublicKey{}

	testPrivkey, _ = crypto.PrivKeyFromBytes(make([]byte, 32))
...
func TestTxWeightEstimator(t *testing.T) {
	...
	p2pkhAddr, err := chainutil.NewAddressPubKeyHash(
	...
	p2wkhAddr, err := chainutil.NewAddressWitnessPubKeyHash(
		make([]byte, 20), netParams)
	require.NoError(t, err, "Failed to generate address")
	p2wkhScript, err := txscript.PayToAddrScript(p2wkhAddr)
	require.NoError(t, err, "Failed to generate scriptPubKey")

	p2wshAddr, err := chainutil.NewAddressWitnessScriptHash(
		make([]byte, 32), netParams)
	require.NoError(t, err, "Failed to generate address")
	p2wshScript, err := txscript.PayToAddrScript(p2wshAddr)
	require.NoError(t, err, "Failed to generate scriptPubKey")

	p2shAddr, err := chainutil.NewAddressScriptHash([]byte{0}, netParams)
	require.NoError(t, err, "Failed to generate address")
...
func (s *maxDERSignature) Verify(_ []byte, _ *crypto.PublicKey) bool {
	return true
}
...
func (s *maxSchnorrSignature) Verify(_ []byte, _ *crypto.PublicKey) bool {
	return true
}
...
var witnessSizeTests = []witnessSizeTest{
	...
	},
	{
		...
		genWitness: func(t *testing.T) wire.TxWitness {
			testKey, err := crypto.NewPrivateKey()
	...
	},
	{
		...
		genWitness: func(t *testing.T) wire.TxWitness {
			...
			revokeKey, err := crypto.NewPrivateKey()
			require.NoError(t, err)
	...
	},
	{
		...
		genWitness: func(t *testing.T) wire.TxWitness {
			senderKey, err := crypto.NewPrivateKey()
			require.NoError(t, err)

			receiverKey, err := crypto.NewPrivateKey()
			require.NoError(t, err)
...
func genTimeoutTx(t *testing.T,
	...
	testKeyPriv, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	...
	timeoutTx, err := lnwallet.CreateHtlcTimeoutTx(
		chanType, false, testOutPoint, chainutil.Amount(testAmt), testCLTVExpiry,
...
func genSuccessTx(t *testing.T, chanType channeldb.ChannelType) *wire.MsgTx {
	...
	successTx, err := lnwallet.CreateHtlcSuccessTx(
		chanType, false, testOutPoint, chainutil.Amount(testAmt), testCSVDelay, 0,
