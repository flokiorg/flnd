...
func TestPeerChannelClosureAcceptFeeResponder(t *testing.T) {
	...
	aliceFee := respClosingSigned.FeeLokis
...
func TestPeerChannelClosureAcceptFeeInitiator(t *testing.T) {
	...
	// message.
	bobFee := closingSignedMsg.FeeLokis
	bobSig, _, _, err := bobChan.CreateCloseProposal(
	...
	if closingSignedMsg.FeeLokis != bobFee {
...
func TestPeerChannelClosureFeeNegotiationsResponder(t *testing.T) {
	...
	preferredRespFee := aliceClosingSigned.FeeLokis
	increasedFee := chainutil.Amount(float64(preferredRespFee) * 2.5)
	bobSig, _, _, err := bobChan.CreateCloseProposal(
	...
	aliceFee := aliceClosingSigned.FeeLokis
	...
	increasedFee = chainutil.Amount(float64(preferredRespFee) * 2.1)
	bobSig, _, _, err = bobChan.CreateCloseProposal(
		increasedFee, dummyDeliveryScript, aliceDeliveryScript,
	)
	require.NoError(t, err, "error creating close proposal")
...
func TestPeerChannelClosureFeeNegotiationsInitiator(t *testing.T) {
	...
	idealFeeRate := closingSignedMsg.FeeLokis
	lastReceivedFee := idealFeeRate

	increasedFee := chainutil.Amount(float64(idealFeeRate) * 2.1)
	...
	aliceFee := closingSignedMsg.FeeLokis
	...
	increasedFee = chainutil.Amount(float64(idealFeeRate) * 1.5)
	lastSentFee = increasedFee
...
func genScript(t *testing.T, address string) lnwire.DeliveryAddress {
	...
	deliveryAddr, err := chainutil.DecodeAddress(
		address,
