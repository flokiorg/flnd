...
func pubkey(t *testing.T) *crypto.PublicKey {
	t.Helper()

	nodeBytes, err := hex.DecodeString(pubkeyStr)
	require.NoError(t, err)

	nodePk, err := crypto.ParsePubKey(nodeBytes)
	require.NoError(t, err)
...
func TestBlindingSpecTestVectors(t *testing.T) {
	...
	require.NoError(t, err)
	nextBlindingOverride, err := crypto.ParsePubKey(nextBlindingOverrideStr)
	require.NoError(t, err)
