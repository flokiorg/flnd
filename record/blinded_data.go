...
type BlindedRouteData struct {
	...
	NextNodeID tlv.OptionalRecordT[tlv.TlvType4, *crypto.PublicKey]
	...
	NextBlindingOverride tlv.OptionalRecordT[tlv.TlvType8, *crypto.PublicKey]
...
func NewNonFinalBlindedRouteData(chanID lnwire.ShortChannelID,
	blindingOverride *crypto.PublicKey, relayInfo PaymentRelayInfo,
...
func NewDummyHopRouteData(ourPubKey *crypto.PublicKey,
...
type PaymentRelayInfo struct {
	...
	// BaseFee is the per-htlc fee charged in milli-loki.
...
func decodePaymentRelay(r io.Reader, val interface{}, buf *[8]byte,
	...
	if t, ok := val.(**PaymentRelayInfo); ok && l <= 10 {
		...
		relayInfo.BaseFee = lnwire.MilliLoki(baseFee)
...
func decodePaymentConstraints(r io.Reader, val interface{}, buf *[8]byte,
	...
	if c, ok := val.(**PaymentConstraints); ok && l <= 12 {
		...
		payConstraints.HtlcMinimumMsat = lnwire.MilliLoki(minHtlc)
