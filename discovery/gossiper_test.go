...
func (ctx *testCtx) createAnnouncements(blockHeight uint32, key1,
	key2 *crypto.PrivateKey) (*annBatch, error) {
	...
	batch.remoteProofAnn = &lnwire.AnnounceSignatures1{
		...
		FlokicoinSignature: batch.chanAnn.FlokicoinSig2,
	}
	...
	batch.localProofAnn = &lnwire.AnnounceSignatures1{
		...
		FlokicoinSignature: batch.chanAnn.FlokicoinSig1,
	}
...
func createNodeAnnouncement(priv *crypto.PrivateKey, timestamp uint32,
...
func createUpdateAnnouncement(blockHeight uint32,
	...
	nodeKey *crypto.PrivateKey, timestamp uint32,
	extraBytes ...[]byte) (*lnwire.ChannelUpdate1, error) {

	var err error

	htlcMinMsat := lnwire.MilliLoki(100)
...
func signUpdate(nodeKey *crypto.PrivateKey, a *lnwire.ChannelUpdate1) error {
...
func (ctx *testCtx) createAnnouncementWithoutProof(blockHeight uint32,
	...
	copy(a.FlokicoinKey1[:], bitcoinKeyPub1.SerializeCompressed())
	copy(a.FlokicoinKey2[:], bitcoinKeyPub2.SerializeCompressed())
...
func (ctx *testCtx) createChannelAnnouncement(blockHeight uint32, key1,
	...
	a.FlokicoinSig1, err = lnwire.NewSigFromSignature(sig)
	if err != nil {
		return nil, err
	}
	...
	a.FlokicoinSig2, err = lnwire.NewSigFromSignature(sig)
	if err != nil {
		return nil, err
	}
	...
	*testCtx, error) {
	...
	gossiper := New(Config{
		...
		NotifyWhenOnline: func(target [33]byte,
			...
			pk, _ := crypto.ParsePubKey(target[:])
...
func TestProcessAnnouncement(t *testing.T) {
	...
	assertSenderExistence := func(sender *crypto.PublicKey, msg msgWithSenders) {
		t.Helper()
...
func TestSignatureAnnouncementLocalFirst(t *testing.T) {
	...
	remoteKey, err := crypto.ParsePubKey(batch.nodeAnn2.NodeID[:])
...
func TestProcessChannelAnnouncementOptionalMsgFields(t *testing.T) {
	...
	assertOptionalMsgFields := func(chanID lnwire.ShortChannelID,
		capacity chainutil.Amount, channelPoint wire.OutPoint) {
	...
	capacity := chainutil.Amount(1000)
