...
type OpenChannelStore interface {
	...
	// returned.
	FetchOpenChannels(nodeID *crypto.PublicKey) ([]*OpenChannel, error)
...
type OpenChannelCommitmentMutationStore interface {
	...
	InsertNextRevocation(channel *OpenChannel, revKey *crypto.PublicKey) error
...
type LinkNodeMaintainer interface {
	...
	RepairLinkNodes(network wire.FlokicoinNet) error
}
