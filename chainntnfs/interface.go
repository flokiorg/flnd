...
// targeted events on the Flokicoin blockchain. The interface specification is
// intentionally general in order to support a wide array of chain notification
// implementations such as: btcd's websockets notifications, Flokicoin Core's
// ZeroMQ notifications, various Flokicoin API services, Electrum servers, etc.
//
...
type ChainConn interface {
	...
	GetBlockHeaderVerbose(blockHash *chainhash.Hash) (
		*chainjson.GetBlockHeaderVerboseResult, error)
...
type TxIndexConn interface {
	...
	GetRawTransactionVerbose(*chainhash.Hash) (*chainjson.TxRawResult, error)
...
func ConfDetailsFromTxIndex(chainConn TxIndexConn, r ConfRequest,
	...
	blockHeight, err := blockchain.ExtractCoinbaseHeight(
		chainutil.NewTx(block.Transactions[0]),
	)
	if err != nil {
