...
type chanController struct {
	...
	minConfs      int32
	confTarget    uint32
	chanMinHtlcIn lnwire.MilliLoki
	netParams     chainreg.FlokicoinNetParams
}
...
func (c *chanController) OpenChannel(target *crypto.PublicKey,
...
func initAutoPilot(svr *server, cfg *lncfg.AutoPilot,
	minHTLCIn lnwire.MilliLoki, netParams chainreg.FlokicoinNetParams) (
	*autopilot.ManagerCfg, error) {
	...
	atplConstraints := autopilot.NewConstraints(
		chainutil.Amount(cfg.MinChannelSize),
		chainutil.Amount(cfg.MaxChannelSize),
	...
	pilotCfg := autopilot.Config{
		...
		ConnectToPeer: func(target *crypto.PublicKey, addrs []net.Addr) (bool, error) {
	...
	return &autopilot.ManagerCfg{
		...
		ChannelState: func() ([]autopilot.LocalChannel, error) {
			...
			for i, channel := range activeChannels {
				...
				balance := localCommit.LocalBalance.ToLokis()
