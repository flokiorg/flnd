...
var openChannelCommand = cli.Command{
	...
	The channel will be initialized with local-amt loki locally and
	push-amt loki for the remote node. Note that the push-amt is
	...
	Flags: []cli.Flag{
		...
		cli.IntFlag{
			Name: "local_amt",
			Usage: "the number of loki the wallet should " +
		...
		cli.Uint64Flag{
			Name: "base_fee_msat",
			Usage: "the base fee in milli-loki that will " +
		...
		cli.IntFlag{
			Name: "push_amt",
			Usage: "the number of loki to give the remote " +
		...
		cli.Uint64Flag{
			...
			Usage: "(optional) the minimum number of loki we " +
...
var batchOpenChannelCommand = cli.Command{
	...
	The channel will be initialized with local_funding_amount loki 
	locally and push_sat loki for the remote node. Note that specifying 
...
func finalizeMsgFromString(tx string,
	...
	// PSBT decode failed, try to parse it as a hex encoded Flokicoin
	// transaction
