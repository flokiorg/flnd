...
func PaymentFlags() []cli.Flag {
	return []cli.Flag{
		...
		cli.Int64Flag{
			Name: "fee_limit",
			Usage: "maximum fee allowed in loki when " +
				"sending the payment",
		},
		cli.Int64Flag{
...
func SendPaymentRequest(ctx *cli.Context, req *routerrpc.SendPaymentRequest,
	...
	case ctx.Uint64(maxShardSizeSatFlag.Name) != 0:
		req.MaxShardSizeMsat = uint64(lnwire.NewMSatFromLokis(
			chainutil.Amount(ctx.Uint64(maxShardSizeSatFlag.Name)),
		))
	}
...
var payInvoiceCommand = cli.Command{
	...
	Flags: append(PaymentFlags(),
		cli.Int64Flag{
			Name: "amt",
			Usage: "(optional) number of loki to fulfill the " +
				"invoice",
		},
	),
...
var queryRoutesCommand = cli.Command{
	...
	Flags: []cli.Flag{
		...
		cli.Int64Flag{
			Name:  "amt",
			Usage: "the amount to send expressed in loki",
		},
		cli.Int64Flag{
			Name: "fee_limit",
			Usage: "maximum fee allowed in loki when sending " +
				"the payment",
		},
		cli.Int64Flag{
			Name: "fee_limit_percent",
...
var buildRouteCommand = cli.Command{
	...
	via the flokicoin.timelockdelta setting (default 80).
	...
	Flags: []cli.Flag{
		cli.Int64Flag{
			Name: "amt",
			Usage: "the amount to send expressed in loki. If" +
...
var estimateRouteFeeCommand = cli.Command{
	...
	Flags: []cli.Flag{
		...
		cli.Int64Flag{
			Name: "amt",
			Usage: "the payment amount expressed in loki " +
