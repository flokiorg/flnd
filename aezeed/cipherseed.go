...
const (
	...
	// for the Flokicoin Days Genesis timestamp, and 16 bytes for entropy. It
	...
	// addresses, the 2 byte timestamp a BDG (Flokicoin Days Genesis) offset,
...
var (
	...
	// own passphrase.
	defaultPassphrase = []byte("/flc/seed/pub")
)

var (
	// FlokicoinGenesisDate is the timestamp of Flokicoin's genesis block.
	...
	// the genesis date. We refer to this time period as ABE (after Flokicoin
	// era).
	FlokicoinGenesisDate = time.Unix(1631485359, 0)
)
...
type CipherSeed struct {
	...
	// the number of days since the timestamp in the Flokicoin genesis block.
...
func New(internalVersion uint8, entropy *[EntropySize]byte,
	...
	// subtract that from the Flokicoin Genesis Date. We'll then convert that
	// value to days.
	birthday := uint16(now.Sub(FlokicoinGenesisDate) / (time.Hour * 24))
...
func (c *CipherSeed) BirthdayTime() time.Time {
	...
	return FlokicoinGenesisDate.Add(offset)
}
