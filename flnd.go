...
func Main(cfg *Config, lisCfg ListenerCfg, implCfg *ImplementationCfg,
	interceptor signal.Interceptor, flnStarted chan<- struct{}) error {
	...
	var network string
	switch {
	case cfg.Flokicoin.TestNet3:
		network = "testnet"

	case cfg.Flokicoin.TestNet4:
		network = "testnet4"

	case cfg.Flokicoin.MainNet:
		network = "mainnet"

	case cfg.Flokicoin.SimNet:
		network = "simnet"
	...
	ltndLog.InfoS(ctx, "Network Info",
		"active_chain", strings.Title(FlokicoinChainName),
		"network", network)
	...
	go func() {
		flnStarted <- struct{}{} // signal that flnd has started
	}()
	...
	// Stop peer connections if node shuts down before sync completes. #FLZ_CHANGE
	defer func() {
