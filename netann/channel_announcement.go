...
func CreateChanAnnouncement(chanProof *models.ChannelAuthProof,
	...
	*lnwire.ChannelUpdate1, *lnwire.ChannelUpdate1, error) {
	...
	chanAnn := &lnwire.ChannelAnnouncement1{
		...
		FlokicoinKey1:   chanInfo.FlokicoinKey1Bytes,
		FlokicoinKey2:   chanInfo.FlokicoinKey2Bytes,
...
func validateChannelAnn1(a *lnwire.ChannelAnnouncement1) error {
	...
	bitcoinSig1, err := a.FlokicoinSig1.ToSignature()
	if err != nil {
		return err
	}
	bitcoinKey1, err := crypto.ParsePubKey(a.FlokicoinKey1[:])
	if err != nil {
		return err
	}
	...
	// digest as well.
	bitcoinSig2, err := a.FlokicoinSig2.ToSignature()
	if err != nil {
		return err
	}
	bitcoinKey2, err := crypto.ParsePubKey(a.FlokicoinKey2[:])
	if err != nil {
		return err
	}
	...
	nodeKey1, err := crypto.ParsePubKey(a.NodeID1[:])
	if err != nil {
		return err
	}
	...
	nodeKey2, err := crypto.ParsePubKey(a.NodeID2[:])
	if err != nil {
		return err
	}
...
func chanAnn2P2WSHMuSig2Keys(a *lnwire.ChannelAnnouncement2) (
	...
	nodeKey1, err := crypto.ParsePubKey(a.NodeID1.Val[:])
	if err != nil {
		return nil, err
	}

	nodeKey2, err := crypto.ParsePubKey(a.NodeID2.Val[:])
	if err != nil {
		return nil, err
	}
	...
	btcKey1Bytes, err := a.FlokicoinKey1.UnwrapOrErr(
	...
	btcKey1, err := crypto.ParsePubKey(btcKey1Bytes.Val[:])
	if err != nil {
		return nil, err
	}

	btcKey2Bytes, err := a.FlokicoinKey2.UnwrapOrErr(
	...
	btcKey2, err := crypto.ParsePubKey(btcKey2Bytes.Val[:])
	if err != nil {
		return nil, err
	}
...
func chanAnn2P2TRMuSig2Keys(a *lnwire.ChannelAnnouncement2,
	scriptAddr chainutil.Address) ([]*crypto.PublicKey, error) {
	...
	if a.FlokicoinKey1.IsSome() && a.FlokicoinKey2.IsSome() {
		var (
			btcKey1 tlv.RecordT[tlv.TlvType12, [33]byte]
			btcKey2 tlv.RecordT[tlv.TlvType14, [33]byte]
		)

		btcKey1 = a.FlokicoinKey1.UnwrapOr(btcKey1)
		btcKey2 = a.FlokicoinKey2.UnwrapOr(btcKey2)

		bitcoinKey1, err := crypto.ParsePubKey(btcKey1.Val[:])
		if err != nil {
			return nil, err
		}

		bitcoinKey2, err := crypto.ParsePubKey(btcKey2.Val[:])
		if err != nil {
			return nil, err
		}
