...
func TestCreateChanAnnouncement(t *testing.T) {
	...
	chanProof := &models.ChannelAuthProof{
		...
		FlokicoinSig1Bytes: expChanAnn.FlokicoinSig1.ToSignatureBytes(),
		FlokicoinSig2Bytes: expChanAnn.FlokicoinSig2.ToSignatureBytes(),
	}
	chanInfo := &models.ChannelEdgeInfo{
		ChainHash:          expChanAnn.ChainHash,
		ChannelID:          expChanAnn.ShortChannelID.ToUint64(),
		ChannelPoint:       wire.OutPoint{Index: 1},
		Capacity:           chainutil.LokiPerFlokicoin,
		NodeKey1Bytes:      key,
		NodeKey2Bytes:      key,
		FlokicoinKey1Bytes: key,
		FlokicoinKey2Bytes: key,
...
func test4of4MuSig2P2WSHChanAnnouncement(t *testing.T) {
	...
	require.NoError(t, err)
	pkAddr, err := chainutil.NewAddressScriptHash(
...
func test4of4MuSig2P2TRChanAnnouncement(t *testing.T) {
	...
	pkAddr, err := chainutil.NewAddressTaproot(
...
func test3of3MuSig2ChanAnnouncement(t *testing.T) {
	...
	outputKeyPriv, err := crypto.NewPrivateKey()
	require.NoError(t, err)
	...
	pubKeys := []*crypto.PublicKey{node1.nodePub, node2.nodePub, outputKey}
...
func genNonceForPubKey(t *testing.T, pub *crypto.PublicKey) *musig2.Nonces {
...
func genChanAnnKeys(t *testing.T) (*keyRing, *keyRing) {
	...
	aliceNodePrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
