...
// optimize the allocation of loki within channels throughput the network's
...
type Agent struct {
	...
	// totalBalance is the total number of loki the backing wallet is
...
func (a *Agent) openChans(ctx context.Context, availableFunds chainutil.Amount,
...
func (a *Agent) executeDirective(directive AttachmentDirective) {
	...
	pub, err := crypto.ParsePubKey(nodeID[:])
	if err != nil {
