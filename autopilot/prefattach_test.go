...
type testGraph interface {
	...
	addRandChannel(*crypto.PublicKey, *crypto.PublicKey,
		chainutil.Amount) (*ChannelEdge, *ChannelEdge, error)

	addRandNode() (*crypto.PublicKey, error)
}
...
func TestPrefAttachmentSelectEmptyGraph(t *testing.T) {
	...
	for _, chanGraph := range chanGraphs {
		...
		success := t.Run(chanGraph.name, func(t1 *testing.T) {
			...
			const walletFunds = chainutil.LokiPerFlokicoin
...
func TestPrefAttachmentSelectTwoVertexes(t *testing.T) {
	...
	for _, chanGraph := range chanGraphs {
		...
		success := t.Run(chanGraph.name, func(t1 *testing.T) {
			...
			const chanCapacity = chainutil.LokiPerFlokicoin
...
func TestPrefAttachmentSelectGreedyAllocation(t *testing.T) {
	...
	for _, chanGraph := range chanGraphs {
		...
		success := t.Run(chanGraph.name, func(t1 *testing.T) {
			...
			peerPub, err := crypto.ParsePubKey(peerPubBytes[:])
			require.NoError(t1, err)
			...
			// wallet balance to be 5.5 FLC. We're shooting for a
			// 50/50 allocation, and have 3 FLC in channels. As a
			...
			// only 0.5 FLC left. That should leave us with channel
			// candidates of that size.
			const remBalance = chainutil.LokiPerFlokicoin * 0.5
			scores, err = prefAttach.NodeScores(
...
func (d *testDBGraph) addRandChannel(node1, node2 *crypto.PublicKey,
	capacity chainutil.Amount) (*ChannelEdge, *ChannelEdge, error) {

	ctx := context.Background()

	fetchNode := func(pub *crypto.PublicKey) (*models.Node, error) {
		if pub != nil {
	...
	var lnNode1, lnNode2 *crypto.PublicKey
	...
	copy(edge.FlokicoinKey1Bytes[:], lnNode1.SerializeCompressed())
	copy(edge.FlokicoinKey2Bytes[:], lnNode2.SerializeCompressed())
	...
	edgePolicy := &models.ChannelEdgePolicy{
		...
		TimeLockDelta:             10,
		MinHTLC:                   1,
		MaxHTLC:                   lnwire.NewMSatFromLokis(capacity),
		FeeBaseMSat:               10,
...
func (d *testDBGraph) addRandNode() (*crypto.PublicKey, error) {
	nodeKey, err := randKey()
	if err != nil {
		return nil, err
	}
...
func randKey() (*crypto.PublicKey, error) {
...
// meant to aide in the generation of random graphs for use within test cases
// the exercise the autopilot package.
func (m *memChannelGraph) addRandChannel(node1, node2 *crypto.PublicKey,
	capacity chainutil.Amount) (*ChannelEdge, *ChannelEdge, error) {
...
func (m *memChannelGraph) addRandNode() (*crypto.PublicKey, error) {
	newPub, err := randKey()
	if err != nil {
		return nil, err
	}
