...
func (m *mockConstraints) ChannelBudget(chans []LocalChannel,
	balance chainutil.Amount) (chainutil.Amount, uint32) {
...
func (m *mockConstraints) MinChanSize() chainutil.Amount {
	return 1e7
}
func (m *mockConstraints) MaxChanSize() chainutil.Amount {
	return 1e8
}
...
func (m *mockHeuristic) NodeScores(_ context.Context, g ChannelGraph,
	chans []LocalChannel, chanSize chainutil.Amount,
...
func (m *mockChanController) OpenChannel(target *crypto.PublicKey,
	amt chainutil.Amount) error {
...
type testContext struct {
	...
	graph          testGraph
	agent          *Agent
	walletBalance  chainutil.Amount
...
func setup(t *testing.T, initialChans []LocalChannel) *testContext {
	...
	var availableFunds chainutil.Amount = 10 * chainutil.LokiPerFlokicoin
	...
	testCfg := Config{
		...
		Heuristic:      heuristic,
		ChanController: chanController,
		WalletBalance: func() (chainutil.Amount, error) {
			ctx.Lock()
			defer ctx.Unlock()
			return ctx.walletBalance, nil
		},
		ConnectToPeer: func(*crypto.PublicKey, []net.Addr) (bool, error) {
			return false, nil
		},
		DisconnectPeer: func(*crypto.PublicKey) error {
			return nil
		},
		Graph:       memGraph,
		Constraints: constraints,
...
func TestAgentChannelOpenSignal(t *testing.T) {
	...
	// with a capacity of 1 FLC.
	newChan := LocalChannel{
		ChanID:  randChanID(),
		Balance: chainutil.LokiPerFlokicoin,
	}
	...
	// now has an additional channel with one FLC.
...
func TestAgentHeuristicUpdateSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     1 * chainutil.LokiPerFlokicoin,
		},
	)
...
func (m *mockFailingChanController) OpenChannel(target *crypto.PublicKey,
	amt chainutil.Amount) error {
...
func TestAgentChannelFailureSignal(t *testing.T) {
	...
	respondMoreChans(t, testCtx, moreChansResp{1, 5 * chainutil.LokiPerFlokicoin})
...
func TestAgentChannelCloseSignal(t *testing.T) {
	...
	initialChans := []LocalChannel{
		...
		{
			ChanID:  randChanID(),
			Balance: chainutil.LokiPerFlokicoin * 2,
		},
	}
...
func TestAgentBalanceUpdate(t *testing.T) {
	...
	// FLC to the amount of available funds.
	testCtx.Lock()
	testCtx.walletBalance += chainutil.LokiPerFlokicoin * 5
	testCtx.Unlock()
...
func TestAgentImmediateAttach(t *testing.T) {
	...
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: numChans,
			amt:     5 * chainutil.LokiPerFlokicoin,
		},
	)
	...
	for i := 0; i < numChans; i++ {
		...
		case openChan := <-chanController.openChanSignals:
			if openChan.amt != chainutil.LokiPerFlokicoin {
				t.Fatalf("invalid chan amt: expected %v, got %v",
					chainutil.LokiPerFlokicoin, openChan.amt)
			}
...
func TestAgentPendingChannelState(t *testing.T) {
	...
	// query, that it needs more channels and has 3 FLC available for
	// attachment.  We'll send over a response indicating that it should
	// establish more channels, and give it a budget of 1 FLC to do so.
	respondMoreChans(t, testCtx,
		moreChansResp{
			numMore: 1,
			amt:     chainutil.LokiPerFlokicoin,
		},
	)
	...
	// heuristic.
	testCtx.Lock()
	testCtx.walletBalance += 0.4 * chainutil.LokiPerFlokicoin
	testCtx.Unlock()
	...
	// channels.
	select {
	case testCtx.constraints.moreChansResps <- moreChansResp{1, chainutil.LokiPerFlokicoin}:
	case <-time.After(time.Second * 10):
		t.Fatalf("need more chans wasn't queried in time")
	}
...
func TestAgentSkipPendingConns(t *testing.T) {
	...
	testCtx.agent.cfg.ConnectToPeer = func(*crypto.PublicKey, []net.Addr) (bool, error) {
...
func respondWithScores(t *testing.T, testCtx *testContext,
	channelBudget chainutil.Amount, existingChans, newChans int,
...
func checkChannelOpens(t *testing.T, testCtx *testContext,
	allocation chainutil.Amount, numChans int) []NodeID {

	var nodes []NodeID

	// The agent should attempt to open channels, totaling what we expect.
	var totalAllocation chainutil.Amount
	chanController := testCtx.chanController.(*mockChanController)
	for i := 0; i < numChans; i++ {
		select {
		case openChan := <-chanController.openChanSignals:
...
func TestAgentChannelSizeAllocation(t *testing.T) {
	...
	// total channel budget of 5 FLC.
	var channelBudget chainutil.Amount = 5 * chainutil.LokiPerFlokicoin
	...
	// channels.
	expectedAllocation := testCtx.constraints.MaxChanSize() * chainutil.Amount(numNewChannels)
	...
	// Set the channel budget to 1.5 FLC.
	channelBudget = chainutil.LokiPerFlokicoin * 3 / 2

	// We'll return a response telling the agent to open 3 channels, with a
	// total channel budget of 1.5 FLC.
	...
	// our budget.
	channelBudget = chainutil.LokiPerFlokicoin * 5
	numNewChannels = 2
	respondWithScores(
		t, testCtx, channelBudget, numExistingChannels,
		numNewChannels, nodeScores,
	...
	expectedAllocation = testCtx.constraints.MaxChanSize() *
		chainutil.Amount(numNewChannels)
