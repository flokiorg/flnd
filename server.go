...
func newServer(ctx context.Context, cfg *Config, listenAddrs []net.Addr,
	...
	featureMgr, err := feature.NewManager(feature.Config{
		...
		NoExperimentalEndorsement: cfg.ProtocolOptions.NoExpAccountability(),
	...
	thresholdSats := chainutil.Amount(cfg.MaxFeeExposure)
	thresholdMSats := lnwire.NewMSatFromLokis(thresholdSats)
	...
	if cfg.Estimator != nil {
		...
		case routing.BimodalEstimatorName:
			...
			bimodalConfig := routing.BimodalConfig{
				...
				BimodalScaleMsat: lnwire.MilliLoki(
					bCfg.Scale,
				),
	...
	pathFindingConfig := routing.PathFindingConfig{
		AttemptCost: lnwire.NewMSatFromLokis(
	...
	strictPruning := cfg.Flokicoin.Node == "neutrino" ||
	...
	s.fundingMgr, err = funding.NewFundingManager(funding.Config{
		...
		RequiredRemoteDelay: func(chanAmt chainutil.Amount) uint16 {
			...
			delay := uint16(chainutil.Amount(maxRemoteDelay) *
		...
		WatchNewChannel: func(channel *channeldb.OpenChannel,
			peerKey *crypto.PublicKey) error {
		...
		MinChanSize:                   chainutil.Amount(cfg.MinChanSize),
		MaxChanSize:                   chainutil.Amount(cfg.MaxChanSize),
...
func (s *server) createLivenessMonitor(cfg *Config, cc *chainreg.ChainControl,
	...
	if cfg.Flokicoin.Node == "nochainbackend" {
...
func (s *server) Start(ctx context.Context) error {
	...
	s.start.Do(func() {
		...
		if s.cfg.Flokicoin.MainNet {
			setSeedList(
				s.cfg.Flokicoin.DNSSeeds,
				chainreg.FlokicoinMainnetGenesis,
			)
		}
		if s.cfg.Flokicoin.TestNet3 {
			setSeedList(
				s.cfg.Flokicoin.DNSSeeds,
				chainreg.FlokicoinTestnetGenesis,
			)
		}
		if s.cfg.Flokicoin.TestNet4 {
			setSeedList(
				s.cfg.Flokicoin.DNSSeeds,
				chainreg.FlokicoinTestnet4Genesis,
			)
		}
		if s.cfg.Flokicoin.SigNet {
			setSeedList(
				s.cfg.Flokicoin.DNSSeeds,
				chainreg.FlokicoinSignetGenesis,
			)
		}
...
func initNetworkBootstrappers(s *server) ([]discovery.NetworkPeerBootstrapper, error) {
	...
	graphBootstrapper, err := discovery.NewGraphBootstrapper(
		chanGraph, s.cfg.Flokicoin.IsLocalNetwork(),
	)
	if err != nil {
		return nil, err
	}
	...
	// seeds.
	if !s.cfg.Flokicoin.IsLocalNetwork() {
		//nolint:ll
...
// ChannelID.
func (s *server) findChannel(node *crypto.PublicKey, chanID lnwire.ChannelID) (
...
//
// NOTE: This function is safe for concurrent access.
func (s *server) FindPeer(peerKey *crypto.PublicKey) (*peer.Brontide, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// dropped.
func shouldDropLocalConnection(local, remote *crypto.PublicKey) bool {
...
func (s *server) notifyPendingOpenChannelPeerEvent(op wire.OutPoint,
	pendingChan *channeldb.OpenChannel, remotePub *crypto.PublicKey) {
...
func (s *server) peerConnected(conn net.Conn, connReq *connmgr.ConnReq,
	...
	thresholdSats := chainutil.Amount(s.cfg.MaxFeeExposure)
	thresholdMSats := lnwire.NewMSatFromLokis(thresholdSats)
...
//
// NOTE: This function is safe for concurrent access.
func (s *server) DisconnectPeer(pubKey *crypto.PublicKey) error {
...
func (s *server) fetchNodeAdvertisedAddrs(ctx context.Context,
	pub *crypto.PublicKey) ([]net.Addr, error) {
...
func (s *server) SendOnionMessage(ctx context.Context, peerPub [33]byte,
	pathKey *crypto.PublicKey, onion []byte) error {
...
func (s *server) getStartingBeat() (*chainio.Beat, error) {
	...
	if s.cfg.Flokicoin.Node == "nochainbackend" {
...
func (s *server) ChanHasRbfCoopCloser(peerPub *crypto.PublicKey,
