...
put any life altering amounts of FLC into the network.   
...
a wallet birthday (days since FLC genesis block) and a version.   
...
port 5050) use TLS with a self-signed certificate for transport level security.
...
* If an external IP is set (either with `--externalip` or `--tlsextraip`) these
  ...
  point 1.
* If port `5521` (or `10005` for gRPC) was forwarded on the router, these
