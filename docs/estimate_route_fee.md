...
A 1 FLC maximum fee limit prevents unreasonable calculations. The estimate
...
amount. The response will include the routing fee in milliloki and an
...
it uses only local data. Convert the returned fee from milliloki to loki
