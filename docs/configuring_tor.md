...
a Flokicoin full-node, that it is also proxying all traffic over Tor. If using the
...
Tor:
      ...
      --tor.dns=                                              The DNS server as host:port that Tor will use for SRV queries - NOTE must have TCP resolution enabled (default: soa.nodes.ln.myfloki.com:53)
