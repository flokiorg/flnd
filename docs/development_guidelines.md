...
func DeriveRevocationPubkey(commitPubKey *crypto.PublicKey,
	revokePreimage []byte) *crypto.PublicKey {
```
**RIGHT**
```go
...
When making use of structured logging calls (there are any `flog.Logger` 
...
log.InfoS(ctx, "Channel open performed", "user_id", userID,
        flog.Fmt("amount", "%.8f", 0.00154), "channel_id", channelID)

// Example 3.
log.InfoS(ctx, "Bytes received", 
        "user_id", userID, 
        flog.Hex("peer_id", peerID.SerializeCompressed()),
        flog.Hex("message", []bytes{
...
log.InfoS(ctx, "Bytes received", 
        "user_id", userID,
        flog.Hex("peer_id", peerID.SerializeCompressed()),
        flog.Hex("message", []bytes{0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08})))
```
