...
- [Available Backend Operating Modes](#available-backend-operating-modes)
  ...
  - [Flokicoind Options](#bitcoind-options)
...
```text
bitcoind:
      --bitcoind.dir=                                         The base directory that contains the node's data, logs, configuration file, etc. (default: /Users/<username>/Library/Application Support/Flokicoin)
...
Otherwise, replace `--flokicoin.testnet` with `--flokicoin.simnet`. If you are
installing `lnd` in preparation for the
[tutorial](https://dev.lightning.community/tutorial), you may skip this step.
```shell
 lnd --flokicoin.active --flokicoin.testnet --debuglevel=debug \
...
in `--flokicoin.simnet` if needed), and also your own `btcd` node if available:
```shell
 lnd --flokicoin.active --flokicoin.testnet --debuglevel=debug \
       --flokicoin.node=neutrino --neutrino.connect=faucet.lightning.community
```
...
Here's a sample `flokicoin.conf` for use with lnd:
```text
testnet=1
server=1
daemon=1
...
```shell
 lnd --flokicoin.active --flokicoin.testnet --debuglevel=debug \
       --flokicoin.node=bitcoind --bitcoind.rpcuser=REPLACEME \
...
- If you DO choose to explicitly pass the auth parameters in your `lnd.conf` or
  ...
  will attempt to get the configuration from your `flokicoin.conf`.
...
instead of `testnet`, adding the `--flokicoin.simnet` flag instead of the
`--flokicoin.testnet` flag.
...
launches, allowing you to simply type `lnd --flokicoin.testnet --flokicoin.active`
...
Notice the `[Flokicoin]` section. This section houses the parameters for the
Flokicoin chain. See a more detailed sample config file available
...
`[Flokicoind]` and `[Neutrino]` depending on which chain and node type you're
using.
