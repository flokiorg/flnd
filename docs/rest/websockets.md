...
```javascript
const host = 'localhost:5050'; // The default REST port of lnd, can be overwritten with --restlisten=ip:port
...
let ws = new WebSocket('wss://localhost:5050/v2/chainnotifier/register/blocks?method=POST', {
...
const fs = require('fs');
const macaroon = fs.readFileSync('LND_DIR/data/chain/bitcoin/simnet/admin.macaroon').toString('hex');
let ws = new WebSocket('wss://localhost:5050/v1/channels/acceptor?method=POST', {
  // Work-around for self-signed certificates.
  rejectUnauthorized: false,
  headers: {
    'Grpc-Metadata-Macaroon': macaroon,
