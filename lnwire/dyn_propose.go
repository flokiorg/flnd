...
type DynPropose struct {
	...
	FundingKey fn.Option[crypto.PublicKey]
...
func (dp *DynPropose) Encode(w *bytes.Buffer, _ uint32) error {
	...
	dp.DustLimit.WhenSome(func(dl chainutil.Amount) {
		protoSats := uint64(dl)
		tlvRecords = append(
	...
	dp.MaxValueInFlight.WhenSome(func(max MilliLoki) {
	...
	dp.ChannelReserve.WhenSome(func(min chainutil.Amount) {
	...
	dp.FundingKey.WhenSome(func(key crypto.PublicKey) {
		keyScratch := &key
		tlvRecords = append(
			tlvRecords, tlv.MakePrimitiveRecord(
...
func (dp *DynPropose) Decode(r io.Reader, _ uint32) error {
	...
	var fundingKeyScratch *crypto.PublicKey
	...
	if val, ok := typeMap[DPDustLimitSatoshis]; ok && val == nil {
		dp.DustLimit = fn.Some(chainutil.Amount(dustLimitScratch))
	}
	if val, ok := typeMap[DPMaxHtlcValueInFlightMsat]; ok && val == nil {
		dp.MaxValueInFlight = fn.Some(MilliLoki(maxValueScratch))
	}
	if val, ok := typeMap[DPChannelReserveSatoshis]; ok && val == nil {
		dp.ChannelReserve = fn.Some(chainutil.Amount(reserveScratch))
	}
