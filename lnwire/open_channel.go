...
// by nodes backed by SPV Flokicoin clients, and have a simpler security models
...
type OpenChannel struct {
	...
	// FundingAmount is the amount of loki that the initiator of the
	...
	// ChannelReserve is the amount of FLC that the receiving party MUST
	...
	RevocationPoint *crypto.PublicKey
	...
	DelayedPaymentPoint *crypto.PublicKey

	// HtlcPoint is the base point used to derive the set of keys for this
	// party that will be used within the HTLC public key scripts. This
	// value is combined with the receiver's revocation base point in order
	// to derive the keys that are used within HTLC scripts.
	HtlcPoint *crypto.PublicKey
...
func (o *OpenChannel) Encode(w *bytes.Buffer, pver uint32) error {
	...
	if err := WriteLoki(w, o.FundingAmount); err != nil {
		return err
	}

	if err := WriteMilliLoki(w, o.PushAmount); err != nil {
		return err
	}

	if err := WriteLoki(w, o.DustLimit); err != nil {
		return err
	}

	if err := WriteMilliLoki(w, o.MaxValueInFlight); err != nil {
		return err
	}

	if err := WriteLoki(w, o.ChannelReserve); err != nil {
		return err
	}

	if err := WriteMilliLoki(w, o.HtlcMinimum); err != nil {
		return err
	}
