...
func (c *ClosingComplete) RandTestMessage(t *rapid.T) Message {
	msg := &ClosingComplete{
		ChannelID: RandChannelID(t),
		FeeSatoshis: chainutil.Amount(rapid.Int64Range(0, 1000000).Draw(
			t, "feeSatoshis"),
		),
...
func (dp *DynPropose) RandTestMessage(t *rapid.T) Message {
	...
	if includeDustLimit {
		dl := chainutil.Amount(rapid.Uint32().Draw(t, "dustLimit"))
		msg.DustLimit = fn.Some(dl)
	}

	if includeMaxValueInFlight {
		mvif := MilliLoki(rapid.Uint64().Draw(t, "maxValueInFlight"))
		msg.MaxValueInFlight = fn.Some(mvif)
	}

	if includeChannelReserve {
		cr := chainutil.Amount(rapid.Uint32().Draw(t, "channelReserve"))
