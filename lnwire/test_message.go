...
func (a *AnnounceSignatures1) RandTestMessage(t *rapid.T) Message {
	return &AnnounceSignatures1{
		...
		FlokicoinSignature: RandSignature(t),
...
func (a *ChannelAnnouncement1) RandTestMessage(t *rapid.T) Message {
	// Generate Node IDs and Flokicoin keys (compressed public keys)
	...
	return &ChannelAnnouncement1{
		...
		FlokicoinSig1:   RandSignature(t),
		FlokicoinSig2:   RandSignature(t),
		Features:        RandFeatureVector(t),
		ChainHash:       hash,
		ShortChannelID:  RandShortChannelID(t),
		NodeID1:         nodeID1,
		NodeID2:         nodeID2,
		FlokicoinKey1:   bitcoinKey1,
		FlokicoinKey2:   bitcoinKey2,
		ExtraOpaqueData: RandExtraOpaqueData(t, nil),
	}
}
...
func (c *ChannelAnnouncement2) RandTestMessage(t *rapid.T) Message {
	...
	if rapid.Bool().Draw(t, "includeFlokicoinKey1") {
		var bitcoinKey1 [33]byte
		copy(bitcoinKey1[:], RandPubKey(t).SerializeCompressed())
		msg.FlokicoinKey1 = tlv.SomeRecordT(
	...
	if rapid.Bool().Draw(t, "includeFlokicoinKey2") {
		var bitcoinKey2 [33]byte
		copy(bitcoinKey2[:], RandPubKey(t).SerializeCompressed())
		msg.FlokicoinKey2 = tlv.SomeRecordT(
...
func (c *ChannelReady) RandTestMessage(t *rapid.T) Message {
	...
	includeAnnouncementFlokicoinNonce := rapid.Bool().Draw(
		t, "includeAnnouncementFlokicoinNonce",
	)
	...
	if includeAnnouncementFlokicoinNonce {
		nonce := RandMusig2Nonce(t)
		msg.AnnouncementFlokicoinNonce = tlv.SomeRecordT(
...
func (a *ChannelUpdate1) RandTestMessage(t *rapid.T) Message {
	...
	maxHtlc := MilliLoki(rapid.Uint64().Draw(t, "maxHtlc"))
	...
	return &ChannelUpdate1{
		...
		HtlcMinimumMsat: MilliLoki(rapid.Uint64().Draw(
			t, "htlcMinimum"),
		),
...
func (c *ChannelUpdate2) RandTestMessage(t *rapid.T) Message {
	...
	htlcMinMsat := MilliLoki(rapid.IntRange(1, 10000).Draw(
		t, "htlcMinMsat"),
	)
	htlcMaxMsat := MilliLoki(rapid.IntRange(10000, 100000000).Draw(
		t, "htlcMaxMsat"),
	)
	...
	msg := &ChannelUpdate2{
		...
		HTLCMinimumMsat: tlv.NewPrimitiveRecord[tlv.TlvType12, MilliLoki](
			htlcMinMsat,
		),
		HTLCMaximumMsat: tlv.NewPrimitiveRecord[tlv.TlvType14, MilliLoki](
			htlcMaxMsat,
		),
...
func (c *ClosingComplete) RandTestMessage(t *rapid.T) Message {
	msg := &ClosingComplete{
		ChannelID: RandChannelID(t),
		FeeLokis: chainutil.Amount(rapid.Int64Range(0, 1000000).Draw(
			t, "feeLokis"),
		),
...
func (c *ClosingSigned) RandTestMessage(t *rapid.T) Message {
	...
	msg := &ClosingSigned{
		ChannelID: RandChannelID(t),
		FeeLokis: chainutil.Amount(
			rapid.Int64Range(0, 1000000).Draw(t, "feeLokis"),
		),
		ExtraData: RandExtraOpaqueData(t, nil),
	}
...
func (dp *DynPropose) RandTestMessage(t *rapid.T) Message {
	...
	if includeDustLimit {
		var rec tlv.RecordT[tlv.TlvType0, tlv.BigSizeT[chainutil.Amount]]
		val := chainutil.Amount(rapid.Uint32().Draw(t, "dustLimit"))
		rec.Val = tlv.NewBigSizeT(val)
		msg.DustLimit = tlv.SomeRecordT(rec)
	}

	if includeMaxValueInFlight {
		var rec tlv.RecordT[tlv.TlvType2, MilliLoki]
		val := MilliLoki(rapid.Uint64().Draw(t, "maxValueInFlight"))
		rec.Val = val
		msg.MaxValueInFlight = tlv.SomeRecordT(rec)
	}

	if includeChannelReserve {
		var rec tlv.RecordT[tlv.TlvType6, tlv.BigSizeT[chainutil.Amount]]
		val := chainutil.Amount(rapid.Uint32().Draw(t, "channelReserve"))
		rec.Val = tlv.NewBigSizeT(val)
...
func (c *UpdateAddHTLC) RandTestMessage(t *rapid.T) Message {
	msg := &UpdateAddHTLC{
		...
		Amount: MilliLoki(rapid.Uint64().Draw(t, "amount")),
