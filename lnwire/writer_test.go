...
func TestWriteLoki(t *testing.T) {
	buf := new(bytes.Buffer)
	data := chainutil.Amount(1)
	expectedBytes := []byte{0, 0, 0, 0, 0, 0, 0, 1}

	err := WriteLoki(buf, data)

	require.NoError(t, err)
	require.Equal(t, expectedBytes, buf.Bytes())
}

func TestWriteMilliLoki(t *testing.T) {
	buf := new(bytes.Buffer)
	data := MilliLoki(1)
	expectedBytes := []byte{0, 0, 0, 0, 0, 0, 0, 1}

	err := WriteMilliLoki(buf, data)
