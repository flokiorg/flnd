...
// WriteLoki appends the Loki value to the provided buffer.
func WriteLoki(buf *bytes.Buffer, amount chainutil.Amount) error {
	return WriteUint64(buf, uint64(amount))
}

// WriteMilliLoki appends the MilliLoki value to the provided buffer.
func WriteMilliLoki(buf *bytes.Buffer, amount MilliLoki) error {
	return WriteUint64(buf, uint64(amount))
}

// WritePublicKey appends the compressed public key to the provided buffer.
func WritePublicKey(buf *bytes.Buffer, pub *crypto.PublicKey) error {
	if pub == nil {
