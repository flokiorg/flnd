...
func TestPureTLVMessage(t *testing.T) {
	...
	var (
		_, pkA   = crypto.PrivKeyFromBytes([]byte{1})
		_, pkB   = crypto.PrivKeyFromBytes([]byte{2})
		capacity = MilliLoki(100)
	)
...
type MsgV1 struct {
	...
	NodeKey  tlv.RecordT[tlv.TlvType0, *crypto.PublicKey]
	Capacity tlv.OptionalRecordT[tlv.TlvType1, MilliLoki]
...
func newMsgV1(nodeKey *crypto.PublicKey, capacity *MilliLoki) *MsgV1 {
	newMsg := &MsgV1{
...
func (g *MsgV1) Decode(r io.Reader, _ uint32) error {
	var capacity = tlv.ZeroRecordT[tlv.TlvType1, MilliLoki]()
...
func (g *MsgV1) AllRecords() []tlv.Record {
	...
	g.Capacity.WhenSome(
		func(capacity tlv.RecordT[tlv.TlvType1, MilliLoki]) {
...
type MsgV2 struct {
	...
	FlokicoinKey tlv.OptionalRecordT[tlv.TlvType3, *crypto.PublicKey]
...
func newMsgV2(nodeKey *crypto.PublicKey, capacity *MilliLoki,
	btcKey *crypto.PublicKey, spvProof []byte, num, other uint8,
	...
	if btcKey != nil {
		newMsg.FlokicoinKey = tlv.SomeRecordT(
...
func (g *MsgV2) Decode(r io.Reader, _ uint32) error {
	var (
		capacity   = tlv.ZeroRecordT[tlv.TlvType1, MilliLoki]()
		btcKey     = tlv.ZeroRecordT[tlv.TlvType3, *crypto.PublicKey]()
	...
	if _, ok := typeMap[g.FlokicoinKey.TlvType()]; ok {
		g.FlokicoinKey = tlv.SomeRecordT(btcKey)
	}
...
func (g *MsgV2) AllRecords() []tlv.Record {
	...
	g.Capacity.WhenSome(
		func(c tlv.RecordT[tlv.TlvType1, MilliLoki]) {
			recordProducers = append(recordProducers, &c)
		},
	)
	g.FlokicoinKey.WhenSome(
		func(key tlv.RecordT[tlv.TlvType3, *crypto.PublicKey]) {
