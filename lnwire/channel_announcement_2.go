...
type ChannelAnnouncement2 struct {
	...
	// Capacity is the number of loki of the capacity of this channel.
	...
	// FlokicoinKey1 is the public key of the key used by Node1 in the
	...
	FlokicoinKey1 tlv.OptionalRecordT[tlv.TlvType12, [33]byte]

	// FlokicoinKey2 is the public key of the key used by Node2 in the
	// construction of the on-chain funding transaction. This is an optional
	// field and only needs to be set if the 4-of-4 MuSig construction was
	// used in the creation of the message signature.
	FlokicoinKey2 tlv.OptionalRecordT[tlv.TlvType14, [33]byte]
...
func (c *ChannelAnnouncement2) nonSignatureRecordProducers() []tlv.RecordProducer {
	...
	c.FlokicoinKey1.WhenSome(func(key tlv.RecordT[tlv.TlvType12, [33]byte]) {
		recordProducers = append(recordProducers, &key)
	})

	c.FlokicoinKey2.WhenSome(func(key tlv.RecordT[tlv.TlvType14, [33]byte]) {
		recordProducers = append(recordProducers, &key)
	})
...
func (c *ChannelAnnouncement2) Decode(r io.Reader, _ uint32) error {
	...
	if _, ok := typeMap[c.FlokicoinKey1.TlvType()]; ok {
		c.FlokicoinKey1 = tlv.SomeRecordT(btcKey1)
	}

	if _, ok := typeMap[c.FlokicoinKey2.TlvType()]; ok {
		c.FlokicoinKey2 = tlv.SomeRecordT(btcKey2)
	}
