...
// ECDSA sig, unlike Flokicoin, we use fixed sized signatures on the wire,
// instead of DER encoded signatures. This type provides several methods to
// convert to/from a regular Flokicoin DER encoded signature (raw bytes and
// *ecdsa.Signature).
type Sig struct {
	bytes [64]byte
...
// NewSigFromECDSARawSignature returns a Sig from a Flokicoin raw signature
