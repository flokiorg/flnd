...
func TestSignatureSerializeDeserialize(t *testing.T) {
	...
	r.Sub(crypto.S256().N, r)
	rScalar := new(crypto.ModNScalar)
	rScalar.SetByteSlice(r.Bytes())

	sig := ecdsa.NewSignature(rScalar, new(crypto.ModNScalar).SetInt(128))
	...
	// Check R = N-1, S = 127.
	sig = ecdsa.NewSignature(rScalar, new(crypto.ModNScalar).SetInt(127))
	err = signatureSerializeDeserialize(sig)
	if err != nil {
	...
	// Check R = N-1, S = N>>1.
	s := new(big.Int).Set(crypto.S256().N)
	s.Rsh(s, 1)
	sScalar := new(crypto.ModNScalar)
