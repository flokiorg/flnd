...
type ChannelReady struct {
	...
	NextPerCommitmentPoint *crypto.PublicKey
	...
	// AnnouncementFlokicoinNonce is an optional field that stores a public
	// nonce that will be used along with the node's bitcoin key during
	// signing of the ChannelAnnouncement2 message.
	AnnouncementFlokicoinNonce tlv.OptionalRecordT[tlv.TlvType2, Musig2Nonce]
...
func NewChannelReady(cid ChannelID, npcp *crypto.PublicKey) *ChannelReady {
	return &ChannelReady{
		ChanID:                 cid,
...
func (c *ChannelReady) Encode(w *bytes.Buffer, _ uint32) error {
	...
	c.AnnouncementFlokicoinNonce.WhenSome(
