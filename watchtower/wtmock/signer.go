...
type MockSigner struct {
	...
	index uint32
	keys  map[keychain.KeyLocator]*crypto.PrivateKey
}

// NewMockSigner returns a fresh MockSigner.
func NewMockSigner() *MockSigner {
	return &MockSigner{
		keys: make(map[keychain.KeyLocator]*crypto.PrivateKey),
	}
}
...
func (s *MockSigner) AddPrivKey(privKey *crypto.PrivateKey) keychain.KeyLocator {
	s.mu.Lock()
	defer s.mu.Unlock()
