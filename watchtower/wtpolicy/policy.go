...
import (
	...
	"github.com/flokiorg/flnd/watchtower/wtwire"
...
const (
	...
	// transactions. The value is expressed in loki per kilo-weight.
...
func (p *Policy) ComputeAltruistOutput(
	totalAmt chainutil.Amount, txWeight lntypes.WeightUnit,
	sweepScript []byte) (chainutil.Amount, error) {
...
func (p *Policy) ComputeRewardOutputs(totalAmt chainutil.Amount,
	txWeight lntypes.WeightUnit,
	rewardScript []byte) (chainutil.Amount, chainutil.Amount, error) {
...
func ComputeRewardAmount(total chainutil.Amount, base, rate uint32) chainutil.Amount {
	rewardBase := chainutil.Amount(base)
	rewardRate := chainutil.Amount(rate)
