...
import (
	...
	"github.com/flokiorg/flnd/watchtower/wtmock"
...
var (
	...
	addr, _ = chainutil.DecodeAddress(
...
func randPrivKey(t *testing.T) *crypto.PrivateKey {
	t.Helper()
...
type mockChannel struct {
	...
	revSK     *crypto.PrivateKey
	revPK     *crypto.PublicKey
	revKeyLoc keychain.KeyLocator

	toRemoteSK     *crypto.PrivateKey
	toRemotePK     *crypto.PublicKey
	toRemoteKeyLoc keychain.KeyLocator

	toLocalPK *crypto.PublicKey // only need to generate to-local script

	dustLimit lnwire.MilliLoki
	csvDelay  uint32
}

func newMockChannel(t *testing.T, signer *wtmock.MockSigner,
	localAmt, remoteAmt lnwire.MilliLoki) *mockChannel {
...
func (c *mockChannel) createRemoteCommitTx(t *testing.T) {
	...
	if c.remoteBalance >= c.dustLimit {
		commitTxn.TxOut = append(commitTxn.TxOut, &wire.TxOut{
			Value:    int64(c.remoteBalance.ToLokis()),
	...
	if c.localBalance >= c.dustLimit {
		commitTxn.TxOut = append(commitTxn.TxOut, &wire.TxOut{
			Value:    int64(c.localBalance.ToLokis()),
...
func (c *mockChannel) sendPayment(t *testing.T, amt lnwire.MilliLoki) {
	t.Helper()
...
func (c *mockChannel) receivePayment(t *testing.T, amt lnwire.MilliLoki) {
	t.Helper()
...
func (h *testHarness) makeChannel(id uint64,
	localAmt, remoteAmt lnwire.MilliLoki) {
...
func (h *testHarness) sendPayments(id, from, to uint64,
	amt lnwire.MilliLoki) []blob.BreachHint {
...
func (h *testHarness) removeTower(pubKey *crypto.PublicKey, addr net.Addr) {
	h.t.Helper()
...
func newServerHarness(t *testing.T, mockNet *mockNet, netAddr string,
	...
	cfg := &wtserver.Config{
		...
		NewAddress: func() (chainutil.Address, error) {
			return addr, nil
		},
	}
...
const (
	localBalance  = lnwire.MilliLoki(100000000)
	remoteBalance = lnwire.MilliLoki(200000000)
)
		...
		fn: func(h *testHarness) {
			var (
				capacity = h.cfg.localBalance +
					h.cfg.remoteBalance
				paymentAmt = lnwire.MilliLoki(2000000)
