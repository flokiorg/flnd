...
func genTaskTest(
	...
	revSK, revPK := crypto.PrivKeyFromBytes(revPrivBytes)
	_, toLocalPK := crypto.PrivKeyFromBytes(toLocalPrivBytes)
	toRemoteSK, toRemotePK := crypto.PrivKeyFromBytes(toRemotePrivBytes)
	...
	return backupTaskTest{
		...
		expTotalAmt:      chainutil.Amount(toLocalAmt + toRemoteAmt),
...
var (
	...
	sweepAddrScript, _ = chainutil.DecodeAddress(
