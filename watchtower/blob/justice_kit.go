...
func (l *legacyJusticeKit) ToLocalOutputSpendInfo() (*txscript.PkScript,
	...
	revocationPubKey, err := crypto.ParsePubKey(l.revocationPubKey[:])
	if err != nil {
		return nil, nil, err
	}

	localDelayedPubKey, err := crypto.ParsePubKey(l.localDelayPubKey[:])
	if err != nil {
		return nil, nil, err
	}
...
func (l *legacyJusticeKit) ToRemoteOutputSpendInfo() (*txscript.PkScript,
	...
	if !crypto.IsCompressedPubKey(l.commitToRemotePubKey[:]) {
	...
	toRemotePubKey, err := crypto.ParsePubKey(toRemoteScript)
	if err != nil {
...
func (l *legacyJusticeKit) HasCommitToRemoteOutput() bool {
	return crypto.IsCompressedPubKey(l.commitToRemotePubKey[:])
}
...
func (a *anchorJusticeKit) ToRemoteOutputSpendInfo() (*txscript.PkScript,
	...
	if !crypto.IsCompressedPubKey(a.commitToRemotePubKey[:]) {
		return nil, nil, 0, ErrNoCommitToRemoteOutput
	}

	pk, err := crypto.ParsePubKey(a.commitToRemotePubKey[:])
	if err != nil {
		return nil, nil, 0, err
	}
...
func (t *taprootJusticeKit) ToRemoteOutputSpendInfo() (*txscript.PkScript,
	...
	toRemotePk, err := crypto.ParsePubKey(t.commitToRemotePubKey[:])
	if err != nil {
		return nil, nil, 0, err
	}
...
func (t *taprootJusticeKit) HasCommitToRemoteOutput() bool {
	return crypto.IsCompressedPubKey(t.commitToRemotePubKey[:])
}
...
func isOddPub(key *crypto.PublicKey) bool {
