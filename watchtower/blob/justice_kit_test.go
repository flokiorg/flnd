...
func makePubKey() *crypto.PublicKey {
...
type descriptorTest struct {
	...
	decVersion           Type
	sweepAddr            []byte
	revPubKey            *crypto.PublicKey
	delayPubKey          *crypto.PublicKey
	commitToLocalSig     lnwire.Sig
	hasCommitToRemote    bool
	commitToRemotePubKey *crypto.PublicKey
...
type remoteWitnessTest struct {
	name             string
	blobType         Type
	expWitnessScript func(pk *crypto.PublicKey) []byte
	expWitnessStack  func(sig input.Signature) wire.TxWitness
	createSig        func(*crypto.PrivateKey, []byte) input.Signature
}
	...
	tests := []remoteWitnessTest{
		{
			...
			createSig: func(priv *crypto.PrivateKey,
...
func testJusticeKitRemoteWitnessConstruction(t *testing.T,
	...
	toRemotePrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)

	revKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)

	toLocalKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
...
type localWitnessTest struct {
	name               string
	blobType           Type
	expWitnessScript   func(delay, rev *crypto.PublicKey) []byte
	expWitnessStack    func(sig input.Signature) wire.TxWitness
...
func testJusticeKitToLocalWitnessConstruction(t *testing.T,
	...
	revPrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)

	delayPrivKey, err := crypto.NewPrivateKey()
	require.NoError(t, err)
