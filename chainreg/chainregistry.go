...
import (
	...
	"github.com/flokiorg/flnd/chainntnfs/bitcoindnotify"
	"github.com/flokiorg/flnd/chainntnfs/btcdnotify"
	"github.com/flokiorg/flnd/chainntnfs/neutrinonotify"
	...
	"github.com/flokiorg/flnd/routing/chainview"
...
// function.
type Config struct {
	// Flokicoin defines settings for the Flokicoin chain.
	...
	// FlokicoindMode defines settings for connecting to a bitcoind node.
	...
	ActiveNetParams FlokicoinNetParams
	...
	// TCP connections to Flokicoin peers in the event of a pruned block being
	// requested.
	Dialer chain.Dialer
}

const (
	// DefaultFlokicoinMinHTLCInMSat is the default smallest value htlc this
	...
	DefaultFlokicoinMinHTLCInMSat = lnwire.MilliLoki(1)

	// DefaultFlokicoinMinHTLCOutMSat is the default minimum htlc value that
	...
	DefaultFlokicoinMinHTLCOutMSat = lnwire.MilliLoki(1000)

	// DefaultFlokicoinBaseFeeMSat is the default forwarding base fee.
	...
	// delta.
	DefaultFlokicoinTimeLockDelta = 800

	// DefaultFlokicoinStaticFeePerKW is the fee rate of 50 sat/vbyte
	// expressed in sat/kw.
	DefaultFlokicoinStaticFeePerKW = chainfee.SatPerKWeight(12500)

	// DefaultFlokicoinStaticMinRelayFeeRate is the min relay fee used for
	// static estimators.
...
func NewPartialChainControl(cfg *Config) (*PartialChainControl, func(), error) {
	cc := &PartialChainControl{
		Cfg: cfg,
		RoutingPolicy: models.ForwardingPolicy{
			MinHTLCOut:    cfg.Flokicoin.MinHTLCOut,
			BaseFee:       cfg.Flokicoin.BaseFee,
			FeeRate:       cfg.Flokicoin.FeeRate,
			TimeLockDelta: cfg.Flokicoin.TimeLockDelta,
		},
		MinHtlcIn: cfg.Flokicoin.MinHTLCIn,
		FeeEstimator: chainfee.NewStaticEstimator(
			DefaultFlokicoinStaticFeePerKW,
			DefaultFlokicoinStaticMinRelayFeeRate,
		),
	}
	...
	case "bitcoind":
		bitcoindMode := cfg.FlokicoindMode
		...
		if strings.Contains(bitcoindMode.RPCHost, ":") {
			...
			if cfg.Flokicoin.RegTest || cfg.Flokicoin.SigNet {
		...
		if err := flokicoinConn.Start(); err != nil {
			return nil, nil, fmt.Errorf("unable to connect to "+
				"bitcoind: %v", err)
		}

		chainNotifier := bitcoindnotify.New(
			flokicoinConn, cfg.ActiveNetParams.Params, hintCache,
		...
		cc.ChainView = chainview.NewFlokicoindFilteredChainView(
		...
		if cfg.Fee.URL == "" {
			...
			cc.FeeEstimator, err = chainfee.NewFlokicoindEstimator(
		...
		// bitcoind version.
		cmd, ver, err := getFlokicoindHealthCheckCmd(chainConn)
		if err != nil {
			return nil, nil, err
		}
		...
		cc.HealthCheck = func() error {
			...
			if cfg.Flokicoin.IsLocalNetwork() {
				return nil
			}
	...
	case cfg.Fee.URL != "":
		...
		cacheFees := !cfg.Flokicoin.RegTest
...
// getFlokicoindHealthCheckCmd queries bitcoind for its version to decide which
...
// getblockchaininfo.
func getFlokicoindHealthCheckCmd(client *rpcclient.Client) (string, int64, error) {
...
var (
	// FlokicoinTestnetGenesis is the genesis hash of Flokicoin's testnet
	// chain.
	FlokicoinTestnetGenesis = *chaincfg.TestNet3Params.GenesisHash

	// FlokicoinTestnet4Genesis is the genesis hash of Flokicoin's testnet4
	// chain.
	FlokicoinTestnet4Genesis = *chaincfg.TestNet4Params.GenesisHash

	// FlokicoinSignetGenesis is the genesis hash of Flokicoin's signet chain.
	FlokicoinSignetGenesis = *chaincfg.SigNetParams.GenesisHash

	// FlokicoinMainnetGenesis is the genesis hash of Flokicoin's main chain.
	FlokicoinMainnetGenesis = *chaincfg.MainNetParams.GenesisHash
	...
	ChainDNSSeeds = map[chainhash.Hash][][2]string{
		FlokicoinMainnetGenesis: {
			{
				"nodes.ln.myfloki.com",
				"soa.nodes.ln.myfloki.com",
			},
		},

		FlokicoinTestnetGenesis: {
			{
				"test.nodes.ln.myfloki.com",
				"soa.test.nodes.ln.myfloki.com",
			},
		},

		FlokicoinSignetGenesis: {
			{
				"signet.nodes.ln.myfloki.com",
				"soa.signet.nodes.ln.myfloki.com",
			},
		},
	}
)
